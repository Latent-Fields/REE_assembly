...
## Amend — availability maintenance: activity-silent trace + maintained-pool readout (`crf-availability-maintenance`; V3-EXQ-666 successor, 2026-06-11)

**Status:** IMPLEMENTED 2026-06-11 (ree-v3 main). Routed by the confirmed `failure_autopsy_V3-EXQ-666_2026-06-11` + the targeted lit-pull `evidence/literature/targeted_review_arc_063_crf_rule_cell_persistence/` (SYNTHESIS verdict: **B-leaning hybrid**). New `substrate_queue.json` entry `crf-availability-maintenance` (kept distinct from the ARC-062 differentiation/gating work, per the user's autopsy-Step-8 choice).

**Problem (a differentiation↔persistence tension, not a falsification).** V3-EXQ-666 (the readiness diagnostic for the 654b mature-pool amend) decomposed the readiness gate into its two sub-properties and found they trade off:

- **Differentiation is delivered only by `crf_context_from_e2_world_forward`.** ARM_1 (raw z_world ctx) is too self-similar — the mint-block collapses the pool to ~2 rules, `crf_max_pairwise_rule_dist`=0.0. ARM_2 (e2-world-forward ctx) mints 10–16 *distinct* rules, dist 1.71. This **confirms and strengthens** the 654b/ARC-065 GAP-A dependency.
- **But more differentiation worsens persistence.** Once each rule matches only a narrow context slice, its match-triggered availability EMA never accumulates above θ between sparse matches and decays in the gaps (`mature_availability_decay` 0.001/tick), so `crf_frac_active` collapses to **0.016** (ARM_2) — *worse* than the undifferentiated legacy 0.125. Every arm clears 0/3 readiness cells.

The next binding constraint is **availability MAINTENANCE under sparse per-rule matching**, not the conflict-gate θ (which 654b already fixed). The match-triggered-EMA scheme has the *symbol* of a PFC rule cell but not its *maintenance* functional role.

**Lit verdict (the A-vs-B fork the autopsy posed).** The targeted review resolved to **B (activity-silent synaptic maintenance), with a bounded role for A.** The unselected differentiated *pool* is held activity-silently (Mongillo 2008 calcium-mediated synaptic facilitation; Stokes 2015; Lundqvist–Herman–Miller 2018 "delay activity yes, persistent activity maybe not" — averaging hides sparse-but-maintained coding); persistent firing (Funahashi 1989; Compte/Wang 2000) is reserved for the *single engaged* rule and is capacity-bounded to one attractor (cannot scale to a 10–16-rule pool). Crucially the verdict says `crf_frac_active` — an *instantaneous* active fraction — is the **wrong readiness readout** for a sparsely-matched pool, and that this survives whichever side of the firing-vs-silent debate prevails (Constantinidis 2018 is the live rebuttal; even the persistent camp concedes a maintained item need not be read out every moment).

**Fix (no-op-default; bit-identical OFF; new behaviour behind one master flag).** Two load-bearing pieces (the lit says doing only one will not lift the gate) + one optional:

1. **Maintenance (prescription 1 — Mongillo activity-silent trace).** Under `availability_maintenance`, the per-tick *silence* decay in `credit()` is **removed** — a minted differentiated rule holds its availability across context-absent ticks. The eligibility-gated negative-outcome credit (the exception/interference path) and retirement are **untouched**, so a consistently-bad rule still erodes and retires; only the gaps stop being punished. A `maintenance_floor` (default 0.45, above the mature 2-way-match θ(1)=0.40) is applied **at mint** so a freshly minted differentiated rule starts robustly reactivatable; it is *not* re-floored every tick, so exceptions can still drive a rule below it. `maintenance_decay` (default 0.0 = pure hold) is the optional "long-horizon leak" knob (set the horizon from the measured typical inter-match interval, not the biological ~1 s constant).
2. **Readout (prescription 2 — maintained-pool metric, the "changes the readout" branch, CONFIRMED).** `get_state()` gains `crf_n_maintained_reactivatable` (minted rules whose availability ≥ the reactivation threshold = the single-match gate floor, i.e. would clear θ if their context recurred — independent of whether it is present this tick), `crf_maintained_pairwise_dist` (differentiation *of the maintained subset*), and `crf_frac_maintained`. The 666-successor readiness gate is re-stated on the maintained pool — **`crf_maintained_pairwise_dist > 0.1 AND crf_n_maintained_reactivatable ≥ 2`** — *retiring* the `crf_frac_active ≥ 0.30` target. `crf_frac_active` is kept as the **secondary** active-on-match efficiency readout, not the persistence criterion.
3. **(Optional, default OFF) Prescription 3 — engaged-rule reverberation.** `engaged_sustain` adds a short sustained-activity bump to the matched-and-selected rule (fork-A complement, Funahashi/Compte-Wang). Not the pool fix; default off.

**Config (all no-op default; bit-identical OFF).** `CandidateRuleFieldConfig.availability_maintenance` (False) + `maintenance_floor` (0.45) + `maintenance_decay` (0.0) + `engaged_sustain` (False) + `engaged_sustain_rate` (0.1) + `maintained_reactivation_threshold` (0.0 = derive from the gate floor). Mirrored as `REEConfig.crf_*` + `from_dims` passthrough, forwarded into the field config at the `agent.py` build site (getattr fallback → absent flat attr is bit-identical). **Designed to run *with* `crf_mature_pool_dynamics` + `crf_context_from_e2_world_forward`** (the differentiation source); the three stay independent flags so each is individually ablatable.

**MECH-094 / phased training.** Unchanged — the maintenance logic lives entirely inside the field's already-simulation-gated state-advancing methods; no new trained parameters, no gradient flow, no phased training. Evidence-staleness (Step 8.5): NOT triggered — no-op-default flag; every existing experiment uses the default (legacy decay + active-fraction readout), so no dependent claim's measured mechanism changed. KEEP all evidence.

**Validation.** Substrate-level: contracts C17 (maintenance default-OFF bit-identical on the two-regime sequence even with absurd `maintenance_*` values + maintained-pool readout keys present/consistent), C18 (under sparse matching, maintenance HOLDS a differentiated ≥2-rule reactivatable pool where legacy mature lets it erode out of the set), C19 (`from_dims` + agent wiring of the maintenance flags) — **19/19 CRF contracts + 7/7 preflight PASS.** Activation smoke (mint two distinct rules, then 3000 context-absent ticks): legacy mature → both rules **retired** (`n_minted=0`, `n_maintained=0`, READY=False — the 666 churn); maintenance → pool **held** (`n_minted=2`, `n_maintained=2`, `maintained_pairwise_dist=1.49`, READY=True); `crf_frac_active≈0.001` in **both** arms — confirming `crf_frac_active` is the wrong readout and the maintained-pool metric is the right one. The CRF-readiness validation EXQ (claim-free 666-successor diagnostic: mature + e2-context + maintenance arms vs OFF, gating on the **maintained-pool** metric, not `crf_frac_active`) is the Step-8 follow-on; the 654c GAP-B behavioural re-run is a SEPARATE later `/queue-experiment` session gated on that readiness PASS. MECH-309/ARC-062/ARC-063 stay candidate / substrate_ceiling / v3_pending / pending_retest_after_substrate — **not** weakened; `claims.yaml`/governance untouched (substrate-only amend).

## Amend — CRF conflict-gate calibration (`crf-availability-maintenance` at the CRF locus, ungated from GAP-A; V3-EXQ-654d successor, 2026-06-17)

**Status:** IMPLEMENTED 2026-06-17 (ree-v3 main). Routed by the confirmed `failure_autopsy_V3-EXQ-654d_2026-06-16` (which hands off `/implement-substrate` amend `crf-availability-maintenance` at the CRF locus). Verify-first finding that motivated it: the 569i top-k-shortlist GAP-A substrate **does** de-collapse the consumed_summary spread (0.073/0.089/0.084 > the 0.05 floor on 3/3 seeds), yet 654d — which already armed an equivalent GAP-A de-collapse — still produced `crf_frac_active=0.0` on all 3 ARM_ON seeds. So the GAP-B blocker is **not** the GAP-A selection-authority context; it is the CRF conflict gate, independent of GAP-A.

**Problem (the gate locks out the whole maintained pool; 654d-measured, not inferred).** The 666c maintenance amend (above) **solved** mint + maintain: `crf_max_pairwise_rule_dist`=1.711, the pool holds ≥2 differentiated rules, and 654d recorded the discriminator the 654c autopsy lacked — `crf_mean_n_matched` 7.08/7.29/8.70. The 16 minted rules' `context_tag`s are mutually within `context_match_threshold` (cosine ≥ 0.5), so **7–8 of them co-match the per-tick context**, and `gate_and_select` then gates **every one** out: `theta = mature_tolerance_floor(0.15) + mature_tolerance_conflict_gain(0.25)·(n_matched−1)` → θ(7)=1.65 ≫ `maintenance_floor` 0.45 (and ≫ the 1.0 availability ceiling). `mean_prop_counterfactual_delta=0.0` confirms the gated-out rule_state never reaches committed action. The 654d controlled test (GAP-A de-collapse armed, gate still locked, n_matched *higher*) proves this is **two distinct loci**: the cross-candidate E3 selection channel (`consumed_summary`, the GAP-A fix) and the cross-rule CRF match context key are different vectors. The 654c re-queue dependency ("gate 654d behind GAP-A context de-collapse") was the right caution but the **wrong dependency**.

**Fix (no-op-default; bit-identical OFF; all consulted only under `mature_pool_dynamics`).** Three independently-togglable levers (matching the MECH-314a/b/c / MECH-341 ablation precedent so 654e can dissociate which carries the load):

1. **`mature_context_match_threshold` (FAULT 1 — context-key crowding).** Sentinel `< 0` (default) → `gate_and_select` uses the legacy `context_match_threshold` (0.5). When ≥ 0 (e.g. 0.7) the **gate** match cutoff is sharpened so fewer of the clustered `context_tag`s co-match a per-tick context → `n_matched` falls to ~2–3. Decoupled from the retrieval threshold (`context_match_threshold`, still used by `maintained_reactivatable_rules`) and from mint-block (`mature_mint_block_threshold`, 0.8): the differentiated pool stays minted; only fewer co-fire per tick.
2. **`tolerance_conflict_cap` (FAULT 2a — theta growth).** Sentinel `< 0` (default) → no cap. When ≥ 0 (e.g. 3) θ uses `min(n_competing, cap)`, bounding θ at `theta_floor + theta_gain·cap` (0.15 + 0.25·3 = 0.90 < 1.0) so it stays reachable under match-crowding spikes.
3. **`maintenance_couple_to_theta` (FAULT 2b — the winner-admit, the load-bearing piece).** Default `False` → legacy maintenance hold. When `True` (with `availability_maintenance` + `mature_pool_dynamics`) the per-tick maintenance step in `credit()` floors each maintained rule's availability to `max(maintenance_floor, θ(_last_n_matched) + ε)` (ε = `MAINTENANCE_THETA_EPS` = 1e-3, clamped to [0,1]) so the maintained differentiated pool **clears** the (capped) gate rather than being suppressed wholesale — electing the differentiated set to co-fire (the autopsy: "a maintained set of differentiated rules must be *selectable*, not gated out by mutual crowding"). Effective only with the cap (or a sharpened match cutoff) keeping θ < 1.0; an uncapped θ > 1.0 cannot be cleared even at maximal availability.

A shared `_theta_for(n_matched)` helper (floor + gain + cap) and `_gate_match_threshold()` keep `gate_and_select` and the `credit()` coupling in exact agreement on the gate. `get_state()` gains `crf_last_theta` (the capped θ at the last match count) + `crf_gate_match_threshold` (the cutoff used); `crf_frac_active` + `crf_n_matched_last` (the readiness signal the 654e C1c precondition reads) were already instrumented by 654d.

**Config (all no-op default; bit-identical OFF).** `CandidateRuleFieldConfig.mature_context_match_threshold` (−1.0) + `tolerance_conflict_cap` (−1) + `maintenance_couple_to_theta` (False). Mirrored as `REEConfig.crf_*` + `from_dims` passthrough, forwarded into the field config at the `agent.py` build site (getattr fallback → absent flat attr is bit-identical).

**MECH-094 / phased training.** Unchanged — all three levers are pure-arithmetic gate calibration inside the field's already-simulation-gated state-advancing methods; no new trained parameters, no gradient flow, no phased training. Evidence-staleness (Step 8.5): NOT triggered — no-op-default levers; every existing experiment uses the defaults (legacy match cutoff, no cap, legacy maintenance hold), so no dependent claim's measured mechanism changed. KEEP all evidence.

**Validation.** Substrate-level contracts C20–C24: C20 (levers default-OFF bit-identical — sentinel cutoff/uncapped θ; the crowded maintained pool stays gated out, the 654d lockout reproduced); C21 (sharpen reduces `n_matched` vs the legacy cutoff on the same pool, without starving to zero); C22 (cap bounds θ below 1.0 while not lowering θ when `n_competing ≤ cap`); **C23 (the LOAD-BEARING 654d inversion — the SAME crowded maintained differentiated pool that legacy gates out wholesale, `frac_active`=0, clears the gate under sharpen+cap+couple, `frac_active`≥0.30)**; C24 (`from_dims` + agent wiring). **24/24 CRF contracts + 7/7 preflight PASS.** Activation smoke (8-rule maintained pool, context_tags spread across cosine [0.52,0.80] to the query): LEGACY n_matched=8, θ=1.90, **n_active=0, frac_active=0.000** (the 654d lockout); ARMED (cutoff 0.7 → n_matched=3, cap 3 → θ=0.65, couple → maintained availability floored to clear) **n_active=3, frac_active=0.98**. The behavioural validation is `/queue-experiment` **V3-EXQ-654e** (the 654d GAP-B committed-class-entropy falsifier ported onto this amend's `frac_active` readiness gate — NOT on GAP-A — arming `mature_context_match_threshold=0.7` + `tolerance_conflict_cap=3` + `maintenance_couple_to_theta=True`, with the C1c `crf_frac_active≥0.30` self-routing precondition that now expects to clear). MECH-309/ARC-062/ARC-063 stay candidate / substrate_ceiling / v3_pending / pending_retest_after_substrate — **not** weakened (654d was non_contributory); `claims.yaml`/governance untouched (substrate-only amend).
