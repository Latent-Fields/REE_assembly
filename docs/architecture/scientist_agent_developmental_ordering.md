...
**V3 substrate status: IMPLEMENTED 2026-06-23** (`ree_core/attribution/scientist_attribution_buffer.py`, `ScientistAttributionBuffer`). The waking-phase feedstock the MECH-275 `BayesianAggregator` consumes is now built: on each waking tick the agent runs the single-pass comparators (SD-031 `E2WorldForward`, domain `place`; ARC-033 `E2HarmSForward`, domain `self`) on `(z_prev, observed, a_actual)` and buffers, per `(domain, region)`, the agency residual `||z_observed - E2(z_prev, a_actual)||` tagged counterfactual-backed iff the counterfactual contrast `||E2(z_prev, a_actual) - E2(z_prev, a_cf)|| >= cf_margin`. The sleep loop (`phase_manager.py`) reads this buffer as the aggregator's evidence **instead of** the MECH-284 staleness scalar when `use_scientist_attribution` is on. No-op-default (`use_scientist_attribution=False`); MECH-094-gated (no buffering on replay/simulation ticks). The `only_counterfactual_backed` lever is the falsifiable distinction — set False for the correlational-control arm (predicted noise-fit). PROMOTES NOTHING; substrate-readiness validation V3-EXQ-703. See `ree-v3/CLAUDE.md`.
