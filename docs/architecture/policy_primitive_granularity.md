...
**ARC-071 substrate BUILT 2026-07-22** (`ree-v3/ree_core/policy/policy_chunking.py`) — MECH-323 formation operator (`ChunkAccumulator`) + MECH-324 maintenance operator (`ChunkLibrary`) + the MECH-322 sleep-replay carve-out, default OFF and bit-identical when OFF. Registered parameter defaults implemented verbatim; 25 interface contracts in `ree-v3/tests/contracts/test_arc071_policy_chunking.py`. No experimental evidence yet — the substrate-readiness diagnostic is queued separately, and all four claims stay `v3_pending: true`.
**ARC-070 substrate BUILT 2026-07-24** (`ree-v3/ree_core/policy/policy_decomposition.py`) — MECH-321 `policy.decomposition_via_event_segmenter`, the rollout-side consumer of the MECH-288 event_segmenter bidirectional substrate (its `input_stream` extension landed 2026-07-24 as MECH-321's substrate prerequisite). Withholds or re-segments an ARC-071 chunk candidate under the R1 OR trigger (region V_s drop OR a MECH-288 rollout-stream boundary firing), recursively up to the R3 depth cap (default 3), and adds a mid-execution abort hook (R4 second phase) alongside the existing MECH-091 / rung-6 / SD-034 principled commit-latch releases. Default OFF and bit-identical when OFF; 21 interface contracts in `ree-v3/tests/contracts/test_arc070_policy_decomposition.py` (incl. C11-C16 for the R5 bottleneck trigger mode added 2026-07-25). No experimental evidence yet — a substrate-readiness diagnostic is queued separately (the full ARM_0/ARM_1/ARM_2 discriminative-pair design from MECH-321's functional_restatement is a later experiment), and both claims stay `v3_pending: true`. The family's decomposition direction is no longer missing: a chunk can now form, dissolve, AND be re-decomposed under prediction failure — closing the loop the 2026-07-22 ARC-071 landing left open.
...
- **Optional secondary (R5) — now a selectable ONLINE trigger mode (BUILT 2026-07-25):** `REEConfig.decomposition_trigger_mode = "bottleneck"` makes MECH-321 decompose *only* at bottleneck states, regardless of V_s (the ARM_2 arm). Bottleneck detection is an online incremental McGovern-Barto diverse-density accumulator (visit-frequency + funnel-topology gates) over a coarse quantised **z_world region key** — deliberately *not* the monotonic MECH-288 `segment_id`, which never recurs. **Faithfulness note:** the R5 synthesis frames bottleneck detection as an *offline / consolidation-phase* mechanism (batch analysis of the ARC-071 ChunkLibrary for bottleneck topology); this landing operationalises the *same statistical signal online* to satisfy ARM_2's primary-trigger requirement, and does **not** claim to be the offline consolidation pass — that (and its integration with the ARC-071 chunk-formation pipeline, once the ARC-071 R6 governance decision lands) stays deferred. Default OFF/bit-identical; the R1 "vs_boundary" mode is unchanged.
...
**MECH-321 substrate is now built (2026-07-24).** `ree-v3/ree_core/policy/policy_decomposition.py` implements the per-tick evaluate() decision (R1 OR trigger against a caller-supplied region V_s and `MECH-288.boundary_on(stream="rollout", ...)`) plus a one-level, library-aware tiling helper (`decompose_sequence`); `ree_core/hippocampal/module.py` wires it into `propose_trajectories` (pre-commit, R4 first phase — withholds a triggering ARC-071 chunk candidate and replaces it with recursively-tiled leaf-tile candidates up to `depth_cap`) and `ree_core/agent.py` adds a mid-execution abort hook (R4 second phase — releases the MECH-090 commit latch when the REMAINING content of an executing chunk trajectory triggers). Default OFF and bit-identical when OFF; no hippocampal sub-config mirror needed (HippocampalModule acts on whether `set_decomposition_source()` was called, not on a config flag it reads itself — the same external-source-injection pattern ARC-071 already established for `_chunk_source`). **The R5 bottleneck trigger mode was added 2026-07-25** (`decomposition_trigger_mode`; see the R5 bullet above), making the full three-arm ARM_0/ARM_1/ARM_2 dissociation buildable — the ARM_0/ARM_1 (OFF / V_s-drop) half is V3-EXQ-816, and ARM_2 (bottleneck) is now added on top. Next: complete the three-arm discriminative validation via `/queue-experiment`.
