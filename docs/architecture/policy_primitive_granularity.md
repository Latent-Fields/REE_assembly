...
**MECH-094 hypothesis_tag**: ARC-070 fires during simulation (must respect MECH-094 — decomposition during waking action does not write residue). ARC-071 fires from real executed sequences (`hypothesis_tag=False` only — a hallucinated chunk would be catastrophic).
