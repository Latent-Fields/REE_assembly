...
| Residue not propagated across transitions | Harm repeated without accumulation; no moral continuity | Psychopathy | **Strongly supported** -- 3-node cascade: amygdala generation failure + encoding failure + vmPFC update failure (PMID 15997022, 16866595, 26359751, 18458210) |
| Social/identity constraints omitted from C | Norm violation despite intact world model | Frontal damage / bvFTD | **Strongly supported** -- knowledge/application dissociation (PMID 1791934, 4069365); constraint removal by vmPFC damage (PMID 17377536); progressive erosion bvFTD (PMID 21810890) |

This failure-mode taxonomy is registered as **MECH-126** (`state_abstraction.failure_modes_psychiatric_analogs`). All nine entries are now grounded with literature evidence. The claim is structural analogy, not identity -- REE state components are not identical to empirical constructs, and causal direction between abstraction failure and psychiatric syndrome is not resolved for most tracks. Three organizing axes span the full taxonomy: (1) hippocampal pattern separation/completion balance (overmerge, anxiety, PTSD, ASD); (2) PFC-hippocampal temporal/constraint binding (ADHD, frontal damage); (3) amygdala-vmPFC valence generation and propagation cascade (mania, depression, psychopathy). The vmPFC is the critical integration node across axes 2 and 3.
