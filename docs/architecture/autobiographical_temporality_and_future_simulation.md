...
- **Taxonomy decision (2026-06-14, ABM-1 / Q-060, user-adjudicated):** ARC-085 is scoped as an **indexing / binding convention over the existing ARC-007 episodic store, NOT a new parallel store**. The "one self-tagged event-token store" *is* the existing episodic store, made autobiographical by the fields each token carries plus an identity index that both backward replay (ARC-007) and forward re-composition (ARC-018) address. Downstream claims (MECH-365/366/361/368/429/430/431) stay field/gate specialisations of the episodic token. Not trivial — the index is still load-bearing V4 work — but no third memory machinery is minted. See Q-060 below.
...
**Decision (2026-06-14, ABM-1, user-adjudicated):** sub-question (1) is answered — autobiographical-event memory is a **tag / indexing convention** over the existing episodic store (ARC-007), **not** a distinct first-class type with its own store. The distinguishing features (`self_state, perspective, affect, residue, source_status, committed_vs_imagined`) are fields on the episodic event token; MECH-121 already models de-tagged content draining toward semantic, so autobiographical content is episodic content that *retains* its self/affect tags and resists full semanticisation. Semantic and task/procedural stay distinct types; only autobiographical-vs-episodic resolves to tag-not-type. Q-060 stays an `open_question` for the record because the empirical falsifier could reopen it — its `what_would_answer` is the ARC-085 joint-degradation test (a V4 experiment): joint degradation confirms the single tagged store; independent degradation would reopen toward a distinct type.
