---
title: "SD-032: Cingulate Integration Substrate"
...
**Amendment (2026-07-21) — μ/κ stability overlays on the mode PRIOR (MECH-048).** The 2026-04-19 implementation gave `pcc_stability` authority over the switch *threshold* and nothing else. `SalienceCoordinatorConfig.affinity_weights["pcc_stability"]` was left empty, so H(`operating_mode`) was **exactly invariant** under μ — measured by live execution on a constructed agent (2026-07-21, session `practical-volhard-2c1876`): 0.167605 at `pcc_stability` 0.0, 1.0 *and* 3.0, identical to 6 dp, while κ (`aic_salience`) moved it freely via its own affinity weight.

That is a substrate gap with a claim-level consequence. MECH-048 asserts the overlays shape **both** mode-prior sharpness (entropy) **and** switching inertia; only the second was built. Any experiment measuring mode entropy against μ would therefore have reported an arithmetic zero rather than a measurement — the DV-symmetry vacuity class of `failure_autopsy_V3-EXQ-604c_2026-07-20.md` §3, and the same defect that made the V3-EXQ-683 design unqueueable. MECH-048's entropy half was untestable by construction, on every substrate and every seed.

The amendment couples μ into the softmax temperature in the literal form given by the claim's own source, `docs/thoughts/2026-02-11_some_control_plane_maths_hypotheses.md:63`:

```
tau = softmax_temperature * exp(alpha_kappa * kappa - alpha_mu * mu)
```

with μ = `pcc_stability` (SD-032d) and κ = `aic_salience` (SD-032c). High μ → lower temperature → sharper mode prior → lower entropy ("stabilise once the regime is safe and coherent"); high κ → flatter prior ("re-evaluation pressure"). The threshold half is the existing `stability_scaling` multiplier and is the same doc's `theta = theta_0 + rho_mu*mu - rho_kappa*kappa` (ibid.:104) — unchanged.

*Why not an `affinity_weights["pcc_stability"]` entry.* That was the other candidate mechanism and it is ruled out by the claim text, not merely disfavoured: a per-mode logit bias makes μ a mode **preference**, whereas `control_plane.md#mech-048` states the overlays "are not scalar reward signals; they act as stability and entropy modulators over control-plane regimes". The empty dict is now documented as intentional rather than as a stub.

Config: `REEConfig.salience_use_stability_temperature` (bool, default **False** → `effective_temperature` is `softmax_temperature` exactly, no `exp()` evaluated, every tick bit-identical to the pre-amendment substrate). Sub-knobs `salience_temperature_mu_alpha=1.0`, `salience_temperature_kappa_alpha=**0.0**`, `salience_temperature_exponent_clip=4.0`. α_κ defaults to zero so that flipping the master switch isolates the μ leg — a single-lever ablation; κ already reaches the mode logits through its affinity weight, so enabling both at once confounds two paths. The exponent clip exists because `aic_salience` is unbounded above (`urgency_scaled + extra_sum`), and an unclipped exponent can overflow or drive the prior to a degenerate point; ±4.0 keeps the temperature factor in [0.018, 54.6]. The multiplicative-exponential form (rather than an additive one) also guarantees τ > 0, so the coordinator's `temperature <= 0` guard is unreachable.

`tick()` additionally returns `effective_temperature` and `mode_entropy` (natural-log Shannon entropy of `operating_mode`), so consumers read the MECH-048 DV rather than recomputing it inconsistently.

Phased training: not applicable (non-trainable arithmetic, no gradient flow). MECH-094: not applicable (no memory content authored).

**Scope caution for experiment design.** With μ *injected directly*, "entropy falls as μ rises" is an arithmetic identity of the coupling — the contract tests assert it as a **substrate-wiring** property, and it is **not** evidence for MECH-048. A validation experiment must drive μ from upstream environmental state (safety / coherence / task success, through `PCCAnalog`) and read a downstream behavioural DV; asserting H(mode) against a directly-set μ would reproduce the exact vacuity this amendment exists to remove. Contract tests: `ree-v3/tests/contracts/test_mech048_stability_temperature.py` (7 contracts, all on continuous readouts — none on a sampled discrete mode, which is not reproducible across machine classes).

**Still open after this amendment (separate build, not bundled).** MECH-048's *switching-pressure* half remains empirically unmeasurable for an independent reason: the contested modes are never occupied. V3-EXQ-464b / 464c / 467d / 464d all FAIL with `fraction_in_external_task = 0.0` at every seed with `use_external_task_drive=True` across a 20× hysteresis sweep; `mean_dwell` is an episode-length artefact and `n_switches == n_episodes` (episode-boundary settles only). 467d self-routed `substrate_not_ready_requeue` / `external_task_mode_not_occupied`. That is a distinct `/implement-substrate` build.
