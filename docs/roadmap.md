...
### Immediate Work Queue (This Cycle, 2026-06-14)

1. **V3-EXQ-677 MECH-180 + V3-EXQ-672a MECH-057b `/failure-autopsy` adjudication** -- both FAILs held by the 2026-06-13 evening governance cycle for `/failure-autopsy` per user directive. Decide `non_contributory` / `non_degenerate` / weakens / superseded per-case; remember the indexer reads `runs/<run_id>/manifest.json` not the flat manifest, so any autopsy correction must be propagated into the pack copy or it will be silently ignored by the next governance cycle.
