...
Generation prompts for the six-part [REE for Psychiatrists](../ree_for_psychiatrists.md) deck.
Each block is a **standalone** prompt that produces one ~10-card chapter. Paste **one block at a
time** into Gamma via **Create new → Paste in text** (paste mode, not "Generate from a prompt"),
choose **one card per divider**, then merge the six decks in Google Slides
(*File → Import slides*, keep original theme).

Each prompt opens with a `DO NOT` guard banning the generic REE intro slides — without it, Gamma
regenerates the Part 1 framing for every chapter. Shared instruction for all six: *audience =
consultant psychiatrists, no ML background; grand-rounds tone; explain each term plainly; one idea
per card (headline + 3–5 bullets); honesty guardrail — REE is a research-stage specification, not a
deployed AI, clinical tool, or validated disease model.*

> Created 2026-06-12. The merged deck (corrected: Part 2 ladder numbered from 0; Part 5 UK spelling)
> is the canonical copy reproduced on the [REE for Psychiatrists](../ree_for_psychiatrists.md) page.
