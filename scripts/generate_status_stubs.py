...
EVIDENCE_FRAGMENT = os.path.join(ROOT, "docs", "evidence_strip.json")
...
def _strip_items_data(data):
    """The strip's items as data, in render order. Single source of the
    framing rules -- see render_evidence_strip for why they exist."""
    if not isinstance(data, dict):
        return []
    items = []
    pct = data.get("closure_pct")
    if pct is not None:
        ...
        if done is not None and total is not None:
            sub = "%s of %s nodes complete" % (_fmt_int(done), _fmt_int(total))
        items.append({"value": "%s%%" % round(float(pct)),
                      "label": "v3 closure", "sub": sub})
    backed = data.get("claims_with_supporting_evidence")
    if backed is not None:
        items.append({"value": _fmt_int(backed),
                      "label": "claims with supporting experimental evidence",
                      "sub": "each linked to at least one run that supports it"})
    runs = data.get("runs_logged")
    if runs is not None:
        items.append({"value": _fmt_int(runs),
                      "label": "logged experiment runs", "sub": None})
    weakens = data.get("evidence_weakens")
    if weakens is not None:
        entries = data.get("evidence_entries")
        items.append({"value": _fmt_int(weakens),
                      "label": "results recorded as weakening a claim",
                      "sub": ("of %s linked evidence entries" % _fmt_int(entries)
                              if entries is not None else None)})
    return items


def render_evidence_fragment(now):
    """docs/evidence_strip.json -- the same figures, published for the org site.

    WHY PRE-RENDERED ITEMS AND NOT RAW FIGURES: the framing rules in
    render_evidence_strip (never show claims_total; take evidence counts
    from the claim/evidence join, never from claim status) are the whole
    point of this strip. Publishing raw numbers would invite the consuming
    site to re-derive its own labels and get them wrong. Publishing the
    rendered items enforces the framing at the source. `figures` is included
    alongside for provenance, not for relabelling.

    `generated_at` exists so a consumer can refuse to display stale data.
    That gate is the consumer's job and it is not optional -- a fetch that
    silently succeeds against a months-old file is the failure mode here.
    """
    payload = {
        "generated_at": now,
        "source": "REE_assembly scripts/generate_status_stubs.py",
        "items": [],
        "figures": {},
    }
    try:
        ...
        if _here not in sys.path:
            sys.path.insert(0, _here)
        import evidence_strip_data
        figures = evidence_strip_data.gather_evidence_figures(ROOT) or {}
        payload["figures"] = figures
        payload["items"] = _strip_items_data(figures)
    except Exception:
        pass
...
def main():
    ...
    outputs = [
        ...
        (EVIDENCE_FRAGMENT, render_evidence_fragment(now)),
    ]
