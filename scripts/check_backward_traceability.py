...
TRACEABILITY_EXEMPT = {
    ...
    "MECH-542": "source-sensitive selective permeability (INV-011 decomposed); 'repertoire' is one item in an enumeration of what a social/imagination model REPRESENTS ('other agents and their possible motivations, policy repertoire, harm/benefit expectations, social contingencies, counterfactual volume') -- a policy inventory, not a developmental behavioural repertoire; same shape as MECH-442/MECH-503",
    "MECH-543": "drama-vs-fiction generative machinery; 'play' matches 'enacted role-play', a MODE of cognitive engagement (running one's own machinery under another identity's boundary conditions), not developmental/pretend play. The claim makes no stage, curriculum or maturational-ordering commitment -- its prediction is about update fingerprints in an already-capable system. Withdraw this exemption if MECH-543 ever acquires a developmental-ordering commitment, or if it is wired as the mechanism under DEV-NEED-012 (pretend play / counterfactual play-frame handling), which is where its developmental content would live",
    "MECH-560": "governed selectivity beats transfer volume; 'REPERTOIRE' is one of six DISCRIMINATING QUANTITIES measured over inter-engine influence (amount / selectivity / stability / repertoire / behavioural appropriateness / causal necessity) -- a measurement axis, not a developmental behavioural repertoire; same shape as MECH-442/ARC-071/MECH-323/MECH-503",
    "GOV-CONTRACT-2": "contract-invariant PROMOTION LADDER (governance_rule); the stage matcher fires on 'Stage 0 named canonical candidate', which is ladder rung 0 of 6 for evidence admission, not a developmental stage. NOTE (structural): a GOV-* id can never clear this gate by adding a register row -- extract_register_claim_ids() matches r'(INV|ARC|MECH|SD|Q|IMPL|RA)-\\d+' and has no GOV- alternative -- so an exemption is the ONLY available disposition for a GOV-* claim that trips a developmental keyword",
    "GOV-ECOL-1": "seeds-versus-worlds reporting rule (governance_rule); 'pretend' matches the VERB in 'rather than pretending it exists' (describing how the rule names the frozen ecological suite as a future requirement), not pretend play. Same structural GOV-* note as GOV-CONTRACT-2: exemption is the only available disposition",
