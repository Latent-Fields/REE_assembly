...
|----------|-----------------|-------------------|-------------|-------------------|-------------|------------|---------------------|
| **ARC-065** diversity stack (umbrella) | partial: MECH-313 + MECH-314 component readiness PASSed; integrated 6-arm heartbeat run (V3-EXQ-561) non_contributory | V3-EXQ-561 diversity stack heartbeat (non_contributory 2026-05-14); entropy=0.0 all 6 arms including entropy-ceiling ARM_3 (alpha=5.0) | **pre-Rung 7**: Layer 1 CEM candidate collapse; all K candidates share identical first-action argmax regardless of temperature or score bias; post-CEM mechanisms cannot diversify | V3-EXQ-561 entropy=0.0 even at ARM_3 entropy ceiling (alpha=5.0) proves collapse is at CEM proposal stage, not post-CEM; candidate_first_action_entropy=0.0 confirmed in EXQ-563 dry-run | uncalibrated-weights (10) + upstream CEM collapse blocking measurement | V3-EXQ-563 forced actuator test (queued 2026-05-14): ARM_4 forced_score_bias_per_class [-2,0,0,0] tests Layer 1 directly; ARM_2/ARM_5 test MECH-320 v_t_floor fix | sleep experiments, ARC-062 rule apprehension, SD-029 |
...
| **MECH-320** tonic vigor | landed: V3-EXQ-547 non_contributory (substrate readable; v_t not moving); V3-EXQ-549 non_contributory (discriminative pair); sign/scale root cause confirmed 2026-05-14; v_t_floor fix implemented in substrate | V3-EXQ-549 ARC-066 discriminative pair (non_contributory 2026-05-11); v_t=0 confirmed; V3-EXQ-561 ARM_1 (use_tonic_vigor natural) also v_t=0 | **Rung 2 confirmed**: root cause identified -- reward_signal = -float(score), E3 score ~70, v_raw approx -70, v_t = max(0, -70) = 0; signal inverted by negation in score-receipt, gate floor kills it | v_raw sign inverted (E3 lower-is-better score is large positive; negation makes v_raw deeply negative; ReLU gate zeroes it); substrate fix: TonicVigorConfig.v_t_floor (default 0.0) bypasses sign gate; forced_score_bias_per_class added to REEConfig as direct Layer 1 actuator | gate-open-zero-signal (2) + mismatched-convention (8) -- SIGN CONFIRMED | V3-EXQ-563 forced actuator test: ARM_1 (tonic_vigor natural, expected v_t=0); ARM_2 (v_t_floor=1.0, expected v_t>0 P3); ARM_5 (combined floor + forced bias) | ARC-065, action selection, wanting/approach pipeline |
...
| **B** | ARC-065 / MECH-320 combined heartbeat: 5-arm matched-entropy test | V3-EXQ-561 non_contributory 2026-05-14 (entropy=0.0 all 6 arms; Layer 1 CEM collapse root cause confirmed; MECH-320 sign/scale root cause confirmed); V3-EXQ-563 forced actuator test queued 2026-05-14 |
