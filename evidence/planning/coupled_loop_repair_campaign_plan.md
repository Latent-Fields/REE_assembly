...
| N2 | E2 world | does retained replay keep the L2R bar while the encoder trains through the read path (Q4c)? | - | /queue-experiment (pinned) | puzzle (known rules) | **ready** (not-started) | - | pre-register + run probe N2 pinned to `9b322d5`: arms = W6a OFF / W6a ON + W3 `replay_latent` `reencode` / W6a ON + W3 `stored` (config only). Cost: ~0.25 s per W6a update at alpha 0.3 (window 26, batch 64) -> size for a cloud worker | 2026-09-25T22:41Z |
