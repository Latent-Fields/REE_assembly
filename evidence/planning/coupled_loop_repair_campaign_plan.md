...
| I1 | instruments | campaign acceptance instruments in `experiments/_lib` (section 3, I1) | MAIN-I | /implement-substrate | complicated (buildable) | **in-progress** (built + contracts green, NOT yet on main) | fleet box for the pre-commit contracts gate | all six instruments + canary tests + F1 staged in `.scratch/wt-i1`; tests/contracts 5745 passed on cloud; commit+push per `.scratch/breakthrough-20260924/i1/RESUME_I1.md`. `cem_trace_diag.py` preserved (REE_assembly `a393dc4cfe`) | 2026-09-25 |
...
| N3 | E3 aggregation | probe N3: which aggregation tracks true consequence with the W3 head (section 4) | - | pinned probe | puzzle (known rules) | not-started (N3-pre proxy: **CANNOT_DETERMINE**) | W3 | N3-pre (`n3_pre_e3_aggregation_probe_20260925.md`, `d4bb6449b3`; PROVISIONAL, a proxy L2R head, not the W3 member): shuffled twin at chance on 3/5 seeds (4/5 required). No aggregation passes (a)-(c). (a) passes for D1 5/5 (SD-081-incompatible) and DISC_0.5 4/5. (b) is capped by valuation (oracle pick in Q-best 0.09-0.22, chance 0.20), so it depends on W5. (c) is non-discriminating, because INIT is the reference. Next: the W4 gate owner decides the (b)/(c) gate form. Fix the twin (class relabelling, not marginal resampling). Then run N3 proper on the W3 member with DISC_0.5 as the lead candidate | 2026-09-25 14:17Z |
...
| W1-alt | proposal (action space) | action-space proposals, the parallel alternative to the W1 codec (user decision rec-20260925-6a675285): stratified one-hot first action + per-class categorical CEM on the continuation, no decoder / `terrain_prior` on the act path, no trainable params (not a trainer member); default-OFF `use_action_space_proposals` + 3 knobs; member gate G-ASP (a)-(f), whose (e) is consumer-mediated (the literal W1(e) containment form is degenerate for a stratified pool). Design: `action_space_proposals_design_20260925.md` (412882b845) | BRANCH | /implement-substrate | complicated (buildable); gate (e)/(f) complex (probe-gated) | **in-progress** (`bt0925-w1alt`: BUILT, 4 knobs incl. ASP-0 mode (U2) and score window (U4); OFF bit-identical; contracts green on cloud targeted run; branch commit gate in flight -- sha to follow) | branch push; gate (e)/(f): W3, W4, I1 | push to branch. Member gate: (a) PASS, (b) PASS, (d) PASS for ASP-E and ASP-0 (contract, D1); (c) needs the W3 head (untrained E2 grows x2.9/H for ASP and codec alike). ASP-R FAILS (d) coverage at floor 0.02 (4/40) and (b) on its final pool (SP-CEM zero-continuation token on collapse), both strict xfails. ASP-E pool is format-homogeneous (all one-hot, no zero tails, no decoder-scale candidates; gate (b) asserts it on the final pool); open: chunk injection / R5b scaffold would add zero tails if turned ON. Record: `action_space_proposals_build_20260925.md` (28c74e80e5). U1, U6 still held | 2026-09-25 15:25Z |
...
| A1 | acceptance | integrated closed-loop acceptance run (section 5), queued from main, `ree_core` pinned to the branch sha | queue (main) | /queue-experiment (user fixes the pre-registration) | complex (probe-gated): the run IS the probe | blocked | W6, N0, I1 | pre-registration **DRAFT v2** (2026-09-25 14:37Z; REE_assembly `c0939a9b4e` + `8fd30619b3`, skeleton `6d1face27d` + `8fd30619b3`): user decisions folded in (floors 0.90/2.4, contacts 1.6/4.8, change 1.44, P1g strict; NOVAL arms required in GROUNDED; consumer-mediated (e) leg on both gates) and design edits E1-E9 (INT-ASP -> INT-ACT; one CEM window + one W3 buffer format for both). **HELD until INT-CODEC and INT-ACT both pass their member gates** (rec-20260925-38b81685; `a1_queueable`). Open before queueing: RT-5 floor probe; tie rule when both PASS (user, at A1 time); N3-pre valuation cap on pick-in-Q-best readouts, which may also block both (e) legs until W5 (O3); CEM window value (U4); P4 headroom at 1.44 (O6) | 2026-09-25 12:58Z |
...
| F1-F3 | small fixes | section 8 | MAIN | per fix | complicated (buildable) | F1 **in-progress** (rides the I1 commit); F2, F3 not-started | F1: as I1 | F1 lands with I1; F2/F3 route per section 6 | 2026-09-25 |
