...
| W3 | E2 world | `E2WorldMember` over a trainer-owned buffer: on-policy plus 25% retained babbling replay, stored as **raw obs** and re-encoded at replay time. FROZEN state only in this step. | BRANCH | /implement-substrate | complicated (buildable) | **done on branch** (`042895a`; branch now `4070b0efa4`: rebased onto main with the SD-008 reset-init knobs, gate (a) contract disc4-only per the 09:33Z decision, record `branch_rebase_w3_disc4_contract_20260926.md`) | - | none. Member gate PASS (D1) on the babbling-probe protocol, seeds 106-110: (a) 4/5 (disc4 0.460-0.523, k 10 on 5/5), (b) retention 5/5 (0.96-1.71), (c) shuffled twin 0/5, (d) guard PASS, (e) bounded (t30/t0 1.06-1.26). Objective MSE by default (native InfoNCE fails the bar, k = 0); O7 set to the defaults (raw obs re-encoded + action as fed to E2). A1 O15 wiring built (`export_retained` step-indexed log, `append_external` / `schedule_external`). Record `w3_e2_world_member_build_20260925.md` (f82cb986c5). **Gate (a) is now disc4_h1 >= 0.47 ALONE per the 09:33Z decision (3), k dropped** (contract change: `branch_rebase_w3_disc4_contract_20260926.md`; this row's own "(a) 4/5 (... k 10 on 5/5)" numbers stay as originally recorded, per that record's own note, and are not rescored here). Re-read on the 15-seed reliability sweep (disc4-only, no new compute): fix OFF 9/15 pass [0.357,0.802]; ALL 4 EMA reset-init knobs ON 13/15 pass [0.621,0.963], net flips +4/-0 -- **RAISES**, inverting the earlier full-gate (disc4 AND k) LOWERS finding (`w3_reliability_ema_fix_20260926.md`) because that finding's own root-cause was the `k` conjunct collapsing as a readout artifact, not a disc4 regression. `gate_reread_user_decisions_20260926.md` (bt0926-decprobe). Now runnable: ASP gate (c), N3 proper; N2 after W6a | 2026-09-26T11:38Z |
| N2 | E2 world | does retained replay keep the L2R bar while the encoder trains through the read path (Q4c)? | - | /queue-experiment (pinned) | puzzle (known rules) | **queued V3-EXQ-1108** (A1: post 1200) | - | Mac probe pinned to `9b322d5`, seeds 611-615, pre-registered post dose cut to 600 (W3: 1200) for cost: the frozen control missed (a) on 4/5 (post 0.377-0.517, the W3 post-phase gain needs the full dose), so rule 1 fired. Seed 611 only (descriptive): `reencode` 0.520 / k 10 in the moved space (encoder norm x8.9, PR 1.3 -> 7.5; (e) late growth 1.236 > 1.2), `stored` 0.257 / k 0 (stored z 0.93 stale). Next: **queued V3-EXQ-1108** (ree-v3 `45a41d4e0c`): same design at post 1200 on the fleet, fresh seeds 811-815, `ree_core` pinned `9b322d5` via substrate_pin, rule unchanged (amendment A1, record sec 4, `fc5c0795221`); est. 5-7 h (cpx22) / 2-3 h (cx43); await its verdict. D0: harm_eval / codec / E1 store z -> need re-encode. Record `n2_replay_encoder_probe_20260925.md` | 2026-09-26T02:07Z |
...
| W2b | babbling | ACh-gated unfreeze: MECH-398 gain g in [0,1] on member learning rates; retained memory re-opened only when surprise AND g coincide (MECH-207) | BRANCH | /implement-substrate after probe N5 | complex (probe-gated) -> puzzle | **blocked (N5 CANNOT_DETERMINE; not buildable as specified)** | a shift detector with measured separation; a revision rule that re-learns | N5 (`n5_ach_gated_unfreeze_probe_20260926.md`): the surprise the plan names (W3 one-step error, raw or action-contrastive) does not separate an action-map shift from no shift (clamped pair, 5/5 seeds) and fires spuriously; even an oracle unfreeze with FIFO retained replacement re-learns only 0.25-0.39 (< 0.47) in 1200 steps. Next: probe N5b -- (1) pick g's surprise by the clamped-pair separation test first (candidate, untested: short periodic W2a babbling probes scored action-contrastively against the retained baseline, since on-policy data is wall-concentrated), (2) revision = quarantine the whole pre-onset retained set + a full re-babble epoch (N5 addendum `shift_oraclefull`), (3) seeds whose pre-shift state meets the W3 bar | 2026-09-26T01:55Z |
...
| N5 | puzzle (known rules) | does ACh-gated unfreeze revise retained memory only when it should? | 2x2 on the W3 member: {env action-map re-permutation (the SD-MEL-PRODUCER shift) vs none} x {g gated vs g clamped low}. **Pass** means: after the shift, gated re-learns to the bar and clamped does not; with no shift, gated keeps the bar (no spurious destabilisation). Pre-register N updates. **Ran 2026-09-26 (`bt0926-n5`, pre-reg `815a8164bf`, results `141a58d803`): CANNOT_DETERMINE** (pre-shift W3 bar 2/5); shift half 0/5 on every seed (gate blind to the shift; oracle FIFO revision 0.25-0.39 in N = 1200); no-shift half kept the map where it existed (2/2) despite 1-3 spurious unfreezes per seed. |
...
- 2026-09-26T09:33Z: USER DEFERRED TO THE ORCHESTRATOR'S RECOMMENDATIONS (live, AskUserQuestion; rec-20260926-daec471d / -40490f2c / -eab8b996 / -b3c04a6c). The user then stated that they did not understand the implications well enough to make an informed choice. So these are PROVISIONAL orchestrator decisions under standing delegation, not informed user sign-off. Revisit all four before A1 is queued; each is reversible because it changes gate readouts or a preset, not landed substrate defaults:
 (1) ASP gate (c) growth is judged RELATIVE TO ENV TRUTH: predicted per-step growth <= 1.05 x the env's true growth at the same state, plus the late-window mean. Why: the literal bound fails every pool including random controls (fc8340fb87); 84% of that is the EMA zero-init artefact (512967ff4b); the rest is a small-norm ratio floor the true dynamics share (d5f3bdc2fd).
 (2) W4 counts as gate-passed on gate (a) (DISC_0.5 4/5, N3 9608f3117a). Choice quality moves to the integrated post-W5 gate alongside (b) (option ii). Subject to the running n3post reset-confound check on gate (a).
