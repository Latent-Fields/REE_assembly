...
| A1 | acceptance | integrated closed-loop acceptance run (section 5), queued from main, `ree_core` pinned to the branch sha | queue (main) | /queue-experiment (user fixes the pre-registration) | complex (probe-gated): the run IS the probe | blocked | W6, N0, I1 | pre-registration DRAFT for the user to fix: `coupled_a1_preregistration_draft_20260925.md` (+ `probes/a1_draft/` skeleton; red-team CONTESTED, fixes folded in, RT-5 open). It adds INT-CODEC/INT-ASP head-to-head arms and needs an ASP build row (none exists) | 2026-09-25 12:58Z |
