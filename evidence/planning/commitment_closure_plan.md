...
closure_plan:
  ...
  nodes:
    ...
    - id: "commitment_closure:GAP-4"
      ...
      resume_condition: "AWAITING /implement-substrate AMEND of commitment-closure-control-plane (de-commit-authority deliverable). The beta-engagement amend LANDED (use_closure_commit_beta_coupling, ree-v3 main f4ceea4) and 460f RAN 2026-06-17 terminal (confirmed failure_autopsy_V3-EXQ-460f_2026-06-18, user-confirmed gate): all four readiness gates cleared so the C2 de-commit occupancy-drop DV ran for the first time -- PASS seed 42 (ON 23.73 < OFF 35.67, -33.5%), FAIL 43/44. Residual gap = de-commit-authority MAGNITUDE + DV POWER: the Leg-B 5-tick refractory (~20-35 tick-blocks) is swamped by ~530-560 natural-commit elevated steps, the coupling fired only on the weak-commit seed (sd034_n_closure_coupled_elevations 36/52 seed 42 vs 0/0 seeds 43/44), and the between-arm unpaired occupancy DV is underpowered. NOT a falsification (seed 42 + 460e seed 44 are existence proofs of the correct sign; SD-034 + MECH-261 -> non_contributory + pending_retest, NOT weakened; the MECH-261 weakens was mis-attributed -- closures all hook-driven, n_automatic_fires=0, mode-conditioning bypassed). NEXT ACTION: /implement-substrate amend commitment-closure-control-plane with the de-commit-authority deliverable -- (a) MAGNITUDE lever: scale the Leg-B hold with committed-run length (refractory proportional to committed_steps-since-entry) OR convert the de-commit into an active maintenance-release-pressure event (MECH-342 CommitMaintenanceRelease-style) that drives the latch DOWN rather than blocking re-entry for 5 ticks; and (b) a within-arm around-closure C2 DV (pre-vs-post-closure occupancy delta on the ON arm) + tighten the non-vacuity gate to require sd034_n_closure_coupled_elevations > 0 on scored seeds. UPDATE 2026-06-18b: 468e RAN + AUTOPSIED (perseveration side; confirmed failure_autopsy_V3-EXQ-468e_2026-06-18) -- it is no longer an OWED successor; it reproduced the 460f de-commit-authority gap through an INDEPENDENT DV (C1 beta_release_near_contradiction PASS 3/3 ON>OFF, but C2 committed_frac_post_absolute pinned at the 1.0 ceiling 3/3 -- release fires with correct sign but sub-threshold authority, and the absolute DV re-pinned at ceiling). Two independent DVs now confirm ONE authority-magnitude gap. After the amend lands + readiness validates: re-issue 460g (de-commit retest on the sensitized within-arm occupancy DV) AND 468f (perseveration retest on a GRADED within-arm post-contradiction de-commit DV replacing the 1.0-saturated committed_frac_post_absolute) on the amended substrate. Do NOT re-author 460d/468d/460e/468e (superseded). The /implement-substrate amend now carries BOTH the de-commit-authority-magnitude lever (shared) AND a graded post-contradiction DV (468e-specific). MECH-090 (active) and MECH-268 (supports, C1 3/3) join SD-034 + MECH-260 + MECH-261 in the closure cluster's pending_retest set. On the post-amend 460g manifest /governance walk: (PASS, readiness met + within-arm post-closure occupancy drop AND ON<OFF >=2/3 seeds with sd034_n_closure_coupled_elevations>0 on scored seeds) -> apply supports to SD-034 (de-commit authority) + MECH-261 + MECH-260, clear SD-034 pending_retest_after_substrate, close GAP-4 (subject to 468e); (readiness met but C2 unmet) -> GENUINE weakens, route /failure-autopsy AND treat as the granularity-debt TIP-POINT (a structurally-different signature here routes the SD-034 closure cluster to /claim-synthesis -- flagged by failure_autopsy_V3-EXQ-460f_2026-06-18); (readiness gate unmet) -> substrate_not_ready_requeue, amend again, NEVER a false weakens. --- HISTORICAL (superseded by the 2026-06-12 *c rewire + the 2026-06-13 *d validation + this 2026-06-16 Leg C build; retained for the substrate-soundness audit trail): the 2026-06-12 *c-cohort RUNNER-EXECUTION resume + the MECH-090 R-c commit-entry readiness conjunction substrate (LANDED two passes 2026-05-28 + 2026-05-29; V3-EXQ-592d measurement-defect diagnosis 2026-06-01, R-c substrate sound). Cross-link IGW-20260531-021."
    ...
    - id: "commitment_closure:GAP-8"
      ...
      resume_condition: "Trained-OFC-head SUBSTRATE landed 2026-06-09 (ree-v3 382db2c): OFCConfig.train_state_bias_head / REEConfig.ofc_train_state_bias_head (default False -> last Linear zeroed, bit-identical OFF) + OFCAnalog.bias_head_parameters(); the SD-033b analogue of SD-033a GAP-D (gradient path E3 loss -> score_bias -> compute_bias() -> head weights). claims.yaml SD-033b evidence_quality_note updated (c8546ae0ff); sd_033b doc + ree-v3/CLAUDE.md GAP-8 entry landed. The deferred trained-OFC-head arm is queued as V3-EXQ-485d (ree-v3 8839724; diagnostic, claim_ids=[], frozen-vs-trainable, C2 routed on state_bias_head weight-delta-from-init, dry-run smoke PASS head_delta 0.027-0.040). GAP-8 stays in-progress: on 485d PASS the trained head produces measurable behaviour change (the GAP-1-analogue silence is closed at substrate-readiness level); the FULL SD-033b candidate->provisional promotion then needs the evidence-grade behavioural devaluation/discrimination run on the trained head (the governance-weighting follow-on). RESUME: review 485d when the runner lands it; on PASS author the evidence-grade trained-head behavioural arm; on substrate_not_ready_requeue re-queue at a larger P1 budget."
