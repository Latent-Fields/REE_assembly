...
- ✅ **(2026-08-12) `write_flat_manifest` now HARD-ENFORCES a subset of the always-core, not just soft-WARNs.** `experiments/_lib/manifest_core.MANDATORY_CORE_KEYS` (`recording_schema`, `substrate_hash`, `substrate_commit`, `machine`, `machine_class` — the fields `stamp_recording_core` computes with zero caller effort) is checked immediately after the manifest is written to disk; if still missing, `write_flat_manifest` raises `ValueError` (escape hatch `REE_ALLOW_INCOMPLETE_PROVENANCE=1`, matching the repo's other explicit-override env vars). Checked AFTER the write so a run's already-spent compute is never lost — a gap here fails the *call*, never the *capture*. Motivated by the audit finding this doc opens with: 0% of flat manifests recorded a substrate hash, and the soft-WARN alone (landed above) left that gap invisible unless someone ran the linter by hand. `elapsed_seconds`/`config`/`seeds` are deliberately **not yet** in the hard-enforced subset: an AST census of `experiments/v3_exq_*.py` found `config=`/`seeds=` present at 98.4% of `write_flat_manifest` call sites but `elapsed_seconds=`/`started_at=` present at only 37.2% (47.7% of call sites truly missing it) — hard-enforcing those three now would break roughly half of the active experiment fleet's next run. Investigation + implementation: `chip-20260812-recording-standard-mandatory-provenance`, following on from `REE_assembly/evidence/planning/architecture_epoch_investigation.md` (2026-08-12) sections 2/10, which found this gap.
