...
| Human-AI coupled cognitive-prosthesis loop; sycophancy weakening the error signal; bridge to "Machine Folie a Deux" | **INV-077** (evaluation channels are evidence-producing boundaries: no agentic subsystem -- including an AI assistant developing REE -- may update architectural confidence directly from a performance score without provenance/review/governance mediation) is the closest existing analog -- same STRUCTURE (self-generated signal must not be mistaken for independent confirmation) -- but INV-077 is explicitly scoped to REE's OWN DEVELOPMENT PROCESS (claims.yaml/governance write paths), not to REE's ordinary conversational use as a cognitive prosthesis for a human user, and not to the two-participant (neither party alone confused, but the coupled output looks confirmed to both) failure mode the thought describes. No existing claim or doc under "Machine Folie a Deux" was found in this pass (searched claims.yaml and `REE_assembly/**/*.md` case-insensitively). | **Genuinely new, separable question -> registered as Q-096** (open_question, `substrate_conditional`, cross-referencing INV-077/INV-012/EXT-003 rather than duplicating any of them). |
...
1. **Machine Folie a Deux**: no REE doc or claim was found under that name in this pass (search
   was `grep -rn -il "folie"` across `REE_assembly/**/*.md` and claims.yaml, case-insensitive,
   excluding worktrees). If this refers to external/unpublished work the user has in mind rather
   than an existing REE artifact, it should be cited explicitly (or the reference removed/
   softened) before Q-096 is treated as more than an internally-motivated question.
