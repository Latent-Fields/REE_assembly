...
- **STATUS: INTERIM** (first written 2026-09-24T18:15Z, updated 18:45Z, under an account usage-window cutoff; updated in place as cells land -- see "Done / not done" at the end)
...
2. **Train the head and the blow-up disappears with dose (t30 norm 326 -> 3.6 -> 1.0 -> 0.52 at 0/600/3000/10000 steps), but the prediction does not become informative in that range.** Fidelity **k = 0 in all 8 cells**: two recipes, 0-10000 head steps (20-100 episodes in C's recipe), clamp on or off. The open-loop rollout is 3.9-800x worse than predicting "nothing changes" one step out (the ratio is still falling at 10000 steps, so slow undertraining of fidelity is not excluded), and its predicted displacement is orthogonal (regime B) or anti-aligned (C's recipe, cos -0.17 to -0.28) to the actual one. One positive: in C_100 the executed first action is ranked correctly at h = 1 (95% vs 25% chance); by h = 3 that is gone.
...
4. **C's state-invariant proposal reproduces at world_dim 32 in both recipes and at every dose, and is NOT caused by the rollout.** Majority first-action class identical at 30/30 states; restricting CEM scoring to 1-10 steps changes nothing; the terrain_prior's initial mean is already one class at all 30 states in every cell, before any rollout is scored. "Not caused by the rollout" is tested for the rollout's DEPTH (windows 1-10); whether the step-1 score contributes was not isolated.
...
Readings:
1. **The norm blow-up is an untraining artefact, and dose-responsive once the head is trained.** B regime t30 norm 326 -> 3.6 -> 1.0 -> 0.52 over 0 / 600 / 3000 / 10000 world-head steps (late growth 1.209 -> 1.004); C regime 10.8 -> 3.2 over 20 -> 100 episodes. The clamp removes it by construction (pins the norm at 2x the start).
2. **The rollout never becomes informative in the doses measured: k = 0 in every cell, trained or not, clamped or not.** Error relative to persistence does fall with dose (h=1: 571 -> 16 -> 5.0 -> 3.9; h=10: 535 -> 26 -> 5.8 -> 2.1 at 0 / 600 / 3000 / 10000 steps), so fidelity is still improving slowly at 10000 steps -- **undertraining of fidelity is not excluded** -- but the best cell (10000 steps, 3-5x the world-head budget of the drivers that train it at all, e.g. 1061's ~2500 steps at 100 episodes) is still 2-4x worse than "nothing changes" at every horizon, with 13% of starts beating persistence and cos 0.04-0.07. Direction is uninformative (cos 0.01-0.15) in regime B and **anti-aligned** in C's regime (cos -0.17 to -0.28 at h=1-3: the model predicts movement the wrong way). C's recipe does not improve from 20 to 100 episodes at h=1 (abs error 0.051 both).
...
| E2 multi-step prediction is informative (implicit premise of every z_world scorer) | **Uninformative at every dose measured; the trend toward persistence is slow and not yet crossed -- UNDETERMINED between slow undertraining and structural, leaning structural for the production budgets drivers actually use** | k = 0 in all 8 cells (0 to 10000 head steps; two recipes; clamp on/off), error/persistence still falling at 10000 steps: the rollout never beats persistence at any horizon, direction is uninformative or anti-aligned. C's recipe does not improve 20 -> 100 episodes. (10000-step dose and random-policy fidelity pending -- see Done/not done.) |
