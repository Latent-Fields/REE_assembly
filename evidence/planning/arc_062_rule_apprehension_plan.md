...
closure_plan:
  ...
  nodes:
    ...
    - id: "arc_062_rule_apprehension:GAP-B"
      ...
      status: in-progress
      severity: load-bearing
      owner_exq: "V3-EXQ-654c QUEUED 2026-06-15 (ree-v3 origin/main 7225065; coordinator /queue/active present; priority 300, machine any, est 620min, 3 seeds x 2 arms x 350 ep; supersedes V3-EXQ-654b; claim_ids=[MECH-309, ARC-062], experiment_purpose=evidence) -- the 666c-gate-cleared GAP-B behavioural falsifier on the NOW-VALIDATED crf-availability-maintenance substrate (V3-EXQ-666c PASS 2026-06-15; substrate_queue ready=True). Single-variable ARM_OFF vs ARM_ON; ARM_ON arms the 666c ARM_2 levers crf_mature_pool_dynamics + crf_context_from_e2_world_forward + crf_availability_maintenance(floor 0.45/decay 0.0) on crf_persist, ENLARGED P0 150->200 ep, frozen-encoder P1 trained-bias-head REINFORCE (GAP-D); committed-class entropy PRIMARY DV. Pre-registered non-vacuity preconditions self-route substrate_not_ready_requeue (never a false weakens): C1c crf_frac_active>=0.30 + C1d propagation (ARM_ON committed bias != ARM_OFF). CONVERSION-CEILING OFF-RAMP (the 654b weakens branch REPLACED): C1-holds-C2-fails self-routes non_contributory + /implement-substrate (shared selection-authority GAP-A 569g/682 gain/contrast amend), NOT a falsification; ONLY the PASS branch weights MECH-309/ARC-062 (as supports). Awaiting run + review. PREDECESSOR V3-EXQ-654b TERMINAL FAIL 2026-06-10T20:05Z (non_contributory, substrate_not_ready_requeue; reviewed /governance 2026-06-10) -- the longer-maturation (P0+P1 100->240 ep) re-run of 654a still did NOT clear the C1c crf_frac_active>=0.30 floor (measured 0.130), so the committed-class falsifier DV never scored; supersedes V3-EXQ-654a. PREDECESSOR V3-EXQ-654a QUEUED 2026-06-09 (priority 250, machine any; supersedes V3-EXQ-654) -- the gated re-run on the landed cross-episode rule-persistence amend (ree-v3 main 9797e84). Single-variable ARM_OFF vs ARM_ON with crf_persist_rules_across_episode_reset=True (matured pool clears the C1c 0.30 floor), a frozen-encoder P1 trained-bias-head REINFORCE phase (GAP-D), and a propagation non-vacuity precondition (ARM_ON bias != ARM_OFF, else substrate_not_ready_requeue); committed-class entropy PRIMARY DV. PREDECESSOR V3-EXQ-654 TERMINAL FAIL 2026-06-09T08:18Z (non_contributory, confirmed failure_autopsy_V3-EXQ-654_2026-06-09): C1c readiness FAIL (CandidateRuleField cold-started per episode) gated out the C2 falsifier DV -- NOT a falsification."
