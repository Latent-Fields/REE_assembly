...
closure_plan:
  ...
  nodes:
    ...
    - id: "arc_062_rule_apprehension:GAP-B"
      ...
      resume_condition: "BOTH routed steps are now DONE (see governance_2026_06_09_amend_and_requeue): the crf_persist amend LANDED (ree-v3 9797e84) and V3-EXQ-654a is QUEUED (priority 250, machine any). RESUME when V3-EXQ-654a RUNS + lands a manifest + is reviewed in /governance. Branch map: (a) C1 readiness met AND C2 committed-class lift (>=2/3 seeds) -> PASS -> MECH-309/ARC-062 supports evidence -> consider GAP-B closure. (b) C1 met AND C2 fails -> weakens MECH-309/ARC-062 on the eval-frozen readout -> /failure-autopsy. (c) C1 fails (pool still not matured below the 0.30 floor, OR propagation vacuous ARM_ON bias == ARM_OFF, OR class axis / GAP-A divergence absent) -> non_contributory / substrate_not_ready_requeue -> re-queue / substrate enrichment; do NOT weaken. Do NOT re-open the GAP-A or rule-creator blockers (both resolved); MECH-309/ARC-062 stay candidate / substrate_ceiling / v3_pending / pending_retest_after_substrate until 654a's run is adjudicated."
