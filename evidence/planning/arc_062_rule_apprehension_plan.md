...
closure_plan:
  ...
  nodes:
    ...
    - id: "arc_062_rule_apprehension:GAP-B"
      ...
      status: in-progress
      severity: load-bearing
      owner_exq: "V3-EXQ-654 TERMINAL FAIL 2026-06-09T08:18Z (non_contributory, confirmed failure_autopsy_V3-EXQ-654_2026-06-09): C1c readiness FAIL (CandidateRuleField cold-started per episode) gated out the C2 falsifier DV -- NOT a falsification. NEXT: /implement-substrate amend ree_core/policy/candidate_rule_field.py cross-episode rule persistence (impl target ARC-063), then re-queue 654a with a trained-bias-head P1 arm + propagation non-vacuity precondition."
      ...
      resume_condition: "V3-EXQ-654 RAN + FAILed non_contributory on the pre-registered C1-fail branch (substrate_not_ready_requeue); confirmed failure_autopsy_V3-EXQ-654_2026-06-09 applied 2026-06-09 (see governance_2026_06_09_pm). The C1c readiness FAIL is a CandidateRuleField per-episode cold-start (agent.reset()->candidate_rule_field.reset() at agent.py:1908), NOT a falsification of MECH-309/ARC-062. NEXT (two steps, ordered): (1) /implement-substrate amend ree_core/policy/candidate_rule_field.py with a no-op-default cross-episode rule-persistence flag so reset() does not clear _rules/_recurrence (impl target ARC-063; bit-identical OFF; substrate_queue ARC-062 entry already carries the 654 failure_record + implementation_hint). (2) After the amend lands, /queue-experiment 654a (gated on the amend) = the same single-variable ARM_OFF/ARM_ON falsifier with a TRAINED-bias-head P1 arm (GAP-D rule_bias_head trainability landed 2026-05-17) + a propagation non-vacuity precondition (ARM_ON lateral_pfc bias must differ from ARM_OFF on firing ticks) so a future C1-pass is not vacuous; committed-class entropy stays PRIMARY DV, within-class-representative entropy the secondary negative control. Do NOT weaken MECH-309/ARC-062; do not re-open the GAP-A or rule-creator blockers (both resolved)."
