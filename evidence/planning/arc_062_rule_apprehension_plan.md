...
closure_plan:
  ...
  nodes:
    ...
    - id: "arc_062_rule_apprehension:GAP-D"
      ...
      cross_plan_link: ["commitment_closure:GAP-1"]
      last_updated: 2026-05-18
      substrate_note: "Substrate implemented 2026-05-17 (pre-GAP-B-PASS pre-positioning). LateralPFCConfig.train_rule_bias_head (bool, default False) added; when True last Linear is NOT zeroed at init. LateralPFCAnalog.bias_head_parameters() method added for optimizer inclusion. REEConfig gains lateral_pfc_train_rule_bias_head (bool, default False). Experiment use: optim.Adam(list(agent.lateral_pfc.bias_head_parameters()), lr=LR) in P1 optimizer. Gradient path: E3 loss -> score_bias -> compute_bias() -> rule_bias_head weights. Default False preserves existing zeroed-last-Linear behavior. Validation EXQ deferred: 2026-05-18 governance superseded the 543f/543g/543h falsifier cluster (substrate_ceiling; all 7 manifests -> V3-EXQ-543i). Sequenced after GAP-C; gated on the V3-EXQ-543i /failure-autopsy + a contributory post-substrate (differential-heads) GAP-B retest -- NOT 543f (superseded)."
