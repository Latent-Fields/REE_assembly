...
closure_plan:
  ...
  nodes:
    ...
    - id: "arc_062_rule_apprehension:GAP-B"
      ...
      resume_condition: "GATED ON V3-EXQ-666 (CRF-readiness diagnostic) PASS. The 654b autopsy-routed substrate enrichment LANDED 2026-06-11 (ARC-063 CandidateRuleField mature-pool amend, ree-v3 7e2e0ef; see governance_2026_06_11) -- the two opt-in flags crf_mature_pool_dynamics + crf_context_from_e2_world_forward recalibrate the conflict-gate so a differentiated >=2-rule pool can form (the C1c crf_frac_active>=0.30 floor the 654/654a/654b lineage never cleared). RESUME path: (1) V3-EXQ-666 (queued, claim-free) must PASS the CRF-readiness gate (crf_frac_active>=0.30 AND crf_max_pairwise_rule_dist>floor on the mature arms >=2/3 seeds) -> confirms the pool now matures; (2) ONLY THEN /queue-experiment the 654c GAP-B BEHAVIOURAL falsifier (committed-class entropy PRIMARY DV) with the mature flags armed on the matched GAP-A/authority/SD-056 stack. If V3-EXQ-666 FAILs (pool still not maturing under the new flags) -> /failure-autopsy on the readiness diagnostic, NOT a 654c re-queue. Branch map for 654c once it runs: (a) C1 met AND C2 committed-class lift >=2/3 -> PASS -> MECH-309/ARC-062 supports. (b) C1 met AND C2 fails -> weakens -> /failure-autopsy. (c) C1 fails -> non_contributory / substrate_not_ready_requeue, do NOT weaken. MECH-309/ARC-062 stay candidate / substrate_ceiling / v3_pending / pending_retest_after_substrate until 654c is adjudicated. Branch map: (a) C1 readiness met AND C2 committed-class lift (>=2/3 seeds) -> PASS -> MECH-309/ARC-062 supports evidence -> consider GAP-B closure. (b) C1 met AND C2 fails -> weakens MECH-309/ARC-062 on the eval-frozen readout -> /failure-autopsy. (c) C1 fails (pool still not matured below the 0.30 floor, OR propagation vacuous ARM_ON bias == ARM_OFF, OR class axis / GAP-A divergence absent) -> non_contributory / substrate_not_ready_requeue -> re-queue / substrate enrichment; do NOT weaken. Do NOT re-open the GAP-A or rule-creator blockers (both resolved); MECH-309/ARC-062 stay candidate / substrate_ceiling / v3_pending / pending_retest_after_substrate until 654a's run is adjudicated."
