...
closure_plan:
  ...
  nodes:
    ...
    - id: "arc_062_rule_apprehension:GAP-B"
      ...
      resume_condition: "GATED ON V3-EXQ-666c (clean fraction-gated CRF-readiness diagnostic) PASS. SUPERSEDES the stale 'GATED ON V3-EXQ-666' gate -- 666/666a/666b all ran FAIL/non_contributory (see governance_2026_06_14). The crf-availability-maintenance mechanism is CONFIRMED FUNCTIONAL (666a clean monotone crf_frac_maintained separation ARM_2 1.0/0.625/0.938 vs ARM_1 0.188/0.125/0.438 vs ARM_0 0.0), and the 666b fraction-gate logic discriminated correctly (both load-bearing criteria passed), but no single run cleared the fraction gate AND its non-vacuity preconditions simultaneously: 666b self-routed substrate_not_ready_requeue because the e2ctx_full_pool_differentiated_supra_floor precondition tripped (min over ARM_1+ARM_2 post-gap = 0.0; ARM_1 erodes to empty under the silence gap by design). RESUME path: (1) /queue-experiment V3-EXQ-666c (claim-free, diagnostic) -- the clean 666b fraction-gated re-run at an ENLARGED P0 maturation window, capturing full-pool differentiation PRE-gap, so the e2ctx arms differentiate supra-floor (crf_max_pairwise_rule_dist > 0.1) on all 3 seeds. PASS criteria: load-bearing ARM_2_MAINTENANCE clears the crf_frac_maintained gate AND ARM_1_NO_MAINTENANCE below the ceiling (maintenance contribution isolable); non-vacuity ALL met -- every (arm x seed) cell mints >=2 rules; e2ctx full-pool differentiation > 0.1; ARM_2 n_maintained_reactivatable >= 2 AND crf_maintained_pairwise_dist > 0.1. Self-route substrate_not_ready_requeue (NOT a weakens) if any precondition is unmet. (2) ONLY on 666c PASS /queue-experiment the 654c GAP-B BEHAVIOURAL falsifier (committed-class entropy PRIMARY DV) with the mature + e2ctx + maintenance flags armed on the matched GAP-A/authority/SD-056 stack. If V3-EXQ-666c FAILs on a load-bearing criterion (maintenance insufficient, OR differentiation-alone clears the ceiling) -> /failure-autopsy on the readiness diagnostic, NOT a 654c re-queue; if it self-routes substrate_not_ready_requeue on an unmet non-vacuity precondition -> re-queue the readiness diagnostic at a still-larger P0. Branch map for 654c once it runs: (a) C1 readiness met AND C2 committed-class lift (>=2/3 seeds) -> PASS -> MECH-309/ARC-062 supports evidence -> consider GAP-B closure. (b) C1 met AND C2 fails -> weakens MECH-309/ARC-062 on the eval-frozen readout -> /failure-autopsy. (c) C1 fails (pool not maintained, OR propagation vacuous ARM_ON bias == ARM_OFF, OR class axis / GAP-A divergence absent) -> non_contributory / substrate_not_ready_requeue -> re-queue / substrate enrichment; do NOT weaken. Do NOT re-open the GAP-A or rule-creator blockers (both resolved); MECH-309/ARC-062/ARC-063 stay candidate / substrate_ceiling / v3_pending / pending_retest_after_substrate until 666c then 654c are adjudicated."
