...
closure_plan:
  ...
  nodes:
    ...
    - id: "global_workspace_jlens:GATE-B"
      ...
      blocking_external: ["competence-localization: V3-EXQ-724 RAN 2026-07-09 terminal FAIL / non_contributory (diagnostic, no claim tags) and did not by itself localize; V3-EXQ-732 then localized the root to H2_observation_interface_unlearnable -> the observation-encoding competence build + a competent all-ON substrate -- see reconcile_2026_09_08 and cross_plan_link"]
      cross_plan_link: ["behavioral_diversity_isolation:GAP-I", "conversion_ceiling_campaign:CAMPAIGN"]
      decision_live: "DECISION SURFACED 2026-07-09 but GATED (user-agreed sequencing): Experiment A (723) returned compact_action_coupled_subspace_present -- the plan's pre-registered trigger for the retrofit build -- BUT 723 read the SAME all-ON substrate that V3-EXQ-719a (2026-07-08, failure_autopsy_V3-EXQ-719a_2026-07-08) showed is competence-limited (forages 0.065/0.0/0.455 resources/ep, below the 1.0 floor; debiased state->commitment MI clears floor on only 1/3 seeds). That confound weakens A: the ~2-dim / 0.0003-activity 'compactness' may partly reflect a near-monostrategy policy with little action-predictive structure to capture, not a genuine broadcast bottleneck. USER-AGREED (2026-07-09): LOCALIZE COMPETENCE FIRST (V3-EXQ-724 competence-localization diagnostic, queued 2026-07-09 brake-exempt; RAN 2026-07-09T21:14Z terminal FAIL / non_contributory -- see reconcile_2026_09_08) before spending the SD-027 gate build. A J-space read on a COMPETENT agent is a much cleaner Experiment-A baseline, and the same competence gap is what Experiment B's integration-DV needs to be non-vacuous. So GATE-B does NOT proceed on the A-positive alone; it resumes once competence is localized + a competent all-ON substrate exists. The build when it happens: /implement-substrate V3 top-k selection gate over the E1/E2 -> E3 forwarding path (use_boundary_access_gate flag, byte-identical OFF), top-k over active z_world/z_self components per heartbeat, WITH a hard ablation mode (k -> all / gate disabled) for the MECH-254 four-cell factorial. SD-027 is otherwise v4-scoped; this is the optional fixed-k V3 retrofit the SD-027 evidence_quality_note names but never queued. Status open (not assembling -- not under construction; correctly gated behind competence-localization)."
      last_updated: 2026-09-08
      reconcile_2026_09_08: "PROSE RECONCILE (docs-only; session admiring-heisenberg-6c7b32, chip-20260902-plan-prose-460k-724-ran-not-queued; NO status change, NO claims.yaml / queue action). blocking_external and decision_live still described V3-EXQ-724 as '(queued)' / 'already queued'. V3-EXQ-724 RAN 2026-07-09T21:14Z and is terminal FAIL / non_contributory (manifest evidence/experiments/v3_exq_724_competence_localization_diagnostic_20260709T211405Z_v3.json; experiment_purpose diagnostic, no claim tags). Verified 2026-09-02 via /morning-digest Step 7c: not in the live queue, manifest present, positive provenance -- neither owed nor phantom. A non_contributory FAIL does not advance this node, so the blocker is still real; what GATE-B now waits on is what node A already records: V3-EXQ-732 localized the competence root to H2_observation_interface_unlearnable (the 4th diagnostic, 719a->724->732->723a, to bottom out on the same V3 competence floor), so the gate resumes only once the observation-encoding competence build lands and a competent all-ON substrate exists (forage >= floor on a majority of seeds) -- owned by the competence cluster / f_dominance_conversion_ceiling / ree_ai_design_critique_plan, see cross_plan_link. No successor EXQ for the SD-027 gate build is named. Status stays open."
    ...
    - id: "global_workspace_jlens:MECH-191"
      ...
      depends_on: ["global_workspace_jlens:A"]
      last_updated: 2026-07-09
      last_updated_provenance: "BACKFILLED 2026-08-18 (session cranky-driscoll-126a36; frontmatter-only, PROMOTES/DEMOTES NOTHING, no claims.yaml touch, no queue edit, status unchanged). This node was the one `last_updated`-less node in the plan: it was created by the 2026-07-09 closure_plan retrofit (commit a2b8e1f20a) which set `last_updated` on its three siblings (A 2026-07-10, GATE-B 2026-07-10, B 2026-07-09) and omitted it here. Date derived mechanically, not estimated: git history of this file shows the node's last substantive (non-`live:`) content change is the retrofit commit itself, so its creation date IS its last-updated date -- same value the retrofit gave sibling B. Backfilled because this node's status is `open`, i.e. it is inside the staleness-scored population; the seven `assembling` nodes in conversion_ceiling_campaign_plan.md were deliberately LEFT without `last_updated` in the same pass, because assembly-frontier nodes are exempt from staleness by design (check_closure_drift.py ASSEMBLING_STATUSES; assembly_vs_closure_plan.md) and their only resume trigger is the optional `revisit_after`."
