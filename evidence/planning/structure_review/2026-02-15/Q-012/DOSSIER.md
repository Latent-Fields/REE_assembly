# Structure Review Dossier: Q-012
...
Q-012 is an open question about latent predictive models / control completion falsifiability.

## Where This Fits in REE as a Whole

This sits in REE's uncertainty layer and defines what remains unresolved before stronger commitments. It depends on 4 upstream claim(s): `MECH-057`, `ARC-015`, `ARC-005`, `ARC-004`. No downstream claims currently list it as a dependency. Primary architecture anchor: `docs/architecture/agency_responsibility_flow.md#q-012`.
...
- Direction counts: supports=6, weakens=4, mixed=1, unknown=0.
...
|---|---:|---|---|
| latent predictive models / control completion falsifiability is broadly correct, but failures are boundary-condition failures. | 0.554 | Supporting evidence dominates enough to keep the core mechanism plausible. | Target stress regimes where failures cluster and test whether boundary constraints recover stability.; Pull replication papers that test similar mechanisms under distribution shift. |
| latent predictive models / control completion falsifiability is over-scoped and should be narrowed or split. | 0.372 | Weakening evidence indicates at least one sub-regime where the current claim phrasing breaks. | Extract disconfirming sources and isolate the exact violated assumption.; Run claim-splitting experiments with explicit sub-claim tags in manifests. |
| A hybrid architecture is needed: keep the core of latent predictive models / control completion falsifiability but add explicit gating or interface separation. | 0.372 | Mixed evidence and conflict patterns suggest partial validity with missing structural guardrails. | Evaluate an ablation that isolates the proposed guardrail/interface addition.; Collect one adjacent-domain source that uses a similar split architecture. |

## Source Wording vs REE Translation

| timestamp | source | direction | confidence | evidence wording (preserved) | REE translation |
|---|---|---|---:|---|---|
| `2026-02-15T21:36:33Z` | Run-pack `2026-02-15T213633Z_claim-probe-q-012_seed1002_trajectory_first_enabled_toyenv_internal_minimal` in `claim_probe_q_012` | `weakens` | 0.75 | FAIL with weakening direction | In REE terms, this is negative pressure on `latent predictive models / control completion falsifiability` and suggests the claim is likely too strong or missing a key gating variable. |
| `2026-02-15T21:36:02Z` | Run-pack `2026-02-15T213602Z_claim-probe-q-012_seed1001_trajectory_first_enabled_toyenv_internal_minimal` in `claim_probe_q_012` | `weakens` | 0.75 | FAIL with weakening direction | In REE terms, this is negative pressure on `latent predictive models / control completion falsifiability` and suggests the claim is likely too strong or missing a key gating variable. |
| `2026-02-15T21:10:32Z` | Run-pack `2026-02-15T211032Z_claim-probe-q-012_seed11_trajectory_first_enabled_toyenv_internal_minimal` in `claim_probe_q_012` | `supports` | 0.75 | PASS with supporting direction | In REE terms, this is positive pressure on `latent predictive models / control completion falsifiability` and supports keeping the mechanism, with tighter boundary conditions where failures recur. |
| `2026-02-15T20:52:12Z` | Run-pack `2026-02-15T205212Z_claim-probe-q-012_seed11_trajectory_first_enabled_toyenv_internal_minimal` in `claim_probe_q_012` | `supports` | 0.75 | PASS with supporting direction | In REE terms, this is positive pressure on `latent predictive models / control completion falsifiability` and supports keeping the mechanism, with tighter boundary conditions where failures recur. |
| `2026-02-15T18:05:24Z` | Run-pack `2026-02-15T180524Z_claim-probe-q-012_seed29_trajectory_first_enabled_toyenv_internal_minimal` in `claim_probe_q_012` | `weakens` | 0.75 | FAIL with weakening direction | In REE terms, this is negative pressure on `latent predictive models / control completion falsifiability` and suggests the claim is likely too strong or missing a key gating variable. |
| `2026-02-15T18:05:23Z` | Run-pack `2026-02-15T180523Z_claim-probe-q-012_seed11_trajectory_first_enabled_toyenv_internal_minimal` in `claim_probe_q_012` | `supports` | 0.75 | PASS with supporting direction | In REE terms, this is positive pressure on `latent predictive models / control completion falsifiability` and supports keeping the mechanism, with tighter boundary conditions where failures recur. |
| `2026-02-15T17:52:23Z` | Run-pack `2026-02-15T175223Z_claim-probe-q-012_seed29_trajectory_first_enabled_toyenv_internal_minimal` in `claim_probe_q_012` | `weakens` | 0.75 | FAIL with weakening direction | In REE terms, this is negative pressure on `latent predictive models / control completion falsifiability` and suggests the claim is likely too strong or missing a key gating variable. |
| `2026-02-15T17:52:23Z` | Run-pack `2026-02-15T175223Z_claim-probe-q-012_seed11_trajectory_first_enabled_toyenv_internal_minimal` in `claim_probe_q_012` | `supports` | 0.75 | PASS with supporting direction | In REE terms, this is positive pressure on `latent predictive models / control completion falsifiability` and supports keeping the mechanism, with tighter boundary conditions where failures recur. |
