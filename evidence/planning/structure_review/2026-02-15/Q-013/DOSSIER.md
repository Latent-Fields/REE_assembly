...
|---|---:|---|---|
| uncertainty / deterministic vs stochastic jepa calibration is broadly correct, but failures are boundary-condition failures. | 0.503 | Supporting evidence dominates enough to keep the core mechanism plausible. | Target stress regimes where failures cluster and test whether boundary constraints recover stability.; Pull replication papers that test similar mechanisms under distribution shift. |
...
| `2026-02-15T18:09:24.657062Z` | Run-pack `exp_0008_20260215T180924657062Z` in `claim_probe_q_013` | `supports` | 0.75 | PASS with supporting direction | In REE terms, this is positive pressure on `uncertainty / deterministic vs stochastic jepa calibration` and supports keeping the mechanism, with tighter boundary conditions where failures recur. |
