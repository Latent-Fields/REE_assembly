...
Generated: `2026-02-14T20:59:13.316649Z`
Cycle: `2026-02-14`
...
- Conflict ratio: 0.923
- Overall confidence: 0.682

## Evidence Mix and Why It Looks This Way

- Direction counts: supports=21, weakens=18, mixed=1, unknown=0.
...
|---|---:|---|---|
| jepa substrate / ema target anchor timescale separation is broadly correct, but failures are boundary-condition failures. | 0.547 | Supporting evidence dominates enough to keep the core mechanism plausible. | Target stress regimes where failures cluster and test whether boundary constraints recover stability.; Pull replication papers that test similar mechanisms under distribution shift. |
| jepa substrate / ema target anchor timescale separation is over-scoped and should be narrowed or split. | 0.427 | Weakening evidence indicates at least one sub-regime where the current claim phrasing breaks. | Extract disconfirming sources and isolate the exact violated assumption.; Run claim-splitting experiments with explicit sub-claim tags in manifests. |
| A hybrid architecture is needed: keep the core of jepa substrate / ema target anchor timescale separation but add explicit gating or interface separation. | 0.427 | Mixed evidence and conflict patterns suggest partial validity with missing structural guardrails. | Evaluate an ablation that isolates the proposed guardrail/interface addition.; Collect one adjacent-domain source that uses a similar split architecture. |
...
| `2026-02-14T18:53:25.228898Z` | Run-pack `bridge_v2_mech_058_anchor_drift_extreme_shift_s58022_20260214t185325228646z` in `jepa_anchor_ablation` | `weakens` | 0.75 | FAIL with weakening direction | In REE terms, this is negative pressure on `jepa substrate / ema target anchor timescale separation` and suggests the claim is likely too strong or missing a key gating variable. |
| `2026-02-14T18:53:25.228121Z` | Run-pack `bridge_v2_mech_058_anchor_drift_extreme_shift_s58021_20260214t185325227820z` in `jepa_anchor_ablation` | `weakens` | 0.75 | FAIL with weakening direction | In REE terms, this is negative pressure on `jepa substrate / ema target anchor timescale separation` and suggests the claim is likely too strong or missing a key gating variable. |
| `2026-02-14T18:53:25.227124Z` | Run-pack `bridge_v2_mech_058_nonstationary_shift_replay_s58022_20260214t185325226899z` in `jepa_anchor_ablation` | `supports` | 0.75 | PASS with supporting direction | In REE terms, this is positive pressure on `jepa substrate / ema target anchor timescale separation` and supports keeping the mechanism, with tighter boundary conditions where failures recur. |
| `2026-02-14T18:53:25.226502Z` | Run-pack `bridge_v2_mech_058_nonstationary_shift_replay_s58021_20260214t185325226295z` in `jepa_anchor_ablation` | `supports` | 0.75 | PASS with supporting direction | In REE terms, this is positive pressure on `jepa substrate / ema target anchor timescale separation` and supports keeping the mechanism, with tighter boundary conditions where failures recur. |
| `2026-02-14T18:48:53.462180Z` | Run-pack `bridge_v2_mech_058_anchor_drift_extreme_shift_s58022_20260214t184853462018z` in `jepa_anchor_ablation` | `weakens` | 0.75 | FAIL with weakening direction | In REE terms, this is negative pressure on `jepa substrate / ema target anchor timescale separation` and suggests the claim is likely too strong or missing a key gating variable. |
| `2026-02-14T18:48:53.461720Z` | Run-pack `bridge_v2_mech_058_anchor_drift_extreme_shift_s58021_20260214t184853461547z` in `jepa_anchor_ablation` | `weakens` | 0.75 | FAIL with weakening direction | In REE terms, this is negative pressure on `jepa substrate / ema target anchor timescale separation` and suggests the claim is likely too strong or missing a key gating variable. |
