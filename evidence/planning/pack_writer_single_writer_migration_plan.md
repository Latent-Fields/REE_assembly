...
closure_plan:
  ...
  nodes:
    ...
    - id: STEP-7.4
      ...
      last_updated: 2026-07-15
      note: >
        DONE (mechanically-retrofittable population exhausted). Cumulative 104
        scripts retrofitted (68 + 35 + 1 across eligible batches) through the
        RUNID_only var-identity batch; every subsequent batch pass (4/6/7) found
        0 eligible and confirmed the remainder are by-design advisory gaps (no
        unaliased timezone import, or a scope-split parse_args anchor). Closed
        2026-07-15: recovering the residual is NOT debt -- it would require a
        scoped extension of retrofit_elapsed_seconds.py to inject at the
        wfm-enclosing-function head, a separate opt-in follow-up only worth
        doing if elapsed_seconds becomes load-bearing (it is not today --
        surfaced for queryability, NOT scored, same as substrate_hash).
    - id: STEP-7.1
      ...
      last_updated: 2026-07-15
      note: >
        PARKED 2026-07-15 (no independent justification; value coupled to STEP-7.2).
        Re-scoped from deferred after a consumer audit: write_pack/ExperimentPackWriter
        is referenced only by run.py (+ its own module and lint/migrator/test tooling);
        the PRODUCTION flat->pack path is the SEPARATE sync_v3_results.build_runpack_docs
        (governance.sh + coordinator). Nothing cross-checks the two skeletons producing
        identical bytes, so the drift this de-risks has low blast radius today. The
        de-drift only becomes load-bearing IF the pack becomes load-bearing -- i.e. if
        STEP-7.2 is adopted. Cost is non-trivial (touches the coordinator writer + the
        golden byte-shape test coordinator/test_phase3_runpack_materialize.py). Do NOT
        pick up as live work unless STEP-7.2 is adopted or write_pack otherwise becomes
        load-bearing.
    - id: STEP-7.2
