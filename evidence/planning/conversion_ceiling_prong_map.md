...
- **DEFERRED wiring:** the substrate_queue `f_dominance_conversion_ceiling` <-> prong-map cross-ref was not written in this pass (an active session held a claim on substrate_queue.json reconciling the 689c Factor-B routing). To be added once that session lands.
