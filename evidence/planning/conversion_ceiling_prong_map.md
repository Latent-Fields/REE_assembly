...
3. **P2 (root C)** — the **live V3 critical path** and now the **SOLE remaining gate** before the full-stack arm (P-comp DONE/additive 2026-06-24; P3 DONE/folded): build the closure-exclusive de-commit eval substrate (dissociates natural-commit from closure-de-commit) -> face-validate -> include. This is the long pole and, post-689c, the most likely true bottleneck (commit-duration, not selection). **P-comp characterizing the selection-face pair as additive unblocks NO full-stack assembly on its own** — it only retires the within-selection-face composition question; the commit-duration face (P2) is still unbuilt.
4. **Full-stack arm** assembled once P-comp + P3 + P2 are composition-ready — i.e. **gated solely on P2** now.
