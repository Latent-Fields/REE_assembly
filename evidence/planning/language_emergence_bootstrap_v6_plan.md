  ...
  nodes:
    ...
    - id: "language_emergence_bootstrap_v6:LANG-2"
      ...
      readiness_gate:
        ...
        - "Fourteenth enabling condition, added 2026-09-10 (GFLAG-0171) and NOT in the 2026-06-05 addendum list: functional states pre-exist as referents (ARC-048). Signals must be ABOUT already-existing internal functional states -- ARC-048 is the registered pre-linguistic bridge to INV-003, and without it a LANG-3 signalling probe is vacuous however well the other thirteen are satisfied"
        - "Each condition maps to a prerequisite tier: object tokens -> object_representation_v4 (OBJ-2 permanence); self-attribution -> self_model_v4; other-attribution -> mirror_modelling_other_self_v5 (MIRROR-1 OTHER_SELFLIKE); joint attention -> ARC-150 (triadic self-other-object co-reference primitive, V4, on ARC-080 slot machinery) -- UPSTREAM of MIRROR-1 as its developmental prerequisite, not its consequence (Sodian + Kristen-Antonow 2015: 12-month declarative pointing precedes and predicts 50-month false belief); coordination pressure + partial observability + partner variation -> multi_agent_ecology_v5 (MAE-1 substrate)"
    ...
    - id: "language_emergence_bootstrap_v6:LANG-4"
      ...
      blocking_on: "Needs the minimal channel (LANG-3) plus joint attention (the ARC-150 triadic co-reference primitive -- the joint-attention node, UPSTREAM of mirror_modelling_other_self_v5:MIRROR-1 as its developmental prerequisite; the games additionally need MIRROR-1's OTHER_SELFLIKE other-model to address) and an information asymmetry (multi_agent_ecology_v5 partial-observability task design). All V5-or-later."
      readiness_gate:
        - "LANG-3 minimal channel in place (the thing whose use the games measure)"
        - "Joint-attention machinery: two agents able to attend to the same object/event (the ARC-150 triadic co-reference binding: self slot + one other slot co-bound to one object token -- upstream of mirror_modelling_other_self_v5:MIRROR-1 OTHER_SELFLIKE detection, its prerequisite rather than its product; joint attention is the immediate precursor of language flagged in emergence_and_bootstrapping.md: WORLD predictions improve by conditioning on what the other attends to, creating simulation overhead that pressures externalisation)"
...
| joint-attention emergence games | LANG-4 | MECH-010 (the emergence driver) | V6 (blocked) | LANG-3 + joint attention (ARC-150, upstream of MIRROR-1) + info asymmetry |
