...
| `status_history_plane:SHP-2` | Collapse closure-plan blobs -> `live:` + `join:` | **done (all plans collapsed; 0 at-risk refs)** 2026-07-11 | Per-plan razor §5 tooling landed. `scripts/shp2_backfill_snapshot.py` lifts each node's `phase`/`owner_exq`/`awaiting` (+ annotation) blobs **verbatim** + the un-derivable governance-apply `master <hash>` provenance into the append-only `evidence/planning/status_history/status_snapshot.v1.jsonl` (idempotent; PROMOTES/DEMOTES NOTHING) BEFORE collapse. `scripts/shp2_collapse_plan.py` then does the two-plane collapse (§4a) via a textual line-based edit (no YAML round-trip) taking `live:` verbatim from the projector's `stored_live_view`; it refuses to collapse an un-lifted node. `project_status_head.py` refactored to a shared `build_projections`/`load_events`/`stored_live_view` so collapse and drift check use ONE projection path. `check_closure_drift.py` extended with the **status-plane drift** section (re-projects every collapsed `live:`, flags projected != stored; warn-only; drift-detection verified against a tamper). `generate_closure_snapshot.py` frontier `awaiting` cell falls back to `live.next` so collapse never blanks it. Applied to `conversion_ceiling_campaign` (6 nodes, all `assembling`): 3 governance-apply commit hashes (65940b83b5/07acd6ad29/f8a55e4525) lifted; diff-vs-blob at-risk refs 71->56; status-plane drift 0/6. **Plans collapsed 2026-07-10 (session vigorous-yalow-bc7f39):** `behavioral_diversity_isolation` (12 nodes, 553c15cbb0), `commitment_closure` (12 nodes, 9f9b3905dc), `arc_062_rule_apprehension` (13 nodes, 1bc1836ffc) -- each verified: frontmatter parses, body byte-identical below frontmatter, full collapse (0 residual blob lines), status-plane drift 0 (43/43 collapsed nodes checked), closure snapshot + IGW clean. **`goal_pipeline` collapsed 2026-07-10 (session mystifying-bose-6bb813):** 7 nodes GAP-1..GAP-7, all lifted (0 at-risk commit bits) then collapsed -- verified frontmatter parses (7 nodes), body byte-identical below frontmatter (sha db354a93afc3d8e0), 0 residual blob lines, status-plane drift 0/50, closure snapshot regen clean (overall 77.5%), diff-vs-blob at-risk refs 19->15 (goal_pipeline's 4 GAP-4/6/7 refs cleared). **`sd033_governance` collapsed 2026-07-10 (session mystifying-bose-6bb813):** 8 nodes (CHK-ANCHOR/CLAIMS/EXP_PROPOSALS/SD034/MECH266/267/268/PUSH), all lifted (0 at-risk commit bits) then collapsed -- verified frontmatter parses (8 nodes), body byte-identical below frontmatter (sha 054aa3521ba65e9a), 0 residual blob lines, status-plane drift 0/58, closure snapshot regen clean (77.5%), diff-vs-blob at-risk refs 15->7 (sd033's 8 CHK-SD034/MECH266/267/268 refs cleared). **Backfill tooling fix (9ffcab17a8):** lift-eligibility re-keyed from "value is a non-empty string" to "field is present" so scalar/null blobs (integer `phase: 3`, `owner_exq: null`) are lifted and no longer make the collapse REFUSE (surfaced on `commitment_closure` GAP-3/6/7/9/10/11). **Final 4 plans collapsed 2026-07-11 (session mystifying-bose-6bb813) -- SHP-2 COMPLETE:** `global_workspace_jlens` (4 nodes A/GATE-B/B/MECH-191, pre-body-sha 005daaa66c7871cb), `infant_substrate` (17 nodes GAP-1..GAP-15, f9636f7de1f4cfa5), `sleep_substrate` (10 nodes GAP-1..GAP-8, 39087937161e4069), `self_model_v4` (9 nodes SELF-1..SELF-9, 0342ab216129f020) -- each verified individually: frontmatter parses, body byte-identical below frontmatter, 0 residual blob lines. Combined: status-plane drift **0/98** (all collapsed nodes across every plan re-project clean), closure snapshot regen clean (77.5%), **diff-vs-blob at-risk history refs 7->0**. Every closure-plan `phase`/`owner_exq`/`awaiting` blob is now lifted into the append-only snapshot log + collapsed to the two-plane `live:` + `join:` schema. **SHP-3 is unblocked.** |
| `status_history_plane:SHP-3` | `status_snapshot/v1` append log + both views | **done** 2026-07-11 | Landed `scripts/promote_status_history.py` (SHP-3 governance promoter; reuses the ONE projection path via `project_status_head`). (1) **Snapshot append** (contract term 4): appends a **change-only** `kind: status_projection` `status_snapshot/v1` record per node -- the derived `stored_live_view` head at the run timestamp -- onto the same append-only `evidence/planning/status_history/status_snapshot.v1.jsonl` the SHP-2 backfill lifts wrote to. Change-only (a node is written only when its projected `live` differs from its last recorded projection; a stable run appends ZERO) deliberately mirrors the state-change-only heartbeat design that retired the git-history-bloating liveness tick; a change-point timeline still answers "what did we believe on DATE". First run seeded 316 records; re-run appended 0 (idempotent). (2) **Committed sidecars**: `project_status_head.py` refactored to a shared `write_history_sidecars(hist_dir, plans, projections, repo_root, ...)` (byte-identical scratch output verified; link prefix now computed via `relpath` so it resolves at either depth) that the promoter calls to regenerate the 42 per-plan `*_history.md` sidecars into the **committed** `evidence/planning/status_history/history/` tree. Wired as `governance.sh` **Step 3c-bis-4** (after the status stubs). (3) **Query API (Q2=both)**: `serve.py` `/api/status_history?node=<id>` / `?claim=<id>` returns BOTH the collapsed-plan `live:` head (status plane, from frontmatter) AND the appended `status_snapshot/v1` history slice + SHP-2 backfill-lift archive (history plane, from the log). Verified end-to-end: `governance.sh` exits 0 (incl. the Step 9c strict gate); `serve.py` starts; node/claim/missing/no-arg queries return correct JSON over HTTP (200, application/json). PROMOTES/DEMOTES NOTHING; edits no `*_plan.md`, no claims.yaml. SHP-4 (port to `claims.yaml`) now the open structural node. |
| `status_history_plane:SHP-4` | Port to `claims.yaml` (`live_status` + `last_reviewed`) | **done** 2026-07-11 | Landed `scripts/apply_live_status.py` (stamper) + `scripts/claims_live_status_drift.py` (drift check) on `master` `f673d342de`. **878 claims stamped** with a derived `live_status` block (`reading`/`as_of`/`needs_review`) computed as a pure fn of `status`+`v3_pending`+`epistemic_category`, plus a record-once `last_reviewed` seeded from `adjudicated_at_utc` (18 claims). Drift-gated via `claims_live_status_drift.py` (mirror of SHP-5's `claims_doc_drift.py`): re-derives each block and warns when stored != derived. `validate_claims --strict` clean; 1 `needs_review` (MECH-095, active + substrate_ceiling). Wiring completed 2026-07-11: warn-only drift check added to `governance.sh` **Step 3c-bis-5** (NOT `--strict` -- the stamper is a claimed manual op, not auto-run in the pipeline). Stamper stays a manual op under a TASK_CLAIMS claim on `docs/claims/claims.yaml` (heartbeat-autostash rule). **Event-provenance augmentation 2026-07-11 (session busy-gagarin-8fca91):** per the Sec.4b schema ("carries the derived current reading + as_of + **from**"), `apply_live_status.build_provenance` now also projects an event-stream provenance sub-block `live_status.evidence` (`from`/`as_of`/`verdict`) over each claim's event slice via `project_status_head` -- the ONE projection path SHP-1/2/3 use, joining each claim by `scope_claims=[claim_id]`. Nested one level under `live_status` so `evidence.as_of` (newest bearing-event date) never collides with the reading-plane `as_of` (reading-change date). **291/878 claims carry it** (the event-bearing set; 587 event-less foundational claims omit it -> minimal churn). Fixed a latent `project_status_head._next` crash on dict `routing_detail` (identity for the string case; closure-plane `status_plane_drift` unchanged at 0/98). `claims_live_status_drift.py` extended with a SOFT event-provenance-drift bucket (re-projects + warns; NEVER `--strict` -- it fluctuates as the fleet produces evidence; the HARD self-field reading-drift stays the `--strict` gate). Terminal wiring stays **warn-only** at Step 3c-bis-5 (comment updated): UNLIKE SHP-5's Step 9c, the stamper WRITES high-contention claims.yaml, which the derive-only pipeline must not do, so it can't run in-cycle to make `--strict` safe. VERIFIED: stamper `--self-test` PASS (incl. nested-evidence idempotency + no as_of clobber); stamp 291 evidence blocks + idempotent re-run 0-change; tamper fires both HARD reading + SOFT provenance buckets (`--strict` exits 1, re-stamp restores 0); `validate_claims --strict` OK (83), `build_claims_json` OK, `generate_inter_governance_workset` still resolves `status`. PROMOTES/DEMOTES NOTHING. |
| `status_history_plane:SHP-5` | Derive architecture-doc `**Status:**` from claims.yaml | **done** 2026-07-10 | Landed `docs/apply_status_frontmatter.py` (stamps `status:`/`status_asof:`/`status_claim:` frontmatter derived from claims.yaml for the 77 docs that name a registered claim id via a `**Claim:**` line or filename stem; idempotent; PROMOTES/DEMOTES NOTHING) + `scripts/claims_doc_drift.py` (mirror of `check_closure_drift.py`: hard frontmatter-drift bucket + soft review/info buckets; warn-only, `--strict` gate). Wired as `governance.sh` Step 9b (stamp, after the nav stamper) + Step 9c (drift, warn-only). `apply_nav_frontmatter.py` now preserves the `status_*` keys across its re-stamp so the two compose either way. Retired the ~9 pure-boilerplate `**Status:**` lines; dated/prose lines KEPT per the razor (Sec.5) and surfaced in the drift report's REVIEW bucket (caught the cited sd_063 `v3_pending` drift + 7 more promotions the docs never absorbed). Standalone-invocation note mirrored into the `/update-docs` skill Step 5b. |
