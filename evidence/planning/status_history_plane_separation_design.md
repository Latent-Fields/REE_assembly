...
| `status_history_plane:SHP-3` | `status_snapshot/v1` append log + both views | **open (unblocked 2026-07-11)** | SHP-2 complete -> all blobs collapsed + snapshot log populated. Remaining: wire snapshot-append + sidecar-gen into `governance.sh`; add query API to `serve.py` (Q2=both) |
