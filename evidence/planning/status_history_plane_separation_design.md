...
| `status_history_plane:SHP-1` | Harden projector -> fleet-wide **shadow** | open | reconcile rule + union join + `needs_review`; emit shadow `live` + `*_history.md` + **diff-vs-blob report** to scratch; touches nothing canonical. Lands as `REE_assembly/scripts/project_status_head.py` |
...
| `status_history_plane:SHP-7` | Retire/auto-stamp abandoned manual docs | open | `GOVERNANCE_STATE.md`, `REE_overview.md`, `index.md` -> generated stubs or deleted (kill the third "looks-maintained-but-isn't" category) |
