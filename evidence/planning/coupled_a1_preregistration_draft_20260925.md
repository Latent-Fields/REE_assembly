...
| X13 | **O12b decided:** the stratum is the (env seed, agent seed) pair, classified from NATIVE's closed-loop steps 0-599 (the v3 pre-registered fallback), relying on shared init so INT gets the same label. The env-only classifier and its validity check stay in the skeleton report-only; Stage S0 is dropped | 3.1, 9, 14, skeleton `STRATUM_DECIDED` | user, 2026-09-25 ~18:50Z (relayed by the orchestrator; RECOMMENDATION_LOG rows not yet present at 2026-09-25T18:36Z) |
| X14 | **O13 decided: keep** NATIVE-R1..R3 and the per-variant INT-v-R1 arms (reported noise, 6.6 readout) | 5.1, 11, 14 | user, 2026-09-25 ~18:50Z (relayed by the orchestrator; RECOMMENDATION_LOG rows not yet present at 2026-09-25T18:36Z) |
| X15 | **O14 confirmed:** the paired-mean form for P1g, the INT-CODEC vs INT-ACT head-to-head and the NOVAL attribution | 8, 8.3, 8.4, 14 | user, 2026-09-25 ~18:50Z (relayed by the orchestrator; RECOMMENDATION_LOG rows not yet present at 2026-09-25T18:36Z) |
| X16 | **O15 decided: ADD both babble-attribution arms** INT-v-NOBABBLE and INT-v-BABBLE-DATA per variant (+4 arms per seed), with a pre-registered SECONDARY contrast that has its own rule and never moves A1's verdict (6.7). Cost now **~23.5 / ~26.6 CPU-h mid** (ABSENT / GROUNDED; 16 / 18 arms per seed) | 5.1, 6.6, 6.7, 11, 14, skeleton `babble_attribution` | user, 2026-09-25 ~18:50Z (relayed by the orchestrator; RECOMMENDATION_LOG rows not yet present at 2026-09-25T18:36Z) |
...
- **Decision:** add both arms (user, 2026-09-25 ~18:50Z (relayed by the orchestrator; RECOMMENDATION_LOG rows not yet present at 2026-09-25T18:36Z)). The contrast separates "behaviour babbling prevents an init attractor" from "babbling supplies extra world-head data", which INT vs NATIVE cannot (INT bundles the codec/ASP, W4 and W5 repairs).
...
| (v3b) O12b, O13, O14, O15 | **CLOSED 2026-09-25 (~18:50Z)**: O12b pair-level NATIVE stratum under shared init; O13 keep reseed arms; O14 paired form confirmed; O15 both babble arms added (6.7) | user, 2026-09-25 ~18:50Z (relayed by the orchestrator; RECOMMENDATION_LOG rows not yet present at 2026-09-25T18:36Z) | 3.1, 5.1, 6.7, 8, 11, skeleton |
...
| **O12b (v3)** | **CLOSED 2026-09-25 (user, 2026-09-25 ~18:50Z (relayed by the orchestrator; RECOMMENDATION_LOG rows not yet present at 2026-09-25T18:36Z)):** stratify per (env seed, agent seed) pair, classified from NATIVE's closed-loop steps 0-599, relying on shared init so INT gets the same label (3.1). The env-only classifier was DEGENERATE (ICC 0.023) and stays report-only | user | implemented: skeleton `STRATUM_DECIDED`; Stage S0 dropped |
| **O13 (v3)** | **CLOSED 2026-09-25 (user, 2026-09-25 ~18:50Z (relayed by the orchestrator; RECOMMENDATION_LOG rows not yet present at 2026-09-25T18:36Z)): KEEP** NATIVE-R1..R3 and the per-variant INT-v-R1 arms (reported noise; the 6.6 readout) | user | 5.1, 11 |
| **O15 (v3 addendum)** | **CLOSED 2026-09-25 (user, 2026-09-25 ~18:50Z (relayed by the orchestrator; RECOMMENDATION_LOG rows not yet present at 2026-09-25T18:36Z)): ADD BOTH** INT-v-NOBABBLE and INT-v-BABBLE-DATA per variant; pre-registered secondary contrast with its own rule, never moving A1's verdict (6.7). Wiring requirements (W2a retained-set log with step indices; W3 external-transition append path) named, not built. Cost +4 arms per seed: ABSENT mid 17 -> 23.5 CPU-h, GROUNDED 20 -> 26.6 | user | 5.1, 6.7, 11, skeleton `babble_attribution` |
| **O14 (v3)** | **CLOSED 2026-09-25 (user, 2026-09-25 ~18:50Z (relayed by the orchestrator; RECOMMENDATION_LOG rows not yet present at 2026-09-25T18:36Z)): CONFIRMED** the paired-mean form for P1g (strict, RT-1 kept), the INT-CODEC vs INT-ACT head-to-head and the NOVAL attribution | user | 6.2, 8, 8.3, 8.4 |
