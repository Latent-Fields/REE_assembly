...
- **Skeleton:** `probes/a1_draft/a1_integrated_acceptance_skeleton.py`. It has the arm wiring and I1 call sites, and it implements the whole scoring half (margins, criteria, verdict ladder, head-to-head, NOVAL attribution, the pre-A1 hold). `--selftest` (v3: 43 cases plus 8 mutations) shows that every criterion can FAIL, that every CANNOT_DETERMINE and INVALID branch can be reached, and that each head-to-head, attribution and hold branch gives its pre-registered answer.
...
- **Arms: none added.** Family NATIVE = NATIVE vs NATIVE-R1..R3 (3 pairs per seed). Family INT = INT-v vs INT-v-R1 (1 pair per seed, per variant). This gives these arms a use under paired scoring (O13).
...
| **O13 (v3)** | NATIVE-R1..R3 and INT-v-R1 no longer set any margin under paired scoring (6.2). They now carry the report-only init-dominance readout (6.6) and RT-5 continuity. Dropping them would save about 35-40% of the cost (11), and would lose 6.6 | the user | DRAFT keeps them, per the v2 decisions that added them and the 6.6 user direction |
| **O15 (v3 addendum)** | **Attribution of any init-dominance change to babbling: two arms, NOT added.** INT-v-NOBABBLE (the W2a babbling source OFF; everything else as INT-v) and INT-v-BABBLE-DATA (the E2 world member gets the same retained babbling transitions, but the agent's own developmental epoch -- phase 1 here -- stays native). Only the behaviour-babbling vs data-only-babbling contrast separates "babbling prevents an init attractor" from "babbling supplies extra world-head data". INT vs NATIVE alone cannot, because INT bundles the codec/ASP, W4 and W5 repairs. **Cost**, for both arms x both variants x 24 seeds (4 INT arms per seed): about **6 CPU-h mid** (4-16 across the measured range), i.e. about +36% on the ABSENT mid and +30% on the GROUNDED mid. For one variant only, half that. BABBLE-DATA needs a harness path that feeds retained babbling transitions to the W3 member without running phase 1 as babbling, which is not in the W2a design | **the user** | report-only either way. If added, the arms go into 6.6 and feed no criterion |
