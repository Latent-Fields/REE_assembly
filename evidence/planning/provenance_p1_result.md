...
**Built against an uncommitted concurrent amendment.** `P1-R7` exists only in another session's (`jolly-neumann-a8857e`) working-tree edit of the design note — not in `HEAD`, not on `origin`. I built the mandatory arm from that in-flight text because omitting a mandatory rival-hypothesis control is worse than building against wording that may still change. **This must be reconciled when that amendment lands.**
