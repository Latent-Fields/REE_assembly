...
1. **Is the usefulness criterion meant to REPLACE MECH-278's causal-coherence
   ...
   (type / anchor / token) rather than crowning one. The natural conservative
   move is a **fourth facet -- a value/affordance facet** -- coregistered with
   the others. The radical move is that usefulness is the *primary* individuation
   criterion and causal coherence is merely evidence for it. These have different
   builds and I do not want to assume which you mean.

2. **Does the funnel run through objects, or do objects and the funnel serve
   different jobs?** Your last sentence ("a combination ... is likely to be
   ...
   revaluability* (what specific things are at stake, worth what to me now). If
   so, the architecture is not "objects instead of deep latents" but "deep
   latents index; objects carry value".

3. **"Information need" as a drive axis -- is this SD-049 per-axis drive with a
   new axis, or something structurally different?** The existing drive axes are
   metabolic (food/water) with a novelty axis. Adding an epistemic axis to the
   *same* mechanism would make curiosity object-bound and drive-conditioned --
   which, given MECH-458's finding that curiosity is a reward-conditional
   *exploitation amplifier* with no proactive pull, might be the missing polarity
   rather than a new mechanism. But it might also be a category error: epistemic
   value is about the *agent's model*, not about the object.

4. **The argmax collapse (section 2) -- is that a defect worth fixing on its own,
   independent of everything else here?** It is a small, concrete, V3-scoped
   observation: the multi-object comparative structure is destroyed by
   `most_wanted()` before selection. It could be surfaced as a governance flag
   regardless of what happens to the larger thought. I have not raised one --
   your call.

5. **Does the re-entanglement worry in 3.2 matter to you?** A joint
   representation-and-value objective reverses the multi-stack separation
   rationale that SD-004/SD-005 were built on. I think the SD-004 precedent
   (3.3) actually shows the separation was never absolute -- O-space is already
   a consumer-defined compression -- but this deserves your judgement, not mine.
