...
closure_plan:
  ...
  nodes:
    - id: "sd_037_axis_b:P1b"
      ...
      awaiting: "conversion_ceiling_campaign:FULLSTACK -- 625e's confirmed autopsy consolidated this node's blocker into the MECH-439 F-dominance conversion-ceiling cluster (654g/485h/625e/445h); P1b cannot re-gate until the campaign's full-stack arm shows committed-action diversity survives a threat-engaged candidate pool"
      assembly_status: in_progress
      owner_exq: "V3-EXQ-625e RAN TERMINAL FAIL/non_contributory 2026-06-20 (run_id v3_exq_625e_sd037_axis_b_phase1b_joint_composite_recalibrated_20260619T233440Z_v3; reviewed; removed from queue). CONFIRMED failure_autopsy_V3-EXQ-625e_2026-06-20: the recalibrated axis-(b) MEASUREMENT threat still could not clear candidate-pool collapse -- z_harm_a remained pinned (0 crossings of 0.4), R3 conversion 1/3, R4 committed-entropy 0/3 -- because the 569i conversion PASS is ENV-CONDITIONAL and does NOT propagate to a threat-engaged candidate pool. The autopsy consolidated 625e into the MECH-439 F-dominance conversion-ceiling cluster (governance 46816d2f1a). The R3/R4 non-vacuity guards fired (self-routed substrate_not_ready_requeue, NEVER a weakens). SD-037/MECH-280/MECH-281 UNWEAKENED (substrate_ceiling / pending_retest_after_substrate). PROMOTES NOTHING."
      unblocks_claims: [SD-037, MECH-281]
      depends_on: []
      cross_plan_link: ["behavioral_diversity_isolation:GAP-A", "behavioral_diversity_isolation:GAP-B", "behavioral_diversity_isolation:GAP-I", "conversion_ceiling_campaign:FULLSTACK"]
      blocked_by: "SUBSTRATE BUILD CLEARED (2026-06-14 reconcile) -- the prior 'AMEND not yet landed' framing is STALE. The behavioural-diversity substrate amend HAS landed: (1) scaffolded_sd054_onboarding flipped ready=True 2026-06-11 (V3-EXQ-603n PASS, all four G-legs >=2/3); (2) the modulatory-bias-selection-authority channel -- the committed-argmax bottleneck behind 614d/604a/624a/640a -- is implemented + ready, validated V3-EXQ-643a PASS 2026-06-07. The NEW (downstream) blocker is a CROSS-PLAN demonstration gate, NOT a substrate build: behavioral_diversity_isolation must first show that scoring-layer diversity actually REACHES committed action. Standing evidence is that it does NOT yet -- per failure_autopsy_569f-661-654a the within-class preserver is a scoring-layer property only, and the FIRST committed-action no-lift instance AFTER the authority gate was proven operative (614e) relocated the bottleneck to GAP-A candidate-pool class collapse (cand_world_pairwise_dist=0.0), whose latest falsifier V3-EXQ-569g FAILED 2026-06-11 (r1a_entropy_only_artefact). Until committed-action diversity is demonstrated, a P1b successor would re-derive 625c's monostrategy lock."
      resume_condition: "RESUME the Phase 1b gate via a redesigned successor (V3-EXQ-625d, JOINT-COMPOSITE-ON) once behavioral_diversity_isolation demonstrates that scoring-layer diversity reaches COMMITTED ACTION (dynamic behavioural sequences) -- the GAP-A 569-lineage falsifier PASS and/or the ARC-062 falsifier on the modulatory-bias-selection-authority substrate. The substrate-BUILD half of the original condition is now MET (scaffolded_sd054_onboarding ready 2026-06-11; authority channel ready 2026-06-07); what remains is the committed-action-diversity DEMONSTRATION half. 625d MUST be the full joint composite (run THROUGH the scaffolded_sd054_onboarding scheduler path -- NOT 625c's warmup_train base policy -- with the modulatory-bias-selection-authority channel active AND MECH-341 + SD-056 diversity levers ON), under the axis-(b) env overlay, with the sharpened C3 (C3a >=1 above->below AND C3b >=1 below->above per seed, on >=2/3 seeds, per failure_autopsy_V3-EXQ-625b_2026-06-02 Section 8.3). SD-037 status is unchanged: the substrate_ceiling diagnosis at the consumer-input-threshold layer stands; P1b is upstream of that gating layer. Do NOT pursue a heavier env curriculum or an axis (c) sustained-threat scheduler -- Section 5.4 was falsified (seed 7 proves the env is capable)."
      last_updated: 2026-06-23
      governance_2026_06_23: "STALENESS RECONCILE + ASSEMBLY-FRONTIER MIGRATION (session closure-map-enhance-20260623T043407Z; plan-frontmatter only, NO claims.yaml/queue change). The governance_2026_06_19 note below said '625e QUEUED + ingested' but V3-EXQ-625e has since RUN TERMINAL FAIL/non_contributory 2026-06-20 (manifest + confirmed failure_autopsy_V3-EXQ-625e_2026-06-20 verified present; reviewed; removed from queue). owner_exq updated from the bare 'V3-EXQ-625e' to the ran/autopsied record. status in_progress -> assembling because 625e's autopsy CONSOLIDATED this node's blocker into the MECH-439 F-dominance conversion-ceiling cluster (the live conversion_ceiling_campaign) -- P1b's real resolution path is now the campaign's FULLSTACK arm (committed-action diversity surviving a threat-engaged pool), not another P1b letter, i.e. substrate being assembled, not a stalled in_progress gap. Added cross_plan_link to behavioral_diversity_isolation:GAP-I (the F-dominance root) + conversion_ceiling_campaign:FULLSTACK -- the 625e->MECH-439 consolidation was prose-only, now drawn. awaiting=FULLSTACK, assembly_status=in_progress. SD-037/MECH-280/MECH-281 substrate_ceiling / pending_retest_after_substrate UNCHANGED. PROMOTES NOTHING."
      governance_2026_06_19: "owner_exq V3-EXQ-625d -> V3-EXQ-625e (625e QUEUED + ingested into the coordinator /queue/active this session). V3-EXQ-625d self-routed substrate_not_ready_requeue (R3 0/3, R4 0/3) -- internally consistent, weakens nothing. Confirmed autopsy failure_autopsy_V3-EXQ-625d_2026-06-18: the committed-action monostrategy-lock (entropy 0.0 3/3) is OVER-DETERMINED by the axis-(b) saturated-threat regime -- z_harm_a pinned ~6 (the TRAINED harm pathway over-drives the 625b-era overlay ~14x; 0 crossings of the 0.4 threshold), candidate-pool collapse (cand_world_pairwise_dist < floor 2/3), and F-dominance over the modulatory range (seed 42 route_range operative yet committed entropy still 0). The 569i conversion PASS is ENV-CONDITIONAL: it rests on the reef-bipartite env's structural guarantee of categorically-opposite first-action argmaxes, which the saturated-threat overlay removes -- it does NOT propagate to the threat-engaged candidate pool. ROUTE: V3-EXQ-625e successor recalibrates the axis-(b) MEASUREMENT threat for the trained-pathway z_harm_a magnitude (hazard_harm/proximity_harm_scale 0.2 -> 0.02 + time-varying/PULSED scheduled-hazard duty cycle ON40/OFF40 so z_harm_a relieves and can cross 0.4 downward) + trains on the 603q-stabilized harm pathway (decoupled encoder_lr 3e-4 + warmup 250 + Stage-H num_hazards=6 so the base harm landscape forms on >=2/3 seeds) + keeps the full scaffold curriculum + 569i top-k conversion + MECH-341/SD-056 + the four R1-R4 non-vacuity guards. GAP-A conversion-propagation under threat is carried as a HARD upstream gate: R3/R4 self-route substrate_not_ready_requeue on a miss (NEVER a weakens), so a recalibrated threat that still cannot clear candidate-pool collapse cannot masquerade as an axis-(b) verdict. STATUS stays in_progress and the node remains gated on the GAP-A conversion-propagation-under-threat DEMONSTRATION (the substrate-BUILD half is met; the env-conditional 569i finding means committed-action diversity must be shown to survive a threat-engaged candidate pool before P1b re-gates). A coupled substrate POINTER (recommended_substrate_queue_entry) records the env-conditional-conversion finding on the SD-037 substrate_queue owner and points at the shared behavioral_diversity_isolation:GAP-A / modulatory-bias-selection-authority conversion-propagation ceiling -- a POINTER to existing substrate work, not a new build. SD-037/MECH-280/MECH-281 substrate_ceiling / pending_retest_after_substrate UNCHANGED; NO claims.yaml edit (tags no claim). closure_status.md + closure_drift.* regenerate from this frontmatter on the next governance.sh."
      governance_2026_06_17: "RESUME -- demonstration gate MET; V3-EXQ-625d QUEUED (session queue-experiment-v3-exq-625d-20260617T0744Z). The committed-action-diversity DEMONSTRATION conjunct of the resume_condition (the missing half at the 2026-06-14 reconcile) is now satisfied: V3-EXQ-569i PASS/supports (claim ARC-065, non_degenerate, 2026-06-16T22:40Z, label r1b_diversity_reaches_committed_action) showed committed-action diversity REACHES committed action on the TOP-K shortlist conversion substrate -- ARM_1 (e2_world_forward source + use_modulatory_shortlist_then_modulate mode=top_k k=3 + use_modulatory_selection_authority gain=2.0 normalize_basis=std + use_modulatory_channel_routing) selected-action class entropy strict-above BOTH the matched-noise control AND the collapsed proposer on 2/3 seeds, with the in-arm route-range gate (0.267>0.01, 3/3) AND e2-divergence (cand_world_pairwise_dist 0.082>0.03, 3/3) met. The behavioural-diversity GAP-A choke (V3-EXQ-569g FAIL referenced in the 2026-06-14 note) was resolved by the 2026-06-15/16 TOP-K shortlist amend to modulatory-bias-selection-authority. STATUS upstream_blocked -> in_progress; owner_exq V3-EXQ-625c -> V3-EXQ-625d. 625d is the JOINT-COMPOSITE-ON Phase-1b falsifier: a policy trained THROUGH the scaffolded_sd054_onboarding full curriculum (603n config -- NOT 625c's warmup_train base policy) running under the 569i-validated conversion config + MECH-341 (use_e3_score_diversity stratified+entropy) + SD-056 (e2_action_contrastive trained online on the axis-(b) env), measuring z_harm_a dynamic crossings (the sharpened C3a >=1 above->below AND C3b >=1 below->above per seed, on >=2/3 seeds, per failure_autopsy_V3-EXQ-625b_2026-06-02 Section 8.3) under the SD-029 axis-(b) sustained-threat overlay (scheduled_external_hazard interval=20 prob=0.7 adjacent_only + hazard_harm=0.2 + proximity_harm_scale=0.2) on a dim-matched scaffold p2 env. claim_ids=[] (substrate-readiness diagnostic; weights NO claim). FOUR pre-registered non-vacuity gates (R1 sustained-threat curriculum fired 3/3; R2 z_harm_a nonzero; R3 in-arm route_range AND cand_world_pairwise_dist above floors -- the SAME RANGE statistics 569i gates on; R4 committed-action entropy above the 569i C_R1B floor) self-route substrate_not_ready_requeue on a miss -- NEVER a weakens -- so a monostrategy re-derivation cannot masquerade as an axis-(b) verdict. Dry-run smoke PASS (full scaffold + conversion + axis-(b) measurement pipeline end-to-end; correctly self-routed substrate_not_ready_requeue at dry scale). ON 625d PASS the chain P2 -> P3 -> P4 (terminal, shared with axis-a; called V3-EXQ-483f here, an id that was in fact NEVER MINTED -- see the P4 completion_note) becomes workable (left to a follow-on session). SD-037/MECH-280/MECH-281 substrate_ceiling / pending_retest_after_substrate UNCHANGED; NO claims.yaml edit. closure_status.md + closure_drift.* regenerate from this frontmatter on the next governance.sh."
      ...
      governance_2026_06_14: "DRIFT RECONCILE (session reconcile-sd037-axisb-p1b-drift-20260614T1948Z). The blocked_pending_substrate status (set 2026-06-05) predated the substrate landings and was STALE. VERIFIED post-2026-06-05 state: (a) scaffolded_sd054_onboarding -- the explicit primary-routing AMEND target named in failure_autopsy_V3-EXQ-625b_2026-06-02 Section 8.1 -- flipped ready=True 2026-06-11 (V3-EXQ-603n PASS, all four G-legs >=2/3, non-vacuity met); (b) the modulatory-bias-selection-authority channel is implemented + ready, validated V3-EXQ-643a PASS 2026-06-07 (THE committed-argmax bottleneck behind 614d/604a/624a/640a). ADJUDICATION = resume_condition PARTIALLY MET: the substrate-BUILD conjunct is satisfied; the 'agent generates oscillating, non-monostrategy z_harm_a' conjunct is NOT, and is not yet demonstrated achievable. Decisive evidence the scoring-diversity -> committed-action bridge is still unproven: failure_autopsy_569f-661-654a establishes the within-class preserver is a scoring-layer property that does NOT reach committed action; the first committed-action no-lift instance AFTER the authority gate was proven operative (614e) RELOCATED the bottleneck to GAP-A candidate-pool class collapse (cand_world_pairwise_dist=0.0), and its latest falsifier V3-EXQ-569g FAILED 2026-06-11. behavioral_diversity_isolation GAP-A (in-progress, 569g frontier FAIL), GAP-B (partial; MECH-341 BINARY within-class preserver exp_conf 0.871, v3_pending HELD), GAP-C (in-progress, 603k owner) are NOT closed. STATUS MOVED blocked_pending_substrate -> upstream_blocked: the blocker is no longer a substrate build (landed) but the upstream cross-plan committed-action-diversity DEMONSTRATION. NOT NEWLY QUEUEABLE without that demonstration: a redesigned successor V3-EXQ-625d (joint-composite-on) is now DESIGNABLE (substrate exists) but would re-derive 625c's monostrategy lock if run before committed-action diversity is shown, conflating two failure modes. When the GAP-A 569-lineage / ARC-062 falsifier confirms committed-action diversity, 625d becomes cleanly queueable (full joint composite via the scaffolded scheduler path + authority channel + MECH-341/SD-056, sharpened C3a/C3b) -- left to a follow-on /queue-experiment session. NOTE on 625c relevance: 625c (2026-06-02) is genuinely PRE-substrate -- it ran before BOTH the authority validation (06-07) and the scaffolded readiness (06-11), and its measurement_path_note confirms it used warmup_train base policy and did NOT run the ScaffoldedSD054OnboardingScheduler; it is therefore not evidence about the joint composite. Case 3 in closure-drift terms still applies (legitimately non-terminal pending the upstream demonstration). NO claims.yaml edit; SD-037/MECH-280/MECH-281 substrate_ceiling / pending_retest_after_substrate UNCHANGED. closure_status.md + closure_drift.* are script-generated globals (regenerated by governance.sh / the concurrent bdiv-reconcile session); under the new status this node moves from drift-checker-invisible to Suppressed (case_3 + owner 625c non_contributory) -- it adds NO drifted/stale_since entry."
    - id: "sd_037_axis_b:P2"
    ...
    - id: "sd_037_axis_b:P3"
      title: "Phase 3 (re-application) -- verification diagnostic: recalibrated thresholds lift consumer outputs above zero; acceptance gate for the Phase-4 behavioural validation (see P4; the id V3-EXQ-483f used for it in earlier prose was NEVER MINTED)"
      phase: 3
      status: blocked
      severity: high
      owner_exq: null
      ...
      last_updated: 2026-06-05
    - id: "sd_037_axis_b:P4"
      title: "Phase 4 (re-application) -- terminal behavioural validation (4-arm 2x2) on the axis-(b)-recalibrated substrate; NO EXQ id minted yet"
      phase: 4
      status: blocked
      severity: high
      owner_exq: null
      ...
      last_updated: 2026-06-05
      completion_note: "Terminal validation shared with axis (a) Phase 4. A PASS clears SD-037/MECH-280/MECH-281 pending_retest_after_substrate and routes to governance for promotion candidacy. ID PROVENANCE (recorded 2026-08-15, docs-only): this node and its axis-(a) twin have named the Phase-4 run V3-EXQ-483f in prose since 2026-05-31, but that id was NEVER MINTED -- no queue entry in ree-v3 current or historical (every `git log -S` hit on the string is a reference inside some OTHER entry's title/note, never a queue_id), no script under ree-v3/experiments/, no manifest under evidence/experiments/. It was a pre-allocated placeholder, not deferred work already created, so owner_exq is null like the sibling P2/P3 nodes rather than carrying a phantom id. The Phase-4 WORK is still genuinely owed and blocked (on P3, itself behind P2/P1b); when the chain clears, /queue-experiment mints a FRESH id for it -- do NOT re-queue or search for 483f."
---
...
### 4.3 Phase 4 (terminal behavioural validation) re-application

> **Id provenance — `V3-EXQ-483f` was NEVER MINTED (recorded 2026-08-15, docs-only).** This
> section and its axis-(a) twin have named the Phase-4 run `V3-EXQ-483f` since 2026-05-31, but no
> such experiment was ever created: no queue entry in `ree-v3` current or historical (every
> `git log -S"V3-EXQ-483f"` hit is a reference inside some *other* entry's `title`/`note`, never a
> `queue_id`), no script under `ree-v3/experiments/`, no manifest under `evidence/experiments/`.
> It is a **pre-allocated placeholder, not an owed successor that someone dropped**. The Phase-4
> *work* is still genuinely owed and correctly blocked behind P3 -> P2 -> P1b; when that chain
> clears, `/queue-experiment` mints a **fresh id** for it. Do not re-queue or go looking for 483f.
> Read the "483f" below as "the Phase-4 run".
...
A Phase-4 PASS would resolve `pending_retest_after_substrate=true` on SD-037 / MECH-280 / MECH-281 and route to governance for promotion candidacy. A Phase-4 FAIL on the axis-(b) substrate would be a load-bearing falsification across BOTH axes: the cascade still cannot engage even with sustained-threat windows engaging the upstream signal AND the consumer-input thresholds cleared. That FAIL would route to MECH-295 axis or to env redesign at a level above current curriculum extensions.
