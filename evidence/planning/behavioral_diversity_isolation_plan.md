...
closure_plan:
  ...
  nodes:
    ...
    - id: "behavioral_diversity_isolation:GAP-B"
      ...
      next_owner_exq: "V3-EXQ-660a QUEUED 2026-06-10 via /queue-experiment (ree-v3 db41b7d; coordinator-ingested /queue/active) -- the OWED graded confirmation, with a KEY DESIGN CORRECTION: the owed test is NOT a temperature-graded re-sweep. 660 proved the within-class TEMPERATURE knob is structurally INERT -- byte-identical 4.861916 across T=0.5/1.0/2.0 on BOTH the primary AND secondary committed-class readouts -- because near-degenerate within-class scores make softmax(-class_scores/T) ~uniform at every T (no leverage). Under uniform within-class sampling within_class_rep_cond_entropy ~= log(distinct within-class reps), so the viable graded axis is the COUNT of available within-class representatives = the CEM POOL SIZE. 660a holds within-class T fixed (None legacy / 1.0 sampled) and sweeps cfg.hippocampal.num_candidates K in {16,32,64,128} (8 arms x 3 seeds). Pre-registered: NON-VACUITY (avail distinct within-class reps must rise with K -- the same-statistic readiness guard; else substrate_not_ready_requeue -> non_contributory, NOT a weakens) + C_GRADED PRIMARY (per-seed paired lift Delta(K)=sampled-legacy monotone-nondecreasing AND Delta(128)-Delta(16) >= 0.05 nats on >=2/3 seeds -> supports; non-vacuity met but flat/non-monotone -> weakens, the ~0.08-nat lift is a fixed structural artifact). Dry-run confirmed the axis MOVES (avail rises 2.9->40.5 across K; K honored 16->128) -- unlike temperature. PASS + governance ratification clears MECH-341 v3_pending (do NOT auto-flip the within-class default). NO supersede -- V3-EXQ-660 stays standing evidence. ARC-062 (in unblocks_claims) still untouched by this owner."
      ...
      last_updated: 2026-06-12
      governance_2026_06_12: "Closure-drift stale-since-review acknowledgement (governance cycle 2026-06-12). Flagged because owner_exq pins V3-EXQ-660 (base) while the later sibling V3-EXQ-660b is terminal -- but 660b's disposition was already absorbed at governance cycle #5 (660b weakens -> non_contributory measurement_test_design_defect; graded-in-K falsifier RETIRED, no 660c; owner deliberately re-pointed 660b -> 660, the standing PASS). No new evidence since; owner_exq stays V3-EXQ-660. Node STAYS partial (MECH-341 v3_pending uncleared). No claims.yaml status change; last_updated bumped to acknowledge."
