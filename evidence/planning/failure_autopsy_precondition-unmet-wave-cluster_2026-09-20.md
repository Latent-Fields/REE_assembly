...
## Cluster reading
**Shape.** five consecutive runs across five unrelated claims (INV-086/MECH-428, MECH-537, ARC-029, MECH-017 x2) self-route substrate_not_ready_requeue on exactly one or a few readiness preconditions; every other gate green; zero claim evidence produced. A sixth same-shape run (V3-EXQ-1050, MECH-021) was autopsied 2026-09-18.

**Independent bugs?** no -- one structural property.

**Structural property.** The gates are GOOD -- each blocked a verdict that would have been wrong. What recurs is foreknowledge and placement: (1) in 4 of 5 runs the pre-queue red-team had NAMED the mechanism that then fired (1039a: measured in the smoke; 1070; 1057; 1057a), and in the fifth (1043a) it named the late evaluation itself; each was queued with the finding owed, declared as a risk, recorded-not-gated, or marked fixed on a statistic that could not fail. (2) the abort primitive exists (_metrics.p0_readiness_gate / P0NotReady, ~35 drivers, documented in /queue-experiment) and is called ONCE, after the seed loop, in 1043a / 1057 / 1057a; 1039a and 1070 do not use it.

| run | hours | gate that fired | what the pre-queue red-team had said | when decidable |
|---|---|---|---|---|
| V3-EXQ-1043a | 11.13 | randrank_control_supra_trivial | red-team H2: 'fires only after all 6 seeds ... early-abort after seed 1 is OWED' | seed 42 (first of 6): 0.020 vs 0.05 |
| V3-EXQ-1070 | 3.52 | training_tick_budget_equalised | red-team F4 'FIXED ... realised 1710-1740, spread 1.02' (arc029_exq1066_prereg_derivation_20260919.md:394) -- a design-time ceil() identity that cannot exceed ~1.02 whatever training does | block 5 of 10 (training and measurement are interleaved per rung x seed), after 12 of 30 cells |
| V3-EXQ-1039a | 3.18 | C0d / A3 (clamp saturation) | red-team F1 named the zero-gradient route; gating it (option C) declined 2026-09-18 | seed 42 (first of 3) training: delta 0.0, saturated_frac 1.0 |
| V3-EXQ-1057 | 0.14 | additive_arm_retains_replay_gain | driver docstring F2 (v3_exq_1057_*.py:282, verbatim): 'DECLARED RISK: this gate may well fire at full scale'; queue note: 'the F2 gate FIRES at --dry-run scale ... the gate may or may not clear' | n/a (9 min) |
| V3-EXQ-1057a | 0.24 | additive_arm_retains_recency_benefit_d2 | red-team G1 predicted D2 collapses onto A | n/a (14 min) |

Compute without claim evidence: 18.2 h. Recoverable-hours claim: WITHDRAWN as a headline. ~13 h is recoverable for the GATE VERDICT only, and not losslessly: a seed-42 abort of 1039a loses the seed-43 pool-collapse finding; a block-5 abort of 1070 loses the L2/L3/L5 depth coverage; and 1043a (9.2 of the 13 h) is a high-variance margin (sd 0.0186 on mean 0.0260) -- the regime failure_autopsy_V3-EXQ-999 learnings 6-7 (confirmed) ruled AGAINST first-seed abort.

**Readings.** process_property_not_substrate, NOT test_design_ceiling, NOT substrate_enrichment

**Planning decision forced.** whether /queue-experiment should treat a red-team finding against a READINESS gate as blocking unless dispositioned with a statistic that can take the failing value. Standing-rule change; needs its own GOV-HELDOUT-1 check; recommended here, not made.

## Recommended (reported for governance; nothing spawned here)
- extend the EXISTING experiments/_metrics.p0_readiness_gate with an incremental form and call it INSIDE the seed/phase loop; on refusal, ABORT-AND-RECORD what ran (V3-EXQ-999 learning 6). Do not build a second gate module.
- scope early refusal to gates whose per-cell statistic has low cross-seed dispersion or is a phase-complete quantity; evaluate high-variance margins over the seed set (V3-EXQ-999 learning 7).
- /queue-experiment Step 4.5: a finding against a readiness gate may be dispositioned FIXED only with a statistic that can take the failing value (1070's could not).
- anchor-reachability guards evaluated at the gate's operating point.
...
## Routing note
No claim moves on this artifact. Follow-on (the helper, the skill rule) is reported here for /governance to chip after ratification; this session spawns nothing (CLAUDE.md Session Land step 6, autopsy-own-routing rule).

## Step 7c red-team (cluster level)
- **model**: opus (claude-opus-5[1m]); drafting session was fable -- cross-model pass
- **verdict**: CONTESTED
- **disposition_survived**: True
- **accepted**: ["F1 first-seed abort is NOT lossless (1039a seed 43, 1070 depth coverage)", "F2 confirmed V3-EXQ-999 autopsy already ruled against first-seed abort for high-variance gates", "F3 the abort primitive already exists; the gap is the call site", "F4 1070 decided at block 5 of 10, not 'before measurement'", "F5 '5 of 5' is 4 of 5"]
- **rejected**: ["F6 '1057 quote fabricated' -- the string is verbatim at v3_exq_1057_*.py:282; the red-team searched the queue note only. Source now cited."]
- **what_changed**: recoverable-hours headline withdrawn; helper recommendation re-scoped to the existing primitive with abort-and-record; 5/5 -> 4/5; 1070 timing corrected.
