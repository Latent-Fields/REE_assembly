...
|---|---|---|---|
| V3-EXQ-751 | diagnostic PASS | expert NOT necessary; RND clears floor unsupervised (ICM fails; floor not ceiling); novelty-class **rejected as the build** by lit-pull | **queue-experiment** — GOV-FANOUT-1 combination-aware portfolio (H-credit / H-return / H-curriculum / H-mode) |
| V3-EXQ-750 | FAIL | non_contributory / starved; precondition unmet | **queue-experiment (shared portfolio)** → substrate → re-queue; `pending_retest_after_substrate` |

> **Update 2026-07-14 (user decisions, two-step):**
> 1. Original routing (`implement-substrate` / create novelty-explorer entry) superseded by **`/lit-pull` first**. Lit-pull landed: [targeted_review_action_learning_bootstrap_class_choice](../literature/targeted_review_action_learning_bootstrap_class_choice/SYNTHESIS.md) (12 entries, 5 classes; MECH-457 lit_conf 0.836). It **refuted the novelty class as the build** (coverage-not-competence; noisy-TV/detachment; duplicates ARC-065/MECH-314) and named four composable candidate classes.
> 2. Routing now a **GOV-FANOUT-1 discrimination portfolio** via `/queue-experiment`, **combination-aware** (hopefully one class wins decisively, but the two additive halves — credit-propagation + frontier-return — may need combining). Probes on distinct axes: **H-credit** (extend the existing `backward_credit_sweep`, reuses landed substrate), **H-return** (Go-Explore archive/return), **H-curriculum** (AMIGo goal-frontier), **H-mode** (critic-utility explore/exploit gate). Portfolio must share one env/baseline + the 742-sparse and RND-5.22 reference cells, run each class alone on a different axis, and include the H-credit×H-return pair cell so additivity is directly readable. Options/skills (DIAYN) deprioritised. No substrate entry until the portfolio names the winning/additive class.
