...
**C1g (lateral_pfc route range) is a genuinely different, more real finding.** `modulatory_channel_route_range_mean` is sub-floor (<0.01) in 9 of 12 cells, and even where "active" the magnitude sits right at the floor (0.0116, 0.0435). This is not a duty-cycle artifact — the injected channel's magnitude itself looks marginal at every rung, corroborating 851's own §5 observation of "a near-zero-range injected channel [that] could still be destabilizing downstream candidate scoring/selection dynamics."
