  ...
  nodes:
    - id: WS-1
      ...
      note: >
        ...
        V3-EXQ-732b -- and NEVER MINTED (no queue entry current or historical,
        no script, no manifest; verified 2026-08-15), so its absence is CORRECT
        and deliberate, not an owed successor. Diagnostic; claim_ids=[].
      diagnostic_recurrence_metabolized:
        ...
        note: >
          ...
          brake's spirit -- the id was NEVER MINTED, and that absence is the
          refusal working, not a dropped successor. (3) BUILT: GOV-FANOUT-1 (WS-14, registered in
          docs/claims/claims.yaml) as the constructive response, which produced
          the 737/738 diverse parallel portfolio whose P-B leg refuted H2 (the
          designed third leg, P-C observation, was held to reserve and never
          minted -- see the WS-14 worked example; "739" was a phantom id, never
          queued, corrected 2026-07-29).
    ...
    - id: WS-3
      ...
      severity: high
      last_updated: 2026-08-01
      note: "DONE 2026-07-09 -> reusable block ree-v3/experiments/_lib/capability_eval.py. Separates claim-wrong from substrate-cant-act. Trained-all-ON point LANDED 2026-08-01 from V3-EXQ-728b (PASS, guard GREEN; 728's run was VOID on frozen z_world, superseded): survives-but-does-not-forage -- foraging/goal-reach/planning at-or-below random floor, survival 81.6 ticks (>oracle). REPORTED CONTEXT (non_contributory, claim_ids=[]); a MECH-457/MECH-180 competence-floor instance."
    - id: WS-4
      tier: 2
...
| WS-3 | 1 | Capability-eval yardstick | Minimal capability benchmark suite (independent of any REE claim) to separate "claim wrong" from "substrate coarse" | **DONE** (2026-07-09) → reusable block `ree-v3/experiments/_lib/capability_eval.py` built (4 metrics + oracle/random anchors, reuses V3-EXQ-724 instrumentation); calibration V3-EXQ-727 + TRAINED all-ON point V3-EXQ-728 (P0=200+P1=90, 724 A0 recipe; reports all four metrics on the 727 scale) both authored/queued (dry-run PASS). Yardstick is now the standing denominator wired into trained all-ON runs — "reported alongside every all-ON run" clause closed. **TRAINED NUMBERS LANDED (2026-08-01, from V3-EXQ-728b):** 728's 2026-07-20 run was VOID (z_world a frozen random projection; guard-red 3/3), superseded by **V3-EXQ-728b** (SD-070 P0a warmup, `zworld_p0_episodes=60`), which ran 2026-07-21 on ree-cloud-4 (PASS, guard GREEN 3/3 seeds, world_encoder 4/4 tensors moved each seed). First *valid* trained-all-ON point on the yardstick — normalized position `[random_floor, oracle_ceiling]`: foraging −0.005 (at floor), goal_reach 0.0 (at floor), planning_depth −0.21 (below floor), survival_horizon +7.21 (81.6 ticks vs 21.2 oracle / 11.5 random). Reads as **survives-but-does-not-forage** — a known MECH-457 competence-floor / MECH-180 instance. `evidence_direction: non_contributory` (baseline, `claim_ids=[]`, excluded from scoring) — REPORTED CONTEXT, not a competence verdict |
...
> **`V3-EXQ-732b` was NEVER MINTED, and that is the refusal working — not an owed successor
> (re-verified 2026-08-15, docs-only).** No queue entry in `ree-v3` current or historical, no
> script under `ree-v3/experiments/`, no manifest under `evidence/experiments/`, no
> `runner_status.json` entry. Every mention of `732b` in this plan is a record of the **decision
> not to run it**. It therefore satisfies an owed-successor absence check perfectly while being
> the opposite of owed — the same shape as the `739` correction recorded below, and the reason
> both are stated explicitly here rather than left to be re-derived. **Do not queue it, and do
> not report it as a missing/unqueued experiment.** A fair, *different-question* probe gets a
> fresh id from `/queue-experiment` once the two preconditions above hold.
...
**Worked example (this session, 2026-07-10):** the 737/738 portfolio replaced the 719a→724→732→732a sequential chain — P-A trainable policy head on the REE latent (representation), P-B local-view-achievable ceiling anchor (measurement), plus a P-C observation-encoder probe (observation) held to reserve. P-B (V3-EXQ-738) ran first and refuted H2 (the 1.0 floor is reachable from the 5×5 local view), re-valuing P-C to reserve — an early answer the sequential chain could not have produced.

**Id correction (2026-07-29 docs reconcile).** Earlier revisions of this section wrote the portfolio as "737/738/739", which read as three minted experiments. **V3-EXQ-739 never existed** — no queue entry current or historical (`git log -S` on `experiment_queue.json` is empty), no script in `ree-v3/experiments/`, no manifest in `evidence/experiments/`, and no reference in any other planning doc. Only the *reserve decision* is real: P-B refuted H2 before P-C was ever minted, so no id was allocated to the observation leg. P-A = V3-EXQ-737 (ran, 4 manifests, reviewed) and P-B = V3-EXQ-738 (ran 2026-07-10, reviewed) are the whole executed portfolio. **Do not "re-queue 739"** — the id is unallocated, and if the P-C observation axis is ever genuinely wanted it goes through `/queue-experiment` under a fresh id. (Distinguish this from V3-EXQ-732b, also absent everywhere: that absence is *correct* and deliberate — it is the REFUSED same-question power bump recorded above.)
...
- **Trained-all-ON point LANDED (2026-08-01, session gated-728b-ws3-reland):** the 727/728 numbers referenced above finally landed, but **not via 728** — 728's 2026-07-20 fleet run was scientifically VOID because `z_world` was a frozen random projection (0/61 latent_stack, 0/4 world_encoder tensors changed; `zworld_encoder_guard` red on 3/3 seeds), adjudicated by the CONFIRMED autopsy `failure_autopsy_V3-EXQ-728_2026-07-20.json`. Its PASS successor **V3-EXQ-728b** (`supersedes: V3-EXQ-728`; SD-070 P0a encoder warmup, `zworld_p0_episodes=60`; ree-v3 `b523b9c`, queued `0e7d33497f`) ran 2026-07-21 on ree-cloud-4: **PASS, guard GREEN on all 3 seeds** (world_encoder 4/4 tensors moved each seed, `world_encoder_max_abs_delta` 0.085–0.103; `n_seeds_failed=0`). This is the **first valid trained-all-ON capability point on the yardstick**. Raw per-policy means (3 seeds × 20 eval episodes, floor=`random_walk` / ceiling=`greedy_oracle`):

  | metric | random floor | **trained all-ON** | oracle ceiling | normalized |
  |---|---|---|---|---|
  | foraging_competence (res/ep) | 0.267 | **0.233** | 6.333 | −0.005 (at floor) |
  | goal_reach_rate | 0.167 | **0.167** | 1.000 | 0.000 (at floor) |
  | planning_depth | 1.933 | **1.433** | 4.267 | −0.214 (below floor) |
  | survival_horizon (ticks) | 11.5 | **81.6** | 21.2 | +7.21 (far above ceiling) |

  **Reading:** even with a correctly prediction-trained `z_world` encoder, the trained all-ON substrate sits **at or below the random-walk floor on foraging, goal-reach, and planning depth, while surviving ~4× longer than the oracle** — it stays alive (avoids hazards) but does not collect resources. This is a **survives-but-does-not-forage** signature, a known MECH-457 competence-floor / MECH-180 (goal_success ≈ 0 = death-by-hazard-avoidance-without-foraging) instance, not a new finding requiring its own EXQ. `evidence_direction: non_contributory`, `claim_ids=[]`, `trained_allon_supra_floor=False` but `trained_allon_capability_valid=True` — the position is REPORTED CONTEXT on an excluded-from-scoring baseline, NOT a governance verdict on all-ON competence. The SD-070 substrate lever that unblocked this (`sd_zworld_warmup_optimizer_group`) was itself moved to `status: validated` in `substrate_queue.json` on 2026-07-22 (validation runs V3-EXQ-742m-b / 728b / 737b), so that step is complete and is not re-done here.
