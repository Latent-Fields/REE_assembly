...
| WS-3 | 1 | Capability-eval yardstick | Minimal capability benchmark suite (independent of any REE claim) to separate "claim wrong" from "substrate coarse" | **IN PROGRESS** (2026-07-09) → reusable block `ree-v3/experiments/_lib/capability_eval.py` built (4 metrics + oracle/random anchors, reuses V3-EXQ-724 instrumentation); V3-EXQ-727 baseline calibration QUEUED (dry-run PASS); awaits run + wiring into next trained all-ON experiment |
...
| WS-9 | 2 | Intrinsic-motivation lit | Lit-pull (Oudeyer, Schmidhuber curiosity, Baldassarre) feeding WS-1 competence + goal/wanting pipeline | **DONE** (2026-07-09) → `evidence/planning/intrinsic_motivation_competence_mechanisms.md`; 4 lit entries added to `targeted_review_intrinsic_motivation_exploration/` (Oudeyer&Kaplan 2007 typology→MECH-314c, Schmidhuber 2010 compression-progress→MECH-314c mixed, Mirolli&Baldassarre 2013 competence-based-IM→UNREGISTERED, Bellemare 2016 pseudocounts→MECH-314a). **Headline: REE's IM stack is entirely *knowledge-based*; WS-1's competence floor is the predicted symptom of a missing *competence-based* IM drive. Top-ranked mechanism (goal-achievement-progress wanting, goal pipeline/drive plane) is UNREGISTERED — candidate-claim registration flagged for governance. #2 per-candidate MECH-314c learning-progress, #3 latent pseudocount MECH-314a, both blocked by the EMA→E3-selection routing (EXQ-141b/590a).** |
...
- **Deliberately did NOT re-train the all-ON stack** — the *trained* all-ON foraging denominator is produced by the in-flight V3-EXQ-724 (P1=90 A0 arm); re-training here would duplicate it. WS-3's job is the reusable yardstick + full-four-metric floor/ceiling scale + REEAgent integration.
- **NEXT (owed):** import `capability_eval` into the next TRAINED all-ON experiment (or a 724-successor) so the trained all-ON point lands on all four metrics against this calibrated scale — that closes the "reported alongside every all-ON run" clause. Chip spawned.
