...
`REEAgent.compute_prediction_loss()` ([ree_core/agent.py:8549-8567](https://github.com/Latent-Fields/ree-v3/blob/main/ree_core/agent.py)) hard-returns a zero-gradient stub (`next(self.e1.parameters()).sum() * 0.0`) whenever `len(self._world_experience_buffer) < 2`. That buffer is appended to **only** inside `_e1_tick()` ([agent.py:4641-4676](https://github.com/Latent-Fields/ree-v3/blob/main/ree_core/agent.py)), which is itself called **only** from `act()` / `act_with_split_obs()` / `act_with_log_prob()` ([agent.py:8208/8238/8260](https://github.com/Latent-Fields/ree-v3/blob/main/ree_core/agent.py)) — never from `sense()`.
