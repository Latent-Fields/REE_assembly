...
## 14. Amendment 2026-09-16T13:06:22Z -- H1 is NOT resolved (evidence_discrepancy from failure_autopsy_V3-EXQ-1039_2026-09-16)

The Step 7c F6 correction in section 13 read V3-EXQ-1039's PASS as 'H1 resolved negative, drive excluded'. V3-EXQ-1039's own autopsy, confirmed at the same user gate, finds that null VACUOUS: shaped_rl is bit-identical to its sparse_rl control on every behavioural channel in all three seeds, demo_warmstart's CE sat at ln(32) after 232 optimizer steps, and readiness gate C0d passed while that signature was present. H1 is therefore NOT RESOLVED in either direction and DRIVE IS NOT EXCLUDED. The case for V3-EXQ-1030a rests on H2's own instrument defect alone. The registry decision block for `waypoint_field_consumer_reach` now reads 0 of 2 resolved with both repaired re-runs (1030a, 1039a) owed. JSON fields `fanout_recommendation.note` and `in_flight_work_not_to_re_propose[0]` carry the replacement text; the superseded F6 wording is preserved inside them.
