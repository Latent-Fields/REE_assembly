...
### 2026-09-25 targeted lit-pull update (Q2 and Q7, per user decision rec-20260925-7f4520df)

Four new entries landed in `targeted_review_mech_080`: Rapp et al. 2024 (transdiagnostic anxiety, information sampling, valuation) and Jacoby et al. 2014 (Beads Task, intolerance of uncertainty, anxiety disorders) for Q2; Pedersen et al. 2026 (methylphenidate, adult ADHD, cost-sensitivity DDM) and Huang-Pollock et al. 2020 (diffusion model, sustained attention, ADHD, 3 samples) for Q7. Neither question closes -- both new pulls complicate the picture rather than resolving it, which is itself the finding.

- **Q2 recommendation (still (c), now with content, not just a commissioning note):** Anxiety should stay **unassigned but no longer empty-handed**. Rapp 2024 uses the identical beads-task paradigm as the banked Hauser entries and finds trait anxiety -- continuously measured, across a transdiagnostic anxious/compulsive sample -- predicts MORE draws-to-decision, carried by a valuation parameter (subjective cost of error) rather than an inferential difference. That is the same over-withholding phenotype arm 2 was vacated to compulsivity for, on Hauser's specificity claim. Jacoby 2014 independently corroborates an IU-anxiety-sampling link in a diagnosed (non-transdiagnostic) anxious sample, though on distress rather than draws-to-decision. Net effect: Hauser's "specific to compulsivity, not affective dimensions" reading is no longer the only result in the literature -- it is now in tension with a same-paradigm anxiety finding. This does not license re-assigning anxiety back to arm 2 outright (the Rapp sample cannot itself separate anxiety's contribution from co-occurring compulsivity, and its mechanism -- a valuation/cost parameter -- is not the same quantity as Hauser's urgency-emergence account or a static threshold). It does mean option (a) (drop anxiety from scope entirely) is now harder to defend than it was on 2026-09-23: there is direct, paradigm-matched evidence anxiety loads on *something* in this space. Recommendation: keep anxiety unassigned, but flag the arm-2 occupancy question (Q4's MECH-434 overlap included) as needing a clean anxiety-vs-compulsivity dissociation design before any re-assignment, rather than treating Hauser's specificity finding as settled.
- **Q7 recommendation (still open, now with a sharper shape):** Huang-Pollock 2020 (three independent samples, N=128/59, a different task -- sustained attention rather than go/no-go) replicates the DRIFT/competence side of the 2017 finding -- reduced target-nontarget sensitivity -- without independently testing starting-point bias, so it strengthens the competence-confound risk on arm 1 without adjudicating the drift-vs-starting-point question Q7 actually asks. Pedersen 2026 (adult ADHD, N=124, a cost-benefit task, three-parameter DDM) is the more consequential new entry: it finds ADHD differs from controls on ALL THREE parameters -- reduced sensitivity (drift-like), a NARROWER decision threshold, AND starting-point-biased premature responding -- which gives the starting-point-bias reading independent adult support but also reports a genuine boundary/threshold effect that Huang-Pollock 2017 explicitly did not find. The two ADHD diffusion-model papers now disagree with each other on the boundary parameter. Recommendation: read this as evidence that ADHD's prematurity is not parameter-pure in either study -- some drift/competence signature is present across every pulled paper, and the threshold/starting-point picture is task-dependent (absent in paediatric go/no-go, present in adult cost-benefit valuation). Q7 should stay open, but the design constraint in section 5 point 3 ("any arm-1 positive must show the effect is not carried by degraded evidence accumulation") should be read as harder to satisfy than it looked on 2026-09-23, not easier -- every pulled ADHD paper shows at least a partial competence contribution.
