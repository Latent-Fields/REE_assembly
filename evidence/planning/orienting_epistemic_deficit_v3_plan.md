...
closure_plan:
  ...
  nodes:
    ...
    - id: "orienting_epistemic_deficit_v3:ORNT-2"
      ...
      reconcile_2026_09_11: >
        RE-OWNING, NOT A STATUS ADVANCE (chip-20260911-ornt2-964a-plannode,
        session goofy-merkle-eebaa1; surfaced by /queue-experiment Step 9.5's
        plan-gap drift check). The owed validation named as UNOWNED by
        governance_2026_08_30 above is now QUEUED: V3-EXQ-964a, "MECH-482/SD-102
        epistemic-deficit accumulator: multi-target readiness validation"
        (ree-v3 origin/main 80e72da, 2026-09-11, priority 5, claim_id MECH-482,
        supersedes V3-EXQ-964), the owed validation for substrate_queue entry
        sd_epistemic_deficit_multitarget_readiness (still
        implemented_pending_validation, ready:false). owner_exq and
        last_updated updated accordingly. This is the same shape as ORNT-6's
        2026-08-21 refresh (chip-20260821-ornt6-owner-exq-stale-after-910b):
        no manifest exists under evidence/experiments/ yet, so status STAYS
        in_progress -- the build landed 2026-08-29 and the validation is
        queued but unrun.
        WHY owner_exq MATTERED MECHANICALLY, not just cosmetically:
        check_closure_drift.py skips any node whose owner_exq is not a string
        (scripts/check_closure_drift.py:721-724), so while it sat null this
        node was invisible to the terminal-state drift pass -- V3-EXQ-964a
        could have run to a terminal state without ORNT-2 ever being flagged
        for the next /governance cycle. Setting it restores that pickup.
        964a's own design closes the V3-EXQ-964 defect this node already
        records: epistemic_deficit is ARMED ON BOTH ARMS (964 armed one arm
        only), the manipulation is the readiness GEOMETRY alone across all
        five levers including target_frame=predicted, and C3 is gated behind
        C1 (n_targets>=2), C2 (cross-candidate range > 0) and a post-F
        magnitude gate -- so an unmet gate self-routes
        substrate_not_ready_requeue rather than emitting any substrate
        verdict. RESUME CONDITION AS OF THIS DATE, superseding both the
        `resume_condition` and the `blocking_on` fields above (which describe
        the pre-build 2026-08-22 state and are retained as history only):
        V3-EXQ-964a runs and reaches a terminal state. If C1/C2 are met and
        C3 discriminates, the multi-target readiness question is answered and
        sd_epistemic_deficit_multitarget_readiness can flip
        implemented_pending_validation -> implemented_validated, advancing
        this node toward done. If a gate is unmet, the self-route returns the
        substrate question rather than a MECH-482 verdict, and this node
        stays in_progress with a re-scoped readiness follow-on. No
        claims.yaml change from this reconcile; MECH-482 stays
        substrate_conditional / candidate with v3_pending true. Plan
        frontmatter only -- nothing queued, nothing regenerated by this pass.
      governance_2026_09_15: >
        DRIFT RECONCILED, STATUS STAYS in_progress (Case 2: terminal owner_exq with a
        CONFIRMED autopsy; governance cycle governance-20260915, user gate). V3-EXQ-964a
        RAN 2026-09-11 (FAIL/diagnostic, ree-worker; supersedes V3-EXQ-964) and its
        autopsy failure_autopsy_V3-EXQ-964a_20260914 was CONFIRMED this cycle: the
        multitarget-readiness build (substrate_queue sd_epistemic_deficit_multitarget_readiness,
        now status_phase validation_pending) is VALIDATED -- C1 max_n_targets 14-16 (floor 2),
        C2 cross-candidate readout range 4.4e-3..7.3e-3 (floor 1e-12), C5 the pre-readiness
        control arm reproduces the old n_targets==1 collapse -- so the V3-EXQ-964 defect this
        node records is CLOSED. What is NOT done: C3 (yoked downstream divergence) measured
        0/540 across all seeds, and the Step 7c red-team (opus, CONTESTED, confirmed by
        recompute) showed the driver's magnitude-reachability instrument is UNINFORMATIVE
        (n_margin_reads / n_flip_reachable_ticks / n_zero_margin_ticks / min_argmax_margin are
        byte-identical between ARM_READINESS and the inert ARM_PREREADINESS control at every
        seed; >=3 of the 10 counted ties carry a strictly negative margin). So the run does
        not establish that a flip was ever reachable on either arm. MECH-482 manifest direction
        corrected mixed -> non_contributory (flat + pack); claim stays candidate /
        substrate_conditional / pending_retest true. RESUME CONDITION (supersedes
        reconcile_2026_09_11 for the validation leg): a retest with a CORRECTED reachability
        instrument and/or a positive-control arm that injects a synthetic deficit large enough
        to force a flip (the predecessor autopsy's recommended control, never added) -- routed
        /queue-experiment by the confirmed autopsy; NOT simply a higher weight. owner_exq stays
        V3-EXQ-964a until that successor is queued; node flips to done only when a successor
        run yields a real (non-arithmetic) C3 reading in either direction.
      completion_note: >
