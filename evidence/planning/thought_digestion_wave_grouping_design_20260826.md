...
| **closure-core** (named by the 34 remaining nodes) | **73** | 34 | **27%** | **11** |
| **closure-core + 1 hop `depends_on`** | **139** | 46 | **12%** | **15** |

**73 claims** -- 13% of the backlog -- are the ones the closure map actually names as
outstanding V3 work. That is 11 waves instead of 109.

### 7b.3 The catch, and why +1 hop is the recommendation
**A narrow filter severs group-mates, and this is measurable, not theoretical.**
Closure-core alone has a **27% solo rate -- twice the unfiltered 13%** -- because the
filter cuts edges to claims just outside it. The same effect appears on the field-union
filter: **44 of 276 V3 claims have their single strongest partner (>=3.0) OUTSIDE the V3
scope**, including strong edges such as `ARC-104` -> `ARC-119` (6.78, v5),
`ARC-054` -> `ARC-055` (6.54, v4), `ARC-102` -> `MECH-299` (6.00, v4).

Expanding one hop along `depends_on` restores them: 139 claims, **solo rate back down to
12%**, mean group size 2.15 -> 3.02, at a cost of 4 extra waves. **Recommended scope:
closure-core + 1 hop.**
