...
## 7b. Scoping the backlog to "what V3 still needs" (user steer, 2026-08-26)

**User steer:** digest the claims likely needed to complete V3 first -- more tractable,
and should not reduce usefulness. Measured: the steer is right, *and* it improves grouping
quality rather than costing it -- but only if the filter is expanded by one hop. A naive
narrow filter measurably damages the groups.

### 7b.1 There is an authoritative source; do not infer from version fields
Field-inference is the obvious approach and it is the weaker one. `implementation_phase`
is **absent on 204 of 545** backlog claims (37%), so `== v3` silently drops a third of the
population. A union of every V3 signal (`implementation_phase: v3` 179, `v3_pending: true`
161, `version_relevance` v3/v3_v4 26, `binds_at_version: v3` 15) gives **276 claims**, with
**187 still carrying no version signal at all**.

The better source already exists and is generated, not hand-maintained:
`evidence/planning/closure_status.md` -- "Remaining work to close v3", currently
**34 open nodes** (+10 assembly-frontier) across 17 `*_plan.md` closure frontmatters,
regenerated by `scripts/generate_closure_snapshot.py` inside `governance.sh`. Its
remaining-work table names claims directly.

### 7b.2 CORRECTION (2026-08-26, same session): the first extraction was wrong

The first pass of this section reported a closure-core of **73** claims and
core+1hop of **139**. **Both were inflated ~5x by an extraction bug** and are
superseded by the figures below. The bug: the id scan took every line *after* the
"Remaining work to close v3" heading, which swept in the `Deferred -- not required for
v3 closure`, `Done (63)`, and `V4 / V5 forward roadmap` sections further down the same
file -- i.e. it included precisely the claims that are NOT outstanding V3 work. Corrected
by scoping the scan to the `Remaining work to close v3` (34 nodes) and
`Assembly frontier` (10 nodes) sections only. 9 backlog claims were wrongly swept in from
Deferred/Done alone.

### 7b.3 The tiers, measured (corrected)

| scope | claims | groups | solo rate | waves @3 groups |
|---|---|---|---|---|
| today: per-claim, unfiltered | 545 | -- | -- | **109** |
| grouped, full backlog | 545 | 187 | 13% | 62 |
| V3-relevant field union | 276 | 84 | 9% | 28 |
| **closure-core** (named by the 34 remaining + 10 frontier nodes) | **15** | 8 | **27%** | **3** |
| **closure-core + 1 hop `depends_on`** (SELECTED) | **31** | 12 | **16%** | **4** |

**Only 15 undigested claims are explicitly named as outstanding V3 closure work**, and 31
with one hop. This is far smaller than the field-union tier (276) -- the two are not
competing estimates of the same thing: closure-core is "named in the closure map as
blocking a remaining node", field-union is "carries a v3 marker anywhere". The gap between
31 and 276 is real and unresolved (Open Question 7).

**User decision (2026-08-26): run closure-core + 1 hop first.** At 31 claims / 12 groups /
~4 waves this is a short campaign and functions as a **pilot** of the grouped mode on the
highest-value slice, with the field-union tier available as the follow-on.

### 7b.3b Why +1 hop earns its keep (worked, not asserted)
Closure-core alone: **27% solo, twice the unfiltered 13%** -- the narrow filter severs
edges. The hop restores them, and Appendix A shows the mechanism concretely: **Group 3**
(`MECH-254`, `SD-027`) only becomes coherent because the hop pulls in `SD-064`, the
global-workspace access channel that is the *organising concept* for both. Without it,
two claims would be digested without the concept they instantiate.

The same effect on the field-union tier: **44 of 276 V3 claims have their single strongest
partner (>=3.0) OUTSIDE the V3 scope**, including `ARC-104` -> `ARC-119` (6.78, v5),
`ARC-054` -> `ARC-055` (6.54, v4), `ARC-102` -> `MECH-299` (6.00, v4).

### 7b.4 Out-of-scope claims as read-only CONTEXT MEMBERS
A second, cheaper mitigation that composes with any tier. The value of grouping is
*visibility*, not *co-processing* -- so an out-of-scope claim can be pasted into the
group's prompt as a **read-only context member**: visible to the agent for
disambiguation, **not digested, given no disposition, and not counted against the review
load**. At `aff>=4.0` this pulls in **72 distinct out-of-scope blocks** (42 v4, 26
unphased, 4 v5/v6) while the review load stays exactly the scoped set. This preserves
cross-version disambiguation without paying review cost for V4/V5 claims the user does not
want to decide on yet.

### 7b.5 The 187 unphased claims
187 backlog claims carry no version signal. **69 of them have a >=3.0 affinity edge into
the V3 set** and can inherit V3 priority from their neighbours; the remaining 118 have no
strong V3 edge and are the natural "later course". This is a use of the affinity graph
that the version fields cannot provide, and it is cheap.

### 7b.6 Convergent validation
`MECH-264` appears in the closure-core -- independently the *same* claim held-out case
(ii) flagged as a live registry inconsistency (its partner `SD-033e` is digested, it is
not). The closure filter surfaces it without being told to. The closure-core also contains
several obvious lettered families (`MECH-312a/b/c`, `MECH-314/314a/314b`,
`MECH-252/253/254`, `MECH-381..384`) which are exactly the shape grouping handles best and
per-claim dispatch handles worst.

### 7b.7 How this composes with the skill
This is a **Step 3 worklist filter**, upstream of and orthogonal to the grouping
machinery -- it changes *which* claims enter the pool, not how the pool is clustered. It
fits the existing `args` contract as a new course (e.g. `v3-closure`), alongside the
existing cluster-prefix / `asked` / `believed` / `retire` filters. The stopping rule,
the two operating modes, and the disposition taxonomy are all untouched.
...
## Appendix A -- the selected worklist (closure-core + 1 hop), 2026-08-26

31 claims, 12 groups, ~4 waves at 3 groups/wave. `cap=5, floor=3.0`, edge-first.
`CORE` = named by a remaining/frontier closure node; `hop` = pulled in one `depends_on` hop.
Ordered by closure-core density. Generated, not hand-curated -- re-derive if the closure map moves.

```
================ CONCRETE WORKLIST: closure-core +1hop ================

--- GROUP 1  (5 claims, 5 closure-core) ---
   [CORE] MECH-312b  practice_maturity_weighting (Smith_Graybiel_habit_takeover_analog). Arbitration weight t
   [CORE] MECH-312c  affective_stream_modulation_of_arbitration_REE_novel. Arbitration weight is modulated by
   [CORE] MECH-316   cross_episode_regularity_extraction (episodic_RL_successor_representation_CLS_monosynapt
   [CORE] MECH-317   behavioural_pattern_compression (option_formation_striatal_chunking_analog). A mechanism
   [CORE] MECH-318   rule_state_abstraction_substrate (meta_RL_recurrent_task_state_representation). A mechan
   join: subject policy.arbitration, same reg date

--- GROUP 2  (4 claims, 2 closure-core) ---
   [CORE] MECH-263   OFC-analog state-space and specific-outcome representation: SD-033b represents both a st
   [CORE] SD-033b    OFC-analog (specific-outcome prediction and task-structure cognitive map): module that r
   [hop ] ARC-113    The rule pipeline is an ORDERED CYCLE of non-collapsible stages: experience -> pattern s
   [hop ] MECH-298   Event-gated frontal write at goal-instantiation moments: when GoalState transitions from
   join: depends_on, ns pfc, same reg date

--- GROUP 3  (3 claims, 2 closure-core) ---
   [CORE] MECH-254   At each E3 heartbeat cycle, a top-k selection over active E1/E2 latents determines which
   [CORE] SD-027     Capacity-limited selection for E3 access occurs at the E1/E2 -> E3 boundary via a pulvin
   [hop ] SD-064     REE instantiates a global-workspace-like access channel: a capacity-limited selection-fo
   join: depends_on, ns attention, same reg date

--- GROUP 4  (5 claims, 1 closure-core) ---
   [CORE] MECH-309   Monomodal policy collapse is the equilibrium of a parametric-policy agent without a rule
   [hop ] MECH-349   CandidateRule mint (ARC-063 CREATE face): a non-gradient structural event mints a distin
   [hop ] MECH-437   Rule-pool distinguishability is a MAINTAINED property, not a minting byproduct: keeping 
   [hop ] MECH-438   Constructive rule-key separability: minted CandidateRule context keys should be assigned
   [hop ] SD-054     Reef enrichment substrate: env-level behavioral-diversity substrate adding coral-reef sa
   join: depends_on, ns policy

--- GROUP 5  (5 claims, 1 closure-core) ---
   [CORE] MECH-314   structured_curiosity_bonus (frontopolar_uncertainty_driven_exploration_expected_free_ene
   [hop ] MECH-314a  novelty_bonus_striatal_analog. A reward bonus that tracks recency-of-encounter; new or r
   [hop ] MECH-314b  uncertainty_driven_curiosity_frontopolar_analog. A reward bonus that tracks epistemic va
   [hop ] MECH-314c  learning_progress_curiosity_intrinsic_motivation_analog. A reward bonus that tracks the 
   [hop ] MECH-441   model_disagreement_directed_curiosity (RND / Plan2Explore analog). A mechanistic refinem
   join: depends_on, ns policy, same reg date

--- GROUP 6  (2 claims, 1 closure-core) ---
   [CORE] ARC-058    Shared forward-model trunk for aversive prediction: a single HarmForwardTrunk substrate 
   [hop ] ARC-061    Self-attribution is implemented via a family of forward-model comparators at motor, inte
   join: depends_on

--- GROUP 7  (2 claims, 1 closure-core) ---
   [CORE] MECH-257   E2_x is a dual-function substrate: one forward model read in two modes, retrospective co
   [hop ] MECH-393   Retrieval-scope vs action-authority split: retrieval for reflection is distinct from ret
   join: depends_on

--- GROUP 8  (1 claims, 1 closure-core) ---
   [CORE] MECH-312a  uncertainty_reliability_weighting (Daw_2005_Lee_2014_dual_systems_arbitration_analog). A

--- GROUP 9  (1 claims, 1 closure-core) ---
   [CORE] SD-031     Single-pass agency attribution on the z_world (causal-footprint) reafferent stream: resi

--- GROUP 10  (1 claims, 0 closure-core) ---
   [hop ] MECH-337   Caregiver-scaffolded rule-population mechanism (the 'in' face of the ARC-063 CandidateRu

--- GROUP 11  (1 claims, 0 closure-core) ---
   [hop ] MECH-343   difficulty_gated_proposal_entropy (controlled proposal-entropy widening under goal block

--- GROUP 12  (1 claims, 0 closure-core) ---
   [hop ] MECH-474   LEARNING-MECHANISM META-SELECTION (narrow form): the control plane selects among the lea
```
