...
- **U1 — the competence wall** (MECH-457 cluster; `mech457_competence_bootstrap_explorer`, substrate_queue `node_class: complex (probe-gated)`): the integrated all-ON agent is not behaviourally competent enough to produce measurable committed behaviour (719a/724/732/737 lineage; re-derive brake fired at the 7th non_contributory; four axes eliminated 769-772). **[CORRECTED 2026-09-01 — see Addendum v1.2 correction 1]** ~~Per `CURRENT_FRONT.md` the hypothesis space is down to **1 of 20 rival explanations standing (~80% ruled out)** — this unknown is nearly discharged.~~ The discrimination is COMPLETE, not nearly-discharged: registry qid `competence_floor` has all 20 hypotheses (and every split child) terminally resolved as of V3-EXQ-821b (2026-08-08) — 16 resolved out, 4 confirmed, 0 alive.
...
| `mech457_competence_bootstrap_explorer` (U1; 724/732/737 lineage) | blocked_pending_discrimination | **complex (probe-gated)** | **[CORRECTED 2026-09-01 — see Addendum v1.2 correction 1]** The GOV-FANOUT-1 discrimination this row cites is COMPLETE (all 20 hypotheses terminally resolved, V3-EXQ-821b 2026-08-08), not standing at 1 rival — the real remaining blocker is the MECH-229 dependency (v3_pending). Whether `node_class` should change from `complex (probe-gated)` accordingly is an open governance call (GFLAG-0093), not decided here. |
...
| INV-050 MEL producer link (+ INV-051, a-fortiori) | **[CORRECTED 2026-08-29 — see Addendum v1.1 correction 4]** producer RESOLVED 2026-08-01; commit-bisection open | **puzzle (known rules)** (narrowed from `complex (probe-gated)`) | This row described the 2026-07-08 RE-PARKED state; stale. The producer test-bed (SD-MEL-PRODUCER, `docs/architecture/sd_mel_producer.md`) was built 2026-07-21 and validated V3-EXQ-798/798a; the ecological end-to-end link RESOLVED 2026-08-01 (`failure_autopsy_V3-EXQ-861_2026-08-01` + `_845_2026-08-01`, epistemic_category GRADUATED substrate_ceiling -> standard) — the original unknown (does ecological graded MEL exist at all) is answered. Live open thread is narrower: which of 11 commits between `f810969` and `17befb8c` caused the 861e seed-271 HIGH-arm collapse (candidate `76cbf84`, ContextMemory.write single-slot-fixed-point repair); H1/H2/H3+CONTROL closed non-contributory per `failure_autopsy_861f/861g/861h` (2026-08-21..25). Method (re-run at each candidate pin) is known, not a fresh empirical fork — routed to a proposed low-priority substrate_queue lane (`mel-f810969-vs-17befb8c-collapse-bisection`, not confirmed landed in `substrate_queue.json` as of this correction). |
...
1. **[CORRECTED 2026-09-01 — see Addendum v1.2 correction 1] SUPERSEDED — ~~Finish the U1 competence discrimination (GOV-FANOUT-1: H-bc-prior vs H-approach-primitive). 1 of 20 rivals standing; one more discriminating result names WHICH competence dependency to build~~.** The discrimination already finished (V3-EXQ-821b, 2026-08-08, all 20 hypotheses terminally resolved) — nothing is owed on this probe as originally stated. The real remaining blocker converting `mech457_competence_bootstrap_explorer`, jlens A->GATE-B->B->MECH-191, SD-033e validation, and the GAP-B retest wall into plain backlog is now the MECH-229 dependency (still v3_pending), not a further discrimination run. Do not queue a re-derive of this discrimination — it would violate the re-derive brake and the qid's own `growth_restriction` (GFLAG-0093).
...
4. **[CORRECTED 2026-08-29 — see Addendum v1.1 correction 4] SUPERSEDED — ~~Graded-MEL diagnostic on a non-converging environment (INV-050 producer)~~.** The named test-bed already exists (SD-MEL-PRODUCER, built 2026-07-21) and has already run repeatedly (V3-EXQ-798/798a, 845, 861, 861a-h through 2026-08-25); the producer link RESOLVED 2026-08-01. Nothing is owed on this probe as originally stated. If anything remains owed on this lineage it is the narrower f810969-vs-17befb8c commit-bisection (candidate `76cbf84`) — diagnostic/non-voting, not a probe for this sweep's list.
...
4. **INV-050 MEL producer row (table + probe #4) — STALE, described the 2026-07-08 RE-PARKED state.** `chip-20260827-inv050-nonconverging-mel-env` (spawned below) found on its own STOP-CHECK that the producer test-bed (SD-MEL-PRODUCER) was already built 2026-07-21 and validated V3-EXQ-798/798a, that the ecological producer link RESOLVED 2026-08-01 (`failure_autopsy_V3-EXQ-861_2026-08-01` + `_845_2026-08-01`, epistemic_category GRADUATED substrate_ceiling -> standard), and that a further diagnostic family (V3-EXQ-861b through 861h, through 2026-08-25) had already isolated the live open question — which of 11 commits between `f810969` and `17befb8c` caused the seed-271 HIGH-arm collapse (candidate `76cbf84`) — as its own separate, already-proposed low-priority substrate_queue lane (`mel-f810969-vs-17befb8c-collapse-bisection`). Corrected 2026-08-29 (`chip-20260829-debt-classification-inv050-stale-row`); the v1.0 table row and probe #4 above are amended in place (rather than left silently stale) so a future sweep does not re-spawn the same completed work a third time.
...
# Addendum v1.2 — 2026-09-01 correction (GFLAG-0093)

**Added:** 2026-09-01, headless chip session `chip-20260901-gflag0093-current-front-stale-renderer`, closing out GFLAG-0093. That flag found the U1 discrimination this doc describes as "1 of 20 rivals standing" had in fact already gone terminal three weeks earlier.

## Corrections to v1.0/v1.1

1. **U1 row (headline finding, summary table, probe #1) — the discrimination is COMPLETE, not "1 of 20 rivals standing".** `chip-20260827-competence-final-discriminator` (dispatched to queue the next discriminating experiment per this doc's own probe #1) found on its own STOP-CHECK that registry qid `competence_floor` has all 20 hypotheses (and every split child) terminally resolved as of V3-EXQ-821b (2026-08-08, eliminating H-consummation-binding and closing the 4-leg retention portfolio) — 16 resolved out (eliminated/split), 4 confirmed, 0 alive; the registry's own decision block already states the discrimination phase is complete and `growth_restriction` is closed to further fan-out. No experiment was queued for it (would have violated the re-derive brake and the qid's own growth restriction). The stale "1 of 20" figure traced to `generate_current_front.py` reading a `hero.surviving`/`reduction_ratio`/`bits_removed` rollup field that folds in a `synthesis.surviving_label` heuristic never updated after the last alive leg resolved — fixed separately (`REE_assembly/scripts/generate_current_front.py`, this session) to derive its hypothesis-space line from the per-hypothesis state tally (`alive`/`confirmed`/`resolved_out`/`superseded`) instead, and `docs/CURRENT_FRONT.md` regenerated. The real remaining blocker on the U1-gated chain (`mech457_competence_bootstrap_explorer`, jlens A->GATE-B->B->MECH-191, SD-033e validation, GAP-B retest) is now the MECH-229 dependency (still v3_pending, provisional), tracked in `substrate_queue.json`'s `mech457_competence_bootstrap_explorer` entry (corrected in place 2026-08-28 by the same chip session).
   **NOT decided here:** whether `mech457_competence_bootstrap_explorer`'s `node_class` should change from `complex (probe-gated)` to something reflecting "dependency-blocked, discrimination done" once MECH-229 clears. That is a governance call, left open by GFLAG-0093 pending `/governance` review — the corrections above leave the `complex (probe-gated)` token in the table as-is rather than pre-empting that call.
