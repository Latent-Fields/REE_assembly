...
**The driver anticipated this confound but pre-committed it to one direction only.** Its F1b/F2 note says -- correctly -- that *"a 'dormant' reading alongside margins that sit far above 0.05 supports a scale/calibration reading; a 'dormant' reading with margins hovering near 0.05 supports a genuine near-tie-rate reading."* Both clauses are conditioned on a **dormant** outcome. The observed outcome is **engaged**, and the confound is exactly as strong in that direction: the engaged LABEL is carried by all 7 seeds (7/7 recruit at least once; 4/4 without the 3 near-threshold seeds), while the recruitment RATE is bimodal: 3 seeds at 87-100% of episodes, 4 at 10-23% with median margins **32x to 223x** the threshold [CORRECTED at the 2026-09-16 cross-model red-team pass (Opus), F3]. A pre-registered confound analysis that names only one of two possible outcomes is a coin-flip away from not applying, and this run landed on the side it did not cover.
...
## Step 7c -- adversarial red-team pass (run 2026-09-16T12:19:13Z, model Opus, cross-model): VERDICT **CONTESTED**, four recommendation-layer findings applied

The science (scale-determined recruitment; PASS unconditional; no ARC-131 test) was CONFIRMED and strengthened by independent recompute. Applied: **F1** SD-091 is CLOSED under `check_substrate_path_overlap._entry_is_open` (status contains 'implemented'), so the severity blast-radius leg (b) and the gating argument in Explicitly-NOT-recommended bullet 3 are void; the `degrading` call now rests on merits alone and governance should consider re-expressing SD-091's status string so a severity on it is enforceable. **F2** the narrow-supports check was waived on the false premise that ARC-131 has no supports; `claim_evidence.v1.json` shows 6 literature supports / 0 experimental (`plausible_unproven`), so `narrow_supports_flag` is now true and the draft note states that posture. **F3** the note's 'carried entirely by 3 of 7 seeds' contradicted the driver's label rule (7/7 recruit; 4/4 without the high seeds) -- reworded to label-vs-rate. **F4** the amend demoted SD-091 from its LIVE priority 2 to 4 while describing the move as internal to the draft -- now disclosed as an explicit 2 -> 4 demotion for governance to accept or refuse.

## Step 8 gate outcome -- CONFIRMED 2026-09-16T12:22:00Z

CONFIRMED as recommended, with red-team corrections F1-F4 applied: direction non_contributory, category standard, ARC-131 stays candidate; set diagnostic_evidence_adjudicated and write the first evidence_quality_note stating the literature-only posture (6 lit supports / 0 exp, plausible_unproven; narrow_supports_flag true); narrow ARC-131's stale 'no endogenous monitor' note; substrate AMEND SD-091 with severity degrading (merits only -- SD-091 is closed under the overlap gate so severity gates nothing) and the DISCLOSED priority demotion 2 -> 4; routing queue-experiment (V3-EXQ-1038a: dissociate scale from conflict; governance chips it).
