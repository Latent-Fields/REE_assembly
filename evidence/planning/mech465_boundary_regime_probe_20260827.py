"""Scratch probe (NOT an experiment, no manifest): does a BOUNDARY-REGIME commit
threshold produce enough WITHIN-SEED running-variance dispersion for MECH-465's
pre-registered P1/P2 gates? Mirrors the v3_exq_785a harness exactly."""
import sys, json
...
def run(seed, thr, n_ticks, rng):
    agent, env, od, landed = build(seed, thr)
    ...
    for tick in range(n_ticks):
        ...
        if done:
            _o, od = env.reset()
    return rows, nfresh, nlat, landed

if __name__ == "__main__":
    thr = float(sys.argv[1]); seed = int(sys.argv[2]); n = int(sys.argv[3])
    rng = np.random.default_rng(1234 + seed)
    rows, nf, nl, landed = run(seed, thr, n, rng)
    post = [r for r in rows if r["tick"] >= 90]
    rv = np.array([r["rv"] for r in post]) if post else np.array([np.nan])
    print(json.dumps(dict(
        seed=seed, thr=thr, from_dims_landed=landed, n_fresh=nf, n_latched=nl,
        n_post=len(post),
        rv_med=float(np.median(rv)), rv_p5=float(np.percentile(rv,5)),
        rv_p95=float(np.percentile(rv,95)),
        ...
        eff_med_by_level={str(u): float(np.median([r["eff"] for r in post if r["u"]==u]))
                          for u in URG},
    ), indent=1))
