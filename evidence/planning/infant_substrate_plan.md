...
closure_plan:
  ...
  nodes:
    ...
    - id: "infant_substrate:GAP-14"
      ...
      last_updated: 2026-09-16
      governance_2026_09_16: "SECTION-3 FORK RESOLVED (i) PORT -- USER DECISION 2026-09-16; SUBSTRATE BUILT AND CONTRACT-VALIDATED BUT NOT YET LANDED; COMPARISON DELIBERATELY NOT QUEUED (session distracted-antonelli-5ba664, chip-20260916-gap14-warmup-port-then-queue). infant_gap14_redesign_staged_20260827.md section 3 offered (i) port the 603n-style harm/survival warm-up into the InfantCurriculumScheduler path vs (ii) re-home the comparison on scaffolded_sd054_onboarding; the user chose (i). PREREQUISITE (b) IS NOW CLEARED ON THE SCHEDULER ITSELF, which is what section 3 said had never been done: it was cleared on 603n, which runs a DIFFERENT harness. BUILT: ree-v3/experiments/_lib/infant_warmup.py, which composes the scaffold Stage-0 -> Stage-0b -> P0 -> Stage-H onto an agent and hands that SAME agent to InfantCurriculumScheduler, unfreezing the goal pipeline at handoff (the scaffold run_p1 normally does that, and the curriculum replaces run_p1). Nothing under ree_core/ changed; the module is new and nothing predating it imports it. substrate_queue entry infant-curriculum-warmup-port. NOT YET LANDED AS OF THIS WRITING -- the two ree-v3 files are written and their contract is green (8/8 locally; on the hub inside a 4963-passed run) but sit UNCOMMITTED in the ree-v3 main checkout. scripts/precommit_contracts.sh fires a full contracts run on any change under experiments/_lib/ and blocks on a non-zero suite, and the suite is RED ON TRUNK for two tests in test_from_dims_flag_reachability.py naming use_e3_channel_commensurability in V3-EXQ-1038a (b544b10) -- failures that reproduce with BOTH of these files removed, and that are themselves a scanner false positive (1038a enables the flag correctly at line 403; its only live from_dims call passing it, line 1153, is a negative control asserting non-acceptance at line 1157). Landing is gated on chip-20260916-fromdims-scanner-false-positive, or on an operator decision to permit a --no-verify commit (attempted, DENIED by the auto-mode classifier as a CI bypass, not worked around). VALIDATION: ree-v3/tests/contracts/test_infant_warmup_port.py, 8 assertions green (locally, and in the full contracts suite on the hub). The load-bearing one asserts the SCHEDULER OWN Phase 1 -> 2 gate ADMITS the warmed z_goal and REFUSES a cold 0.0 -- prerequisite (b) stated as the decision it governs, not a bare magnitude; measured z_goal 0.452 at contract budgets against the 0.30 gate. All assertions are upstream of the discrete quantizer, so the contract is portable across machine classes. THE COMMISSIONED SHAPE WAS NOT IMPLEMENTABLE AND MUST NOT BE RE-ATTEMPTED: the chip specified a phase-0 knob ON InfantCurriculumScheduler wired through the three REEConfig.from_dims sites. Verified at ree-v3 HEAD -- experiments/infant_curriculum.py imports only math and typing, holds no agent/optimizer/trainable state and its __init__ takes no agent; scaffold_train_harm_pathway is a ScaffoldedSD054OnboardingConfig field (scaffolded_sd054_onboarding.py:584) backed by driver code (:1311/:1396/:1493/:2413) with ZERO occurrences under ree_core/; and there is no ree_core/curriculum/ package. A contract (C5) pins the scheduler torch-free so the shape cannot be re-attempted silently. Independently flagged the same night by orchestrate-20260916-2250 pre-flight preflightV-science-20260916 (RED). TWO FINDINGS THE REDESIGN DOC DID NOT ANTICIPATE. (1) DIMENSION MISMATCH: scaffold envs are body 17 / world 350, the 591-lineage infant env is body 12 / world 250 (limb damage +5 body, reef view +25, SD-049 per-type views +75). A warmed agent has fixed-width encoders and cannot step a 12/250 env. Zero-padding is REFUSED (100 constant-zero dims where the encoder was trained to read per-type resource proximity and the reef gradient). Resolved by building the INFANT envs with the same structural kwargs; measured, all four phases then match at 17/350/5. Those kwargs are orthogonal to what the curriculum manipulates so cross-ARM comparability holds, but the env is no longer bit-identical to 591 and comparability to 591 own numbers must NOT be claimed. (2) COST: measured per env step at 17/350 -- warmed 603n-style build 0.1168 s/step, 591c diversity build 0.0909 s/step. The section-7 3-arm x 5-seed x 2000-episode shape is ~13.0 h per cell and ~195 h total before warm-up, against a corpus maximum single run of 66.5 h. PHASE_EP_MIN is [0, 100, 500, 2000], so below ep 500 the curriculum arm never enters Phase 2 and below ep 2000 never Phase 3 -- every affordable budget tests something narrower than the 4-phase curriculum section 7 describes. WHY NOTHING WAS QUEUED -- THE DECISIVE REASON IS THIS NODE OWN STANDING INSTRUCTION, not the cost: blocked_by ends 'EXQ-ISEF-005 ... stays blocked_pending_substrate until (c-1) resolves AND the (c-2) crossing-count criterion is wired into the scheduler; do NOT re-queue it on the (b)-cleared signal alone.' This session cleared EXACTLY (b) and nothing else. (c-2) has been wired since 2026-06-19; (c-1) is OPEN -- confirmed failure_autopsy_V3-EXQ-996_2026-09-04 records that 996 seed 46 reproduces c-1 exactly and that EXQ-ISEF-005 stays blocked. So queueing the comparison now would run it straight into a known-open blocker AND violate a standing instruction on this node. The chip that commissioned this work asked for the queue step; the plan forbids it on this basis, and the plan wins. Independently, the cost finding above means no affordable budget tests the 4-phase curriculum section 7 describes, so choosing that budget silently would also change what the experiment is -- a scope decision for the user rather than an authoring default. V3-EXQ-1049 was reserved during this session and is RELEASED unconsumed -- an unqueued reservation must not squat an ID, so the eventual authoring session takes the next free number rather than assuming 1049. The queue step is a small increment on the landed port once (c-1) clears and a budget is chosen. status UNCHANGED (blocked_pending_substrate): (c-1) seed-46 exploration-strength collapse is untouched by this work and remains the parent gate, repointed 2026-07-21 to the behavioural-competence wall. No claims.yaml change; no experiment queued; EXQ-ISEF-005 stays blocked."
