...
**066 and 122 (both timestamps)**: backfill registration only, matching the already-correct claims.yaml narrative. 122's two timestamps are genuinely distinct (not naming duplicates): the earlier is a thin 1-seed pilot showing an illusory positive on noise (AUC_ON=0.64>AUC_ABLATED=0.36 at min_contacts=10, failing its own C4 gate), the later is the well-powered 2-seed full run where the sign correctly reverses (AUC_ON=0.41<AUC_ABLATED=0.75 at min_contacts=171-192) -- a clean small-N-noise-manufactures-illusory-signal case. **052**: backfill only, diagnostic bug already resolved.
