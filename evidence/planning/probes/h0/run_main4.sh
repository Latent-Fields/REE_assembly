...
# re-taken (waiting if another worker holds it), and the process is SIGCONTed. DEVIATION: TICKS (default 300) for seeds 32-33 and arm C; see record.
...
R=$H/results/main.jsonl; TICKS=${TICKS:-300}
...
for job in "$@"; do
  ...
  [ "$a" = C ] && r=$(rate $s)
  take; HELD=1
  /opt/local/bin/python3 $H/h0_reset_probe.py . $a $s $TICKS $r $R > $H/results/main_${a}_${s}.out 2>&1 &
  pid=$!
  t0=$(date +%s)
