"""Worker G Mac SMOKE 2 (ADDENDUM 2, pre-registered at REE_assembly 13621d6db8): detector v2 + shrunk M2S on
fresh seeds 45/46. Derived from valuation_smoke_probe.py (ADDENDUM 1); only M2S and detector_v2 are new.

Original docstring follows. Worker G Mac SMOKE: harness validation for the grounded-valuation battery (NOT evidence for any
...
class Rule:
    ...
    def update(self, closed):
        ...
        if self.kind == "M0" or closed is None:
            ...
            elif self.kind in ("M2", "M2S"):
                self.A.append(closed["a"]); self.V.append(v); self.R.append(closed["R"])
                n = len(self.R)
                if n >= 20:
                    gain = 1.0 if self.kind == "M2" else n / (n + 100.0)
                    self.theta = 0.35 * fe_t(self.A, self.V, self.R) * gain
...
def detector_v2(arm):
    """Pre-registered (13621d6db8): FIRE if theta_harm <= -ln4 at ANY tick, OR (final theta_harm < -0.5
    AND fraction of nonzero harm updates negative > 0.75). Reads only the harm weight's trajectory."""
    th = np.array([t["theta"][1] for t in arm["ticks"]]); d = np.array([t["dtheta"][1] for t in arm["ticks"]])
    nz = d[np.abs(d) > 1e-12]
    frac_neg = float(np.mean(nz < 0)) if len(nz) else 0.0
    ever_floor = bool(np.any(th <= -LN4 + 1e-9)) if len(th) else False
    final = float(th[-1]) if len(th) else 0.0
    fires = ever_floor or (final < -0.5 and frac_neg > 0.75)
    return {"ever_at_floor": ever_floor, "final_theta_harm": final, "min_theta_harm": float(th.min()) if len(th) else 0.0,
            "frac_nonzero_harm_updates_negative": frac_neg, "n_nonzero_harm_updates": int(len(nz)), "FIRES": bool(fires)}
...
def main():
    ...
    for kind in ("M0", "M1RAW", "M2S"):
        arm = run_arm(a.seed, enc_state, headD, evT, gate_n, a.steps, Rule(kind, a.seed))
        arm["detector_v1_info"] = detector(arm); arm["detector"] = detector_v2(arm); out["arms"][kind] = arm
    ...
    m2d = np.array([t["dtheta"] for t in out["arms"]["M2S"]["ticks"]])
    m4_sd = m2d.std(0)
    arm = run_arm(a.seed, enc_state, headD, evT, gate_n, a.steps, Rule("M4", a.seed, m4_sd=m4_sd))
    arm["detector_v1_info"] = detector(arm); arm["detector"] = detector_v2(arm); arm["m4_sd"] = m4_sd.tolist(); out["arms"]["M4"] = arm
