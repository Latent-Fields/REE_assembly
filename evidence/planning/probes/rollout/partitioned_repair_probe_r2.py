"""Worker I fresh-seed REPLICATION of ADDENDUM 3's partitioned closed-loop test
(REE_assembly/evidence/planning/e2_rollout_divergence_and_proposal_state_dependence_20260924.md).
bt0924-replication (Worker I, orchestrate-20260924-breakthrough), chip_ref
chip-20260924-r5b-r2-fresh-seed-replication.

BYTE-IDENTICAL to the original partitioned_repair_probe.py (Worker D,
origin/main 44c55300ca, ree_core unchanged through this worker's HEAD
863d23d65a9093de2a7a02482e9d78f4bf6f5702 -- confirmed no diff on
ree_core/predictors/e3_selector.py, ree_core/hippocampal/{module,config}.py,
ree_core/environment/causal_grid_world.py, experiments/_harness.py) EXCEPT the
ARMS dict below, which is redefined per this worker's PRE-REGISTRATION
(COV dropped, per ADDENDUM 3 finding 2: "COV: NOT necessary"). All arms here
run with cov=0 (the native-replay-trained head, ADDENDUM 2 arm A); the COV
head is still trained and COV*_R2* choice-quality cells are still computed by
the unmodified harness code (dead weight, left in to keep the diff minimal),
but no arm in ARMS uses it. Seeds and --wake/--probe-steps/--ncont/--clen
defaults (600/200/6/4) are otherwise unchanged; tiebreak stays OFF (the
harness's build_B never passes proximity_approach_magnitude_tiebreak, so it is
at causal_grid_world.py's default False, same as ADDENDUM 3).

Original REPAIRS docstring below is preserved unchanged for provenance;
"ARMS (closed loop): ..." line describes the ORIGINAL script's arm set, not
this replication's (see ARMS dict for the actual arms run here):
...
DEPTH = 2
# Worker I pre-registration (chip-20260924-r5b-r2-fresh-seed-replication): FULL = R5b + R2, COV
# dropped throughout (ADDENDUM 3 finding 2: COV not necessary). Tuple = (r5, cov, r2).
...
def closed_loop_rewards(seed, enc_state, head, r5, r2, steps):
    ...
    _f, obs = env.reset(); agent.reset(); h.reset()
    rew, acts, log, ep = [], [], [], 0
    # Worker I addition (observational only, does not affect reward/action/entropy
    # arithmetic above, which is byte-identical to the original harness): tally
    # info["transition_type"] per step so harm contacts (true contact events) can
    # be reported separately from hazard-proximity steps, per Worker F's
    # correction (evaluation_edge_probe.py CONTACT set:
    # {agent_caused_hazard, env_caused_hazard, env_caused_multisource}).
    tt_counts = Counter()
    for _ in range(steps):
        r = h.step(obs)
        ...
        tt_counts[r.info.get("transition_type", "none") if isinstance(r.info, dict) else "none"] += 1
        obs = r.next_obs_dict
        if r.done:
    ...
    true_harm_contacts = int(sum(v for k, v in tt_counts.items() if k in CONTACT))
    hazard_proximity_steps = int(tt_counts.get("hazard_approach", 0))
    return {"steps": steps, "episodes_ended": ep, "reward_per_100": float(rew.sum() * 100 / steps),
            ...
            "transition_types": {str(k): v for k, v in sorted(tt_counts.items())},
            "true_harm_contacts_per_100": float(true_harm_contacts * 100 / steps),
            "hazard_proximity_steps_per_100": float(hazard_proximity_steps * 100 / steps),
