...
|---|---|---|---|---|
| **V3-EXQ-621a** | governance-cycle "cleared substrate-readiness diagnostic"; superseded V3-EXQ-621 (SD-054 scaffold) | PASS (`overall_pass=true`) | **autopsy-done 2026-06-06 (VACUOUS_PASS confirmed; correction applied)** | **VACUOUS_PASS-adjacent.** `overall_pass=true` while the load-bearing **`C2_z_goal_floor_met=false`** and `C1` only **6/12 cells completed**. The PASS is routed via C1-partial + C3 cascade; the substantive scientific gate (z_goal floor) FAILED. A partial triage exists (`z_goal_collapse_triage_2026-05-31.md`) but no full autopsy; the gate clear rests on the non-substantive criteria. |
...
1. **V3-EXQ-621a** -- *highest*. **[AUTOPSY DONE 2026-06-06 -> `failure_autopsy_V3-EXQ-621a_2026-06-06.{md,json}`; verdict VACUOUS_PASS confirmed; governance-side correction appended to the two `claims.yaml` evidence_quality_note blocks + the `substrate_queue.json` 621a note; no claim confidence moved (run non_contributory / scoring_excluded); fault self-corrected by `failure_autopsy_V3-EXQ-603d_2026-06-01`.]** A governance-cycle gate clear + supersession where `overall_pass=true`
