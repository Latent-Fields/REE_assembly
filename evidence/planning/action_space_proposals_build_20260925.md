...
- **Branch commit:** PENDING at the time of writing. The commit gate (the full contracts suite, run remotely) is in flight, and this record is updated with the sha once the push lands. The branch was fast-forwarded from `cc20be5` to origin/main (`6707bab`, which carries MECH-157 `1a61800` and MECH-287b `aa14769`, both touching `ree_core`), and the build was re-applied cleanly on top. Files: `ree_core/hippocampal/module.py`, `ree_core/utils/config.py`, `tests/contracts/test_action_space_proposals.py` (new), `tests/test_flag_inertness.py` (registry entry). No docs, experiments or queue on the branch (plan sec 6).
