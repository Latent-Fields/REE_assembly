...
- **Status:** ACCEPTED + IMPLEMENTED 2026-06-06. User signed off the design and all four open questions (Section 7) on the recommended path: (Q1) keep `/failure-autopsy`, add `/diagnostic-autopsy` alias; (Q2) checklist-first enforcement (harden to `validate_queue.py` later); (Q3) gate all three actions incl. thought-intake; (Q4) cover `diagnostic` + `baseline`. Implemented in session `implement-diagnostic-adjudication-gate-20260606T0828Z`. **Implementation note (discovered during build):** the indexer reads the *sync-normalized* `runs/**/manifest.json`, not the flat script manifest, so `interpretation` had to be carried through `sync_v3_results.py` (a 5th edit beyond the 4 parts below); governance.sh Step 2 DOES run that sync (its "no sync needed" comment is stale). Parts B+C+sync are read-only/backward-compatible and landed in code; Parts A+D landed in the three skills (both `.claude` + `.agents`). **Headless-safe subset landed (follow-up session `adjgate-headless-safe-subset-20260606T0937Z`):** (i) a non-blocking index-time WARN in `build_experiment_indexes.py` when a `diagnostic`/`baseline` manifest lacks `preconditions[]` (`adjudication=='unverified'`) — per-run only for runs on/after the `20260607` cutoff (the gate landed 2026-06-06, so same-day runs are legacy → aggregate-count-only, no flood; first run measured **283** legacy unverified, 0 post-convention); (ii) a DORMANT, strictly-keyed guard `adjudication_blocks_governance_action()` + `BLOCKING_ADJUDICATIONS` constant (canonical in the indexer, mirrored in `generate_pending_review`) keyed ONLY on `precondition_unmet`/`vacuous_pass` — never `unverified` — so it is inert against the legacy record and ready for the future headless-governance path to call. **Still deferred (the breakage-prone half):** the queueing presence-ENFORCEMENT belongs in `ree-v3/validate_experiments.py` (a static script-lint), NOT `validate_queue.py` (queue items carry no `interpretation`); and the live `governance.sh` hard-block. Both wait on (a) a few real post-convention diagnostics validating the convention and (b) the retrospective audit (`retrospective_diagnostic_selfroute_audit_*`) clarifying legacy handling.
