...
## 9. Addendum (2026-08-01T21:19:59Z) — "completely dead / 0.0" overstates this run's own finding

**Does not change the confirmed routing above** (the targeted diagnostic V3-EXQ-859 already ran per that routing, and V3-EXQ-863's full-budget replication is live as of this addendum — ree-cloud-3, claimed 20:04Z). This is a correction to how the finding should be read, surfaced while investigating a separate false-negative readiness gate in the same GOV-FANOUT-1 lineage (`failure_autopsy_V3-EXQ-847_2026-08-01.md`).

**What prompted the check.** V3-EXQ-847 (Leg P-D) self-routed `substrate_not_ready_requeue` off a genuine denominator bug (its `demotion_active_ticks` numerator is gated to fresh-selection ticks only, but divided by `n_p2_ticks`, which counts held/latched ticks too). Checking whether this run's own "MECH-448/449 measure EXACTLY 0.0 (completely dead)" finding rests on the same bug: **it does not.** This driver's tick loop (line ~1000) explicitly falls back to the last fresh diagnostic snapshot on a held tick (`diag = _fresh_diag if _fresh_diag is not None else (_prev_diag_snapshot or {})`), with a code comment stating this is "BIT-IDENTICAL to 654j" — numerator and denominator are on the same all-P2-ticks basis in both this run and its 654j template. No denominator mismatch here.

**But the raw manifest data show the "0.0" is a compound-gate artifact, not a magnitude-zero finding.** `mech448_demotion_lever_live_and_excluding_both_arms` reads `measured=0.0` against `threshold=2.0` because it counts cells clearing **both** `active_frac >= 0.8` **and** `excluded_count > 0` — a gate introduced in this run's generation that 654j was never held to (654j's own precondition for the same name only checks `excluded_count > 0`, threshold 0.0, which it trivially passes; 654j's own reported `f_eligibility_demotion_active_frac` is a flat `1.0` on every seed/arm, since its driver has no fresh-vs-held distinction at all — it's the same "BIT-IDENTICAL" fallback convention, just without a floor ever being checked against it).

Reading the raw per-seed values directly from both manifests:

| | V3-EXQ-654j (`cand_world_summary`) | V3-EXQ-851 (`lateral_pfc`) |
|---|---|---|
| `f_eligibility_excluded_count_mean` (when active) | 17.4 – 29.1 | 17.0 – 28.3 |
| `go_nogo_suppressed_per_tick_mean` (when active) | 1.5 – 13.2 | 1.5 – 12.2 |
| `f_eligibility_demotion_active_frac` (all P2 ticks, held ticks credited via snapshot) | **1.0** (every seed/arm) | **0.24 – 0.58** |

**The magnitude of MECH-448/449's effect, when engaged, is essentially at parity with the 654j baseline** — excluded-candidate counts and suppression rates overlap almost exactly. **What actually differs is duty cycle**: under `lateral_pfc` routing the mechanisms read as engaged (via the held-tick-carries-forward convention) on only ~24–58% of P2 ticks, versus ~100% under `cand_world_summary`. That is a real, still-worth-explaining effect of the route-source change — but it is a **reduced engagement frequency at preserved magnitude**, not the mechanisms "measuring EXACTLY 0.0" or being "completely dead" as Facts/§2/§7's learning-extracted line 1 state. Those phrasings should be read as describing the compound-gate outcome, not the underlying substrate behavior.

**Why this matters now, concretely:** V3-EXQ-858 (ree-cloud-4, claimed 12:44Z, still running — the ~20h Leg P-B allocation that was left running per user instruction) and V3-EXQ-863 (ree-cloud-3, claimed 20:04Z — the full-budget replication of V3-EXQ-859's short ablation) both exercise this identical `lateral_pfc` + MECH-448/449-armed configuration. When either lands, its result should be read against "reduced duty-cycle, preserved magnitude" rather than "complete collapse" — a full-magnitude-but-lower-frequency demotion/No-Go effect has different implications for whatever downstream DV those runs score (e.g. committed-class entropy accumulates across ticks, so a lower duty-cycle at full magnitude is a partial, not total, loss of the mechanism's influence) than a true zero-effect would. This also gives V3-EXQ-859's own "arms identical, doesn't reproduce 851" result (see `failure_autopsy_V3-EXQ-859_2026-08-01.md`) a candidate reconciling explanation worth checking when 863 lands: 859 measured active_frac against `n_p2_fresh_select` (fresh ticks only, no held-tick credit), a different statistic than 851/654j's all-ticks convention — so 859's clean 1.0-on-both-arms reading and 851's 0.24–0.58 all-ticks reading are not directly comparable on their face, and are not necessarily in tension.

Not independently verified against 847's own excluded_count/suppressed magnitudes (847's manifest does not record those fields), so this addendum does not extend the "preserved magnitude" claim to 847 — only the fresh-tick engagement rate (847's own denominator-corrected reading, 1.0 on every seed/arm) was checked there.

No change to `epistemic_category`, `evidence_direction`, or the confirmed routing. Recommending governance read this addendum alongside V3-EXQ-863's result when it lands, and consider whether the `evidence_quality_note` this autopsy feeds should be revised to "duty-cycle reduction" language rather than "collapse" language at that point.
