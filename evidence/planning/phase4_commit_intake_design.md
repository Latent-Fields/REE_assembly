...
| `evidence/planning/governance_flags.v1.json` | flag registry | typed `/flag/raise` + `/flag/resolve` verbs | added 2026-08-29 (completeness red-team C1): busiest uncovered writer on the fleet (88 commits/14d, multi-box, bot-authored) with a confirmed stranding incident already cited in root CLAUDE.md |
| `evidence/planning/inter_governance_workset.md` / `.v1.json` | derived regen (tick-rewritten) | joins the igw intake slice -- same client, same landing | added 2026-08-29 (C4): 56 commits/7d; DP-3's whole-set rule means the igw slice's benefit does not hold without it |
| `evidence/planning/experiment_proposals.v1.json` | editorial | CAS; part of the igw slice (the tick's `_commit_proposals()` is a call site) | added 2026-08-29 (C6): multi-box writer; without this row any tick commit pairing a routed ledger with this file stays git-entire under DP-3 |
| `scripts/steward/state/steward_ledger.jsonl` (+ `steward_state.json`, `steward_ref_pins.json`) | append/state (daily sweep) | later append slice; INTERIM RULE (lands with the campaign's W6): the steward sweep commits its append in the same step, never leaves it dirty | added 2026-08-29 (C2 BLOCKER): this file's uncommitted append kept REE_assembly dirty for hours on 2026-08-29, blocking every push-retry on the repo -- the R2 wedge-keeper |
| `chip_archive/<YYYY-MM>.json` + the TASK_CHIPS.json field strip | daily archiver | coordinator-side archive verb: strip the DB `entry_json` rows, materializer renders the stripped state; git-side strip retired | added 2026-08-29 (C7): post-cutover the git-side strip races the DB-authoritative render (resurrection risk -- the section-6 authority-inversion problem recurring on the registry plane) |
| `RECOMMENDATION_LOG.jsonl` sibling appends: `decision_log.v1.jsonl`, `scripts/stash_dispositions.json` | append | later append slices, same template | added 2026-08-29 (C11): low-frequency, near-single-writer; enumerated so the coverage claim stays a disposition, not an inference |
| `worktree_session_registry.json` / `.md`, `morning_agenda.md`, governance-regen outputs (`pending_review.md`, `promotion_demotion_recommendations.md`, `claims.json`, indexes) | regen | stays git-direct, JUSTIFIED: machine-scoped single-writer (registry: Mac-owned) or single-session claim-guarded regen | added 2026-08-29 (C5/C12): explicit disposition so the per-file completeness claim holds |
