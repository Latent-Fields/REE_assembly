...
| 2026-09-14 | C | HELD. Needs an interactive session to confirm a frontmatter'd skill still lists and invokes; inputs ready (Step 1 recipe above). |
| 2026-09-14 | B2 | HELD for user review. Inputs ready: WI-A findings file (146 rows, survive-column filled, do-not-tone-down list) and the WI-I cache baseline. Recommended first commit: replace AGENTS.md with a pointer to CLAUDE.md + NEW_AGENT_START_HERE.md (fixes five live contradictions, not a tone change). Second: the archaeology-disclaimer convention (49 verbatim repeats in CLAUDE.md, 31 in failure-autopsy -> one stated convention). Then the chip-volume exhortations. One section per commit, GOV-HELDOUT-1 on each. Also pending from H: widening the "Hooks" rule from PreToolUse to any event. |
