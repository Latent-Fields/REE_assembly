#!/usr/bin/env python3
"""Validate convergence packet receipt JSON files against schema."""
...
def _default_schema_path(repo_root: Path) -> Path:
    return repo_root / "evidence" / "planning" / "schemas" / "v1" / "convergence_packet_receipt.schema.json"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Path to convergence packet receipt schema JSON.",
    )
    parser.add_argument("--input", nargs="+", default=[], help="One or more receipt JSON file paths.")
    parser.add_argument(
        "--input-glob",
        default="",
        help="Glob pattern for receipt JSON files (absolute or repo-relative).",
    )
    ...
    if args.input_glob and not input_paths:
        print(f"No receipt files matched: {args.input_glob}")
        return 0
