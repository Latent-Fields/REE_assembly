...
## 12. Addendum (2026-09-10, GFLAG-0131) — eval-protocol confound inside the competence DV

**Recorded after confirmation. The disposition does NOT move:** `non_contributory` stands, and the INV-088 / MECH-457 peripheral co-tags stand (§3 unchanged). This section records a confound §2 and §6 did not name.

**Mechanism.** The eval reader is greedy-argmax (`x737.LatentPPOEvalPolicy`), while PPO trains by sampling and therefore never experiences its own argmax trajectory. The frozen policy falls into a deterministic small-footprint limit cycle at eval time. The env is `D3_hazard_free` (`num_hazards` 0, recorded in the episode log's `env_config`), but `ree_core/environment/causal_grid_world.py:135-140` still applies the SD-094 causal-footprint contamination rule by default (`contamination_spread` 0.5, `contamination_threshold` 2.0, `contaminated_harm` 0.4). That module's own docstring (`:71-78`, `:180-196`) warns that hazard-free probes contaminate themselves to death unless `contamination_spread=0.0` or `hazard_free_contamination_gate=True` is passed. **V3-EXQ-978 passes neither** — env kwargs come from `x724.ENV_KWARGS` via `x734._env_kwargs_for_rung`, and neither sets them. Dithering in a small footprint retypes the agent's own cells `contaminated` and then kills it.

**Measured from the seed-42 episode log** (12 episodes, 6 OFF + 6 ON, frozen policies): **10 of 12** episodes end at health 0.0 between steps **11 and 23**, each with 3 terminal `agent_caused_hazard` transitions; **9 of 12** visit 4 or fewer distinct cells (a strict two-cell cycle in 4 of them). Only **2** episodes run the full 200 steps, one of those a 5-cell footprint with ~195 no-op steps. **Total resource events across all 12 episodes: 2.** Against the manifest's `random_walk` anchor (`foraging_competence` 1.05, `survival_horizon` 48.1) the greedy reader scores below a random walk for a reason that has nothing to do with `z_world`.

**Consequence for the null.** The OFF/ON arm contrast is compressed toward zero by episode truncation *independently of what `z_world` carries* — roughly 11 of 200 steps of foraging opportunity in the typical cell. §2's "the bed is weak" and §6's `environment: too sparse` / `measurement: under-instrumented` rows record the symptoms; this is the mechanism behind them. **The OFF/ON null is therefore weaker evidence than the manifest reads** and must not be cited as a clean null. It does not become a positive result, and nothing here re-opens the run.

**Gate owed on the successor lineage — now retroactive.** The §7 discriminator should have pre-registered one of: a stochastic eval policy; `contamination_spread=0.0` / `hazard_free_contamination_gate=True`; or a survival-normalised DV. It did not. `chip-20260903-exq978-oracle-adapter-discriminator` was resolved `done` 2026-09-04 and the probe ran as **V3-EXQ-1002** (`v3_exq_1002_zworld_actor_adequacy_oracle_adapter_20260905T005017Z_v3`, FAIL, `non_contributory`, label `zworld_geometry_blocks_oracle_mapping_h_c_geometry_mismatch`) on 2026-09-05, and none of the three mitigations appears in its manifest (`contamination_spread` and `hazard_free_contamination_gate` absent; no stochastic-eval or survival-normalised DV declared). Its verdict is the H-C leg — exactly the reading this confound could manufacture. The gate is therefore owed **retroactively** against V3-EXQ-1002 before its H-C verdict is banked, and as a hard readiness gate on any further `zworld_actor_adequacy_locus` leg. Per the substrate docstring this is a documented, opt-out-able env trap, so the same gate belongs on **every hazard-free probe in this lineage**, not just this one.

**Provenance.** GFLAG-0131 (INV-088, MECH-457), verdict HOLDS-AS-STATED, re-derived independently from the in-repo episode log 2026-09-10. Two counts in the original flag text are corrected here: it is 10 of 12 episodes dying at steps 11-23 (not 9), and 2 episodes run to 200 (not 3); the "9 of 12" figure matches footprint ≤ 4 cells, not strictly two-cell.
