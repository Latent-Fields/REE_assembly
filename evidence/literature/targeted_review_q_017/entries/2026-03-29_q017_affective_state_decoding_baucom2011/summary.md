...
MECH-142 ("Valence-arousal axis orthogonality in the control plane is not a static geometric property but requires active cholinergic maintenance during learning; without it, axes drift toward correlation under repeated co-activation") cites this paper in its `notes` field. On review, `claim_ids_tested` was deliberately left as `["Q-017"]`: like Bush et al. (2018) in this same directory, this is a passive block-design study with no learning manipulation and no cholinergic measure -- it establishes that valence/arousal orthogonality exists as a recoverable neural geometry, but says nothing about whether that geometry is actively maintained or would drift under repeated co-activation without a gating mechanism, which is MECH-142's actual (and novel) mechanism claim. It supports the premise MECH-142 builds on, not the claim itself. See the Gonzalez-Redondo et al. (2025) entry in this directory, extended to MECH-142 on 2026-08-10, which directly tests a gated-vs-ungated contrast and is the paper's own cited "critical addition."
