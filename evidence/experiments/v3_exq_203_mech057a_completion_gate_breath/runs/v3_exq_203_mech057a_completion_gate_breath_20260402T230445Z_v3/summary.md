...
**Seeds:** [42, 123]
**Conditions:** GATE_ON vs GATE_ABLATED
**Warmup:** 400 eps  **Eval:** 50 eps
**Env:** CausalGridWorldV2 size=6, 4 hazards, nav_bias=0.45
**BreathOscillator:** period=50, sweep_amp=0.3, sweep_dur=10
...
Key improvement over EXQ-139: BreathOscillator (MECH-108) creates periodic uncommitted windows (sweep_duration=10 steps every 50 steps), preventing degenerate always-committed states.

GATE_ON: running_variance update suppressed while committed; accumulated errors flushed as batch at completion boundary (uncommitted transition). BetaGate active (elevates when committed, releases at completion). BreathOscillator-driven uncommitted windows.

GATE_ABLATED: continuous per-step running_variance updates; BetaGate disabled (always released). BreathOscillator still active (matching env dynamics).

## Pre-Registered Thresholds (need 4/5)
...
Criteria met: 3/5 -> **FAIL** (need 4/5)
