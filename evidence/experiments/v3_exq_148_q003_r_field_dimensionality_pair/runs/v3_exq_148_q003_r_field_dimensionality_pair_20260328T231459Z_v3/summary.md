...
|-----------|-----------|-------------|
| SCALAR | 0.44386 | 0.26021 |
| VECTOR | 0.44386 | 0.26021 |
...
| Metric | Value |
|--------|-------|
| dim_var_0 (harm intensity) | 0.0041 |
| dim_var_1 (harm recency)   | 0.0000 |
| dim_var_2 (benefit prox.)  | 0.0000 |
| benefit_dim2_mean          | 0.0000 |

### Per-Seed SCALAR
  seed=42 cond=SCALAR: harm=0.25600 benefit=0.34608
  seed=123 cond=SCALAR: harm=0.63173 benefit=0.17433

### Per-Seed VECTOR
  seed=42 cond=VECTOR: harm=0.25600 benefit=0.34608 dim_var=(0.0045,0.0000,0.0000) benefit_dim2=0.0000
  seed=123 cond=VECTOR: harm=0.63173 benefit=0.17433 dim_var=(0.0036,0.0000,0.0000) benefit_dim2=0.0000
...
Q-003 INCONCLUSIVE: vector dims collapsed. harm_ratio=1.0000. dim_var=(0.0041,0.0000,0.0000). All dims track the same pattern -- benefit terrain not populated or recency RBF collapses to intensity. Q-003 not answerable with current data density.
...
- C2 FAIL: dim_var (0.0041,0.0000,0.0000) -- one or more dims < 0.01. Vector dims have collapsed: all three dimensions track the same scalar pattern. The vector residue field adds no new information. Q-003 is not answerable at current scale with this implementation.
- C3 FAIL: mean_benefit_dim2=0.0000 < 0.01. Benefit terrain (dim 2 of vector) is near-zero -- insufficient benefit events accumulated during training to populate the benefit RBF field. Benefit terrain is not contributing to the vector's informational content.
